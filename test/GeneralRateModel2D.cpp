...
TEST_CASE("GRM2D Jacobian forward vs backward flow",
		  "[GRM2D],[UnitOp],[Residual],[Jacobian],[fixGRM2D]") // todo fix. off by some tolerance
{
	// Test all WENO orders
	for (unsigned int i = 1; i <= cadet::Weno::maxOrder(); ++i)
...
TEST_CASE("GRM2D forward sensitivity vs FD",
		  "[GRM2D],[Sensitivity],[Simulation],[failedFDtestGRM2D]") // todo fix. off by a bigger tolerance
{
	...
	cadet::test::column::testFwdSensSolutionFD("GENERAL_RATE_MODEL_2D", "FV", false, fdStepSize, absTols, relTols,
											   passRatio);
}

TEST_CASE("GRM2D forward sensitivity forward vs backward flow",
		  "[GRM2D],[Sensitivity],[Simulation],[fixGRM2D]") // todo fix. off by a lot
{
...
TEST_CASE("GRM2D consistent initialization with SMA binding",
		  "[GRM2D],[ConsistentInit],[fixGRM2D]") // todo fix. adjust tolerances?
{
...
TEST_CASE("GRM2D consistent sensitivity initialization with linear binding",
		  "[GRM2D],[ConsistentInit],[Sensitivity],[CIgrm2d]")
{
	...
	cadet::test::column::testConsistentInitializationSensitivity("GENERAL_RATE_MODEL_2D", "FV", y.data(), yDot.data(),
																 true, 1e-14);
}

TEST_CASE("GRM2D consistent sensitivity initialization with SMA binding",
		  "[GRM2D],[ConsistentInit],[Sensitivity],[CIgrm2d]")
{
	// Fill state vector with given initial values
...
TEST_CASE("GRM2D linear binding single particle matches particle distribution",
		  "[GRM2D],[Simulation],[ParticleType],[CIgrm2d]")
{
	cadet::test::particle::testLinearMixedParticleTypes("GENERAL_RATE_MODEL_2D", "FV", 5e-8, 5e-5);
}

TEST_CASE("GRM2D multiple particle types Jacobian analytic vs AD",
		  "[GRM2D],[Jacobian],[AD],[ParticleType],[fixGRM2D]") // todo fix. AD and analytical Jacobians dont match
{
...
TEST_CASE("GRM2D multiple particle types time derivative Jacobian vs FD",
		  "[GRM2D],[UnitOp],[Residual],[Jacobian],[ParticleType],[FDtestGRM2D]")
{
	cadet::test::particle::testTimeDerivativeJacobianMixedParticleTypesFD("GENERAL_RATE_MODEL_2D", "FV", 1e-6, 0.0,
																		  5e-3);
}
...
TEST_CASE("GRM2D multiple spatially dependent particle types flux Jacobian vs FD",
		  "[GRM2D],[UnitOp],[Residual],[Jacobian],[ParticleType],[FDtestGRM2D]") // todo fix. only one assertion is
																				 // slightly off (5.0401-6 != 5.0247-6)
{
...
TEST_CASE("GRM2D dynamic reactions time derivative Jacobian vs FD bulk",
		  "[GRM2D],[Jacobian],[Residual],[ReactionModel],[FDtestGRM2D]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL_2D", "FV", true, false,
...
TEST_CASE("GRM2D dynamic reactions time derivative Jacobian vs FD particle",
		  "[GRM2D],[Jacobian],[Residual],[ReactionModel],[FDtestGRM2D]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL_2D", "FV", false, true,
																		false, 1e-6, 1e-14, 8e-4);
}
...
TEST_CASE("GRM2D dynamic reactions time derivative Jacobian vs FD bulk and particle",
		  "[GRM2D],[Jacobian],[Residual],[ReactionModel],[FDtestGRM2D]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL_2D", "FV", true, true,
																		false, 1e-6, 1e-14, 8e-4);
}
...
TEST_CASE("GRM2D dynamic reactions time derivative Jacobian vs FD bulk and modified particle",
		  "[GRM2D],[Jacobian],[Residual],[ReactionModel],[FDtestGRM2D]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL_2D", "FV", true, true, true,
																		1e-6, 1e-14, 8e-4);
}
...
TEST_CASE("GRM2D multi particle types dynamic reactions time derivative Jacobian vs FD bulk",
		  "[GRM2D],[Jacobian],[Residual],[ReactionModel],[ParticleType],[FDtestGRM2D]")
{
