...
TEST_CASE("GRM numerical Benchmark with parameter sensitivities for linear case",
		  "[GRM],[FV],[Simulation],[Reference],[Sensitivity],[dddff]") // todo CI flag: currently only runs locally but
																	   // fails on server
{
...
TEST_CASE("GRM numerical Benchmark with parameter sensitivities for SMA LWE case",
		  "[GRM],[FV],[Simulation],[Reference],[Sensitivity],[dddff]") // todo CI flag: currently only runs locally but
																	   // fails on server
{
...
TEST_CASE("GRM numerical EOC Benchmark with parameter sensitivities for linear case",
		  "[GRM],[FV],[releaseCI],[EOC],[EOC_GRM_FV],[dddff]")
{
	const std::string& modelFilePath = std::string("/data/model_GRM_dynLin_1comp_benchmark1.json");
	...
	cadet::test::column::testEOCReferenceBenchmark(modelFilePath, refFilePath, convFilePath, "001", absTol, relTol, 3,
												   disc, true);
}

TEST_CASE("GRM numerical EOC Benchmark with parameter sensitivities for SMA LWE case",
		  "[GRM],[FV],[releaseCI],[EOC],[EOC_GRM_FV],[dddff]")
{
	const std::string& modelFilePath = std::string("/data/model_GRM_reqSMA_4comp_benchmark1.json");
	...
	cadet::test::column::FVparams disc(8, 1);
	cadet::test::column::testEOCReferenceBenchmark(modelFilePath, refFilePath, convFilePath, "000", absTol, relTol, 2,
												   disc, true);
}
...
TEST_CASE("GRM rapid-equilibrium binding with surf diff par dep flux Jacobian vs FD",
		  "[GRM],[FV],[UnitOp],[Residual],[Jacobian],[ParameterDependence],[CI],[FD]")
{
...
TEST_CASE("GRM dynamic binding with surf diff par dep Jacobian vs AD",
		  "[GRM],[FV],[UnitOp],[Residual],[Jacobian],[ParameterDependence],[CI]")
{
...
// TEST_CASE("GRM forward sensitivity vs FD", "[GRM],[FV],[Sensitivity],[Simulation],[failedFDtestGRM]") // todo fix.
// fails for SMA_KA for both binding modes
//{
//	// todo comment
...
// }
//
// TEST_CASE("GRM forward sensitivity forward vs backward flow", "[GRM],[FV],[Sensitivity],[Simulation],[fixGRM]") //
...
TEST_CASE("GRM consistent sensitivity initialization with linear binding",
		  "[GRM],[FV],[ConsistentInit],[Sensitivity],[CI]")
{
	...
	cadet::test::column::testConsistentInitializationSensitivity("GENERAL_RATE_MODEL", "FV", y.data(), yDot.data(),
																 true, 1e-14);
}

TEST_CASE("GRM consistent sensitivity initialization with SMA binding",
		  "[GRM],[FV],[ConsistentInit],[Sensitivity],[CI]")
{
	// Fill state vector with given initial values
	const unsigned int numDofs = 4 + 4 * 16 + 16 * 4 * (4 + 4) + 4 * 16;
...
TEST_CASE("GRM linear binding single particle matches particle distribution",
		  "[GRM],[FV],[Simulation],[ParticleType],[CI]")
{
...
TEST_CASE("GRM multiple particle types time derivative Jacobian vs FD",
		  "[GRM],[FV],[UnitOp],[Residual],[Jacobian],[ParticleType],[CI]")
{
...
TEST_CASE("GRM multiple spatially dependent particle types Jacobian analytic vs AD",
		  "[GRM],[FV],[Jacobian],[AD],[ParticleType],[CI]")
{
...
TEST_CASE("GRM multiple spatially dependent particle types flux Jacobian vs FD",
		  "[GRM],[FV],[UnitOp],[Residual],[Jacobian],[ParticleType],[CI],[FD]")
{
...
TEST_CASE("GRM dynamic reactions Jacobian vs AD bulk and modified particle",
		  "[GRM],[FV],[Jacobian],[AD],[ReactionModel],[CI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("GENERAL_RATE_MODEL", "FV", true, true, true);
}

TEST_CASE("GRM dynamic reactions time derivative Jacobian vs FD bulk",
		  "[GRM],[FV],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "FV", true, false, false,
																		1e-6, 1e-14, 9e-4);
}
...
TEST_CASE("GRM dynamic reactions time derivative Jacobian vs FD particle",
		  "[GRM],[FV],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "FV", false, true, false,
																		1e-6, 1e-14, 9e-4);
}
...
TEST_CASE("GRM dynamic reactions time derivative Jacobian vs FD modified particle",
		  "[GRM],[FV],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "FV", false, true, true,
																		1e-6, 1e-14, 9e-4);
}
...
TEST_CASE("GRM dynamic reactions time derivative Jacobian vs FD bulk and particle",
		  "[GRM],[FV],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "FV", true, true, false,
																		1e-6, 1e-14, 9e-4);
}
...
TEST_CASE("GRM dynamic reactions time derivative Jacobian vs FD bulk and modified particle",
		  "[GRM],[FV],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "FV", true, true, true,
																		1e-6, 1e-14, 9e-4);
}
...
TEST_CASE("GRM multi particle types dynamic reactions Jacobian vs AD bulk",
		  "[GRM],[FV],[Jacobian],[AD],[ReactionModel],[ParticleType],[CI]")
{
...
TEST_CASE("GRM multi particle types dynamic reactions Jacobian vs AD particle",
		  "[GRM],[Jacobian],[FV],[AD],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("GRM multi particle types dynamic reactions time derivative Jacobian vs FD bulk",
		  "[GRM],[FV],[Jacobian],[Residual],[ReactionModel],[ParticleType],[CI],[FD]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
