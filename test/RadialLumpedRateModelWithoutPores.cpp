...
TEST_CASE(
	"Radial LRM numerical Benchmark with parameter sensitivities for linear case",
	"[RadLRM],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails on server
{
...
TEST_CASE("Radial LRM transport Jacobian", "[RadLRM],[UnitOp],[Jacobian],[CI]")
{
	cadet::JsonParameterProvider jpp =
		createColumnLinearBenchmark(false, true, "RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", "FV");
...
{
	cadet::JsonParameterProvider jpp =
		createColumnWithTwoCompLinearBinding("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", "FV");

	cadet::test::column::testFwdSensJacobians(jpp, 1e-4, 3e-7, 5e-5);
}

// TEST_CASE("Radial LRM forward sensitivity vs FD", "[RadLRM],[Sensitivity],[Simulation],[failedFDtestLRM],[FD]") //
// todo (for all models) find tolerances
//{
...
//	cadet::test::column::testFwdSensSolutionFD("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", false, fdStepSize, absTols,
// relTols, passRatio);
// }
//
// TEST_CASE("Radial LRM forward sensitivity forward vs backward flow", "[RadLRM],[Sensitivity],[Simulation],[fixLRM]")
// // todo (for all models) find tolerances? why is there a pass ratio here, shouldnt this be precise?
//{
...
//	cadet::test::column::testFwdSensSolutionForwardBackward("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", absTols, relTols,
// passRatio);
// }

TEST_CASE("Radial LRM consistent initialization with linear binding", "[RadLRM],[ConsistentInit],[ReleaseCI]")
{
	cadet::test::column::testConsistentInitializationLinearBinding("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", "FV",
																   1e-12, 1e-12);
}

// TEST_CASE("Radial LRM consistent initialization with SMA binding", "[RadLRM],[ConsistentInit],[fixLRM]") // todo (for
// all models) fix
//{
...
//
//	cadet::test::column::testConsistentInitializationSMABinding("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", y.data(),
// 1e-14, 1e-5);
// }

TEST_CASE("Radial LRM consistent sensitivity initialization with linear binding",
		  "[RadLRM],[ConsistentInit],[Sensitivity],[ReleaseCI]")
{
	...
	cadet::test::column::testConsistentInitializationSensitivity("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", "FV",
																 y.data(), yDot.data(), true, 1e-12);
}

TEST_CASE("Radial LRM consistent sensitivity initialization with SMA binding",
		  "[RadLRM],[ConsistentInit],[Sensitivity],[ReleaseCI]")
{
	// Fill state vector with given initial values
	const unsigned int numDofs = 4 + 16 * (4 + 4);
...
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", "FV", true,
															  false, false);
}

TEST_CASE("Radial LRM dynamic reactions Jacobian vs AD modified bulk",
		  "[RadLRM],[Jacobian],[AD],[ReactionModel],[ReleaseCI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", "FV", true,
															  false, true);
}

TEST_CASE("Radial LRM dynamic reactions time derivative Jacobian vs FD bulk",
		  "[RadLRM],[Jacobian],[Residual],[ReactionModel],[ReleaseCI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", "FV",
																		true, false, false, 1e-6, 1e-14, 8e-4);
}

TEST_CASE("Radial LRM dynamic reactions time derivative Jacobian vs FD modified bulk",
		  "[RadLRM],[Jacobian],[Residual],[ReactionModel],[ReleaseCI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES", "FV",
																		true, false, true, 1e-6, 1e-14, 8e-4);
}
