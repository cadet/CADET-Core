...
json createColumnWithSMAJson(const std::string& uoType, const std::string& spatialMethod)
{
	...
	if (uoType.substr(0, 6) == "RADIAL")
	{
		...
		config["CYLINDER_HEIGHT"] = height;
		config["CROSS_SECTION_AREA_OUTER"] = inletA;
		config["CROSS_SECTION_AREA_INNER"] = height * 2.0 * pi * (0.024 - 0.014); // optional and only used to double-check the geometry
	}
	...
	particle["INIT_CP"] = {50.0, 0.0, 0.0, 0.0};
	...
	// Discretization
	{
		...
		discPar["PAR_DISC_TYPE"] = std::string("EQUIDISTANT");
...
json createColumn2ParType1GeneralRate1HomoParticleBothWithTwoCompLinearJson(const std::string& uoType, const std::string& spatialMethod)
{
	...
	particle["SURFACE_DIFFUSION"] = { 1e-10, 1e-10 };
	...
	particle["HAS_SURFACE_DIFFUSION"] = true;
	config["particle_type_000"] = particle;
	particle["HAS_PORE_DIFFUSION"] = false;
	particle["HAS_SURFACE_DIFFUSION"] = false;
...
json createColumnWithTwoCompLinearJson(const std::string& uoType, const std::string& spatialMethod)
{
	...
	if (uoType.substr(0, 6) == "RADIAL")
	{
		...
		config["CYLINDER_HEIGHT"] = radCylinderHeight;
		config["CROSS_SECTION_AREA_OUTER"] = 2.0 * pi * 0.024 * radCylinderHeight;
		config["CROSS_SECTION_AREA_INNER"] = 2.0 * pi * 0.01 * radCylinderHeight; // optional, only used for check
	}
	else if (uoType.substr(0, 7) == "FRUSTUM")
	{
		config["GEOMETRY"] = "AXIAL_FLOW_FRUSTUM";
	...
	// Model
	{
		...
		// GRM - unit 000
		{
			...
			if (uoType.substr(0, 6) == "RADIAL")
			{
				...
				grm["CROSS_SECTION_AREA_INNER"] = 2.0 * pi * 0.001 * radCylinderHeight; // optional, only used for check
				grm["CROSS_SECTION_AREA_OUTER"] = 2.0 * pi * 0.004 * radCylinderHeight;
				grm["BED_LENGTH"] = 0.003;
			}
			else if (uoType.substr(0, 7) == "FRUSTUM")
			{
