...
void printHelp()
{
    log::emit() << "Usage: createRegTestSCL <FILE>" << log::endl;
    ...
    log::emit() << "When called with no arguments, default file name is regSCL.h5." << log::endl;
...
{
    std::string fileName("regSCL.h5");  // default output filename
...
//                        par_disc_vector.at(2)   = 1.0;
    ...
    double              abstol                  = 1e-11; // check
    double              reltol                  = 0.0; // check
    double              abstol_sens_ref         = 1e-9; // check
