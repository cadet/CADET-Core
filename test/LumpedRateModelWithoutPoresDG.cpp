...
TEST_CASE("LRM_DG linear pulse vs analytic solution", "[LRM],[DG],[Simulation],[Analytic],[CI]")
{
	...
	cadet::test::column::testAnalyticBenchmark("LUMPED_RATE_MODEL_WITHOUT_PORES", "/data/lrm-pulseBenchmark.data", true,
											   true, disc, 2e-5, 1e-7);
	cadet::test::column::testAnalyticBenchmark("LUMPED_RATE_MODEL_WITHOUT_PORES", "/data/lrm-pulseBenchmark.data", true,
											   false, disc, 2e-5, 1e-7);
	cadet::test::column::testAnalyticBenchmark("LUMPED_RATE_MODEL_WITHOUT_PORES", "/data/lrm-pulseBenchmark.data",
											   false, true, disc, 2e-5, 1e-7);
	cadet::test::column::testAnalyticBenchmark("LUMPED_RATE_MODEL_WITHOUT_PORES", "/data/lrm-pulseBenchmark.data",
											   false, false, disc, 2e-5, 1e-7);
}

TEST_CASE("LRM_DG non-binding linear pulse vs analytic solution",
		  "[LRM],[DG],[Simulation],[Analytic],[NonBinding],[CI]")
{
	cadet::test::column::DGparams disc;
...
TEST_CASE("LRM_DG numerical Benchmark with parameter sensitivities for linear case",
		  "[LRM],[DG],[Simulation],[Reference],[Sensitivity],[CI]")
{
...
TEST_CASE("LRM_DG numerical Benchmark with parameter sensitivities for SMA LWE case",
		  "[LRM],[DG],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails on
															   // server
{
...
// TEST_CASE("LRM_DG forward sensitivity vs FD", "[LRM],[DG],[Sensitivity],[Simulation]") // todo fix tolerances (also
// for FV)
//{
...
//	cadet::test::column::testFwdSensSolutionFD("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", false, fdStepSize, absTols,
// relTols, passRatio);
// }

// TEST_CASE("LRM_DG forward sensitivity forward vs backward flow", "[LRM],[DG],[Sensitivity],[Simulation]") // todo fix
// (also for FV) tolerances? why is there a pass ratio here, shouldnt this be precise?
//{
...
//	cadet::test::column::testFwdSensSolutionForwardBackward("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", absTols, relTols,
// passRatio);
// }

TEST_CASE("LRM_DG consistent initialization with linear binding", "[LRM],[DG],[ConsistentInit],[CI]")
{
	cadet::test::column::testConsistentInitializationLinearBinding("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", 1e-12,
																   1e-12);
}

// TEST_CASE("LRM_DG consistent initialization with SMA binding", "[LRM],[DG],[ConsistentInit]") // todo fix (also for
// FV)
//{
...
//
//	cadet::test::column::testConsistentInitializationSMABinding("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", y.data(),
// 1e-14, 1e-5);
// }

TEST_CASE("LRM_DG consistent sensitivity initialization with linear binding",
		  "[LRM],[DG],[ConsistentInit],[Sensitivity],[CI]")
{
	...
	cadet::test::column::testConsistentInitializationSensitivity("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", y.data(),
																 yDot.data(), true, 1e-14);
}

//// todo fix: sigsev read access violation when allocating _tempState = new double[numDofs()] in configure model
/// discretization
// TEST_CASE("LRM_DG consistent sensitivity initialization with SMA binding",
// "[LRM],[DG],[ConsistentInit],[Sensitivity],[todo]")
//{
//	// Fill state vector with given initial values
//	const unsigned int numDofs = 4 + 10 * (4 + 4);
//	std::vector<double> y(numDofs, 0.0);
...
TEST_CASE("LRM_DG transport Jacobian", "[LRM],[FV],[UnitOp],[Jacobian],[CI]")
{
	cadet::JsonParameterProvider jpp =
		createColumnLinearBenchmark(false, true, "LUMPED_RATE_MODEL_WITHOUT_PORES", "DG");
...
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", true, false,
															  false, std::numeric_limits<float>::epsilon() * 100.0);
}

TEST_CASE("LRM_DG dynamic reactions Jacobian vs AD modified bulk", "[LRM],[DG],[Jacobian],[AD],[ReactionModel],[CI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", true, false,
															  true, std::numeric_limits<float>::epsilon() * 100.0);
}

TEST_CASE("LRM_DG dynamic reactions time derivative Jacobian vs FD bulk",
		  "[LRM],[DG],[Jacobian],[Residual],[ReactionModel],[CI]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", true,
																		false, false, 1e-6, 1e-14, 8e-4);
}

TEST_CASE("LRM_DG dynamic reactions time derivative Jacobian vs FD modified bulk",
		  "[LRM],[DG],[Jacobian],[Residual],[ReactionModel],[CI]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("LUMPED_RATE_MODEL_WITHOUT_PORES", "DG", true,
																		false, true, 1e-6, 1e-14, 8e-4);
}
