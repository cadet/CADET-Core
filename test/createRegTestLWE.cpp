...
void printHelp()
{
    log::emit() << "Usage: createRegTestLWE <FILE>" << log::endl;
    ...
    log::emit() << "When called with no arguments, default file name is regLWE.h5." << log::endl;
...
{
    std::string fileName("regLWE.h5");  // default output filename
...
//                        par_disc_vector.at(2)   = 1.0;
    ...
    for (int t = 1; t <= 20; t += 1)
        user_solution_times.push_back(9.0 + 0.1 * static_cast<double>(t));

    for (int t = 12; t <= 89; t += 1)
        user_solution_times.push_back(t);
    for (int t = 1; t <= 20; t += 1)
        user_solution_times.push_back(89.0 + 0.1 * static_cast<double>(t));
    for (int t = 92; t <= 1500; t += 1)
        user_solution_times.push_back(t);
    ...
    bool                write_solution_all              = false; // check
    bool                write_sens_column_outlet        = true; // check
    bool                write_sens_all                  = false; // check
    ...
    double              abstol                  = 1e-08; // check
    double              reltol                  = 0.0; // check
    double              abstol_sens_ref         = 1e-10; // check
