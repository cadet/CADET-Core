 ...
 */
inline cadet::IUnitOperation* createAndConfigureUnit(cadet::IModelBuilder& mb, cadet::JsonParameterProvider& jpp,
													 cadet::test::column::DiscParams& disc)
{
...
void setNumRadCells(cadet::JsonParameterProvider& jpp, unsigned int nRad, std::string unitID,
					const bool mctModel = false)
{
	int level = 0;
...
void setNumericalMethod(cadet::IParameterProvider& pp, nlohmann::json& setupJson, const std::string unitID,
						const bool copy = false)
{
	// copy over numerical methods from reference file. Note that we leave out spatial and time step resolution
	// parameters
	pp.pushScope("model");
	pp.pushScope("solver");
	...
	if (pp.exists("NBOUND"))
		discretization["NBOUND"] = pp.getIntArray("NBOUND"); // note: in the future this might be included somewhere
															 // else in the setup as its part of the model
	...
	if (pp.exists("PAR_GEOM")) // note: in the future this might be included somewhere else in the setup as its part of
							   // the model
...
void copyMultiplexData(cadet::IParameterProvider& pp, nlohmann::json& setupJson, const std::string unitID)
{
	...
	if (pp.exists("PORE_ACCESSIBILITY_MULTIPLEX"))
		setupJson["model"]["unit_" + unitID]["PORE_ACCESSIBILITY_MULTIPLEX"] =
			pp.getInt("PORE_ACCESSIBILITY_MULTIPLEX");

	if (pp.exists("REACTION_MODEL_PARTICLES_MULTIPLEX"))
		setupJson["model"]["unit_" + unitID]["REACTION_MODEL_PARTICLES_MULTIPLEX"] =
			pp.getInt("REACTION_MODEL_PARTICLES_MULTIPLEX");
...
void testAnalyticBenchmark(cadet::JsonParameterProvider jpp, const char* refFileRelPath, bool forwardFlow,
						   bool dynamicBinding, double absTol, double relTol)
{
	if (!forwardFlow)
		reverseFlow(jpp);
...
void testAnalyticBenchmark(const char* uoType, const char* refFileRelPath, bool forwardFlow, bool dynamicBinding,
						   DiscParams& disc, const std::string method, double absTol, double relTol)
{
	const std::string fwdStr = (forwardFlow ? "forward" : "backward");
	SECTION(method + ": Analytic " + fwdStr + " flow with " + (dynamicBinding ? "dynamic" : "quasi-stationary") +
			" binding")
	{
		// Setup simulation
...
void testAnalyticBenchmark(const char* uoType, const char* refFileRelPath, bool forwardFlow, bool dynamicBinding,
						   FVparams& disc, double absTol, double relTol)
{
	testAnalyticBenchmark(uoType, refFileRelPath, forwardFlow, dynamicBinding, disc, "FV", absTol, relTol);
}

void testAnalyticBenchmark(const char* uoType, const char* refFileRelPath, bool forwardFlow, bool dynamicBinding,
						   DGparams& disc, double absTol, double relTol)
{
	testAnalyticBenchmark(uoType, refFileRelPath, forwardFlow, dynamicBinding, disc, "DG", absTol, relTol);
}

void testAnalyticNonBindingBenchmark(const char* uoType, const char* refFileRelPath, bool forwardFlow, DiscParams& disc,
									 const std::string method, double absTol, double relTol)
{
	const std::string fwdStr = (forwardFlow ? "forward" : "backward");
...
void testAnalyticNonBindingBenchmark(const char* uoType, const char* refFileRelPath, bool forwardFlow, FVparams& disc,
									 double absTol, double relTol)
{
	testAnalyticNonBindingBenchmark(uoType, refFileRelPath, forwardFlow, disc, "FV", absTol, relTol);
}

void testAnalyticNonBindingBenchmark(const char* uoType, const char* refFileRelPath, bool forwardFlow, DGparams& disc,
									 double absTol, double relTol)
{
...
void testJacobianAD(cadet::JsonParameterProvider& jpp, const double absTolFDpattern)
{
	...
	cadet::test::checkJacobianPatternFD(unitAna, unitAD, y.data(), nullptr, jacDir.data(), jacCol1.data(),
										jacCol2.data(), tls, absTolFDpattern);
	...
	SECTION("Forward then backward flow (nonzero state)")
	{
		...
		if (jpp.getString("UNIT_TYPE").substr(0, 6) == "RADIAL")
		{
			...
			const bool paramSet = unitAna->setParameter(
				cadet::makeParamId(cadet::hashString("VELOCITY_COEFF"), 0, cadet::CompIndep, cadet::ParTypeIndep,
								   cadet::BoundStateIndep, cadet::ReactionIndep, cadet::SectionIndep),
				-jpp.getDouble("VELOCITY_COEFF"));
			REQUIRE(paramSet);
			// Reverse flow
		...
		// jacCol1.data(), jacCol2.data()); 				cadet::test::compareJacobianFD(unitAna, unitAD, y.data(),
...
void testJacobianWenoForwardBackwardFD(const std::string& uoType, const std::string& spatialMethod, int wenoOrder,
...
void testTimeDerivativeJacobianFD(const std::string& uoType, const std::string& spatialMethod, double h, double absTol,
								  double relTol)
{
	cadet::IModelBuilder* const mb = cadet::createModelBuilder();
	REQUIRE(nullptr != mb);
...
void testFwdSensJacobians(cadet::JsonParameterProvider jpp, double h, double absTol, double relTol,
...
void testFwdSensSolutionFD(const std::string& uoType, const std::string& spatialMethod, bool disableSensErrorTest,
						   double const* fdStepSize, double const* absTols, double const* relTols,
...
void testFwdSensSolutionForwardBackward(const std::string& uoType, const std::string& spatialMethod,
...
void testConsistentInitializationLinearBinding(const std::string& uoType, const std::string& spatialMethod,
											   double consTol, double absTol, const int reqBnd, const int useAD)
{
	cadet::IModelBuilder* const mb = cadet::createModelBuilder();
	REQUIRE(nullptr != mb);
...
void testConsistentInitializationSMABinding(const std::string& uoType, const std::string& spatialMethod,
											double const* const initState, double consTol, double absTol,
											const int reqBnd, const int useAD)
{
	cadet::IModelBuilder* const mb = cadet::createModelBuilder();
	REQUIRE(nullptr != mb);
...
void testConsistentInitializationSensitivity(const std::string& uoType, const std::string& spatialMethod,
											 double const* const y, double const* const yDot, bool linearBinding,
											 double absTol, const int reqBnd, const int useAD)
{
	cadet::IModelBuilder* const mb = cadet::createModelBuilder();
	REQUIRE(nullptr != mb);
...
void testReferenceBenchmark(const std::string& modelFileRelPath, const std::string& refFileRelPath,
							const std::string& unitID, const std::vector<double> absTol,
							const std::vector<double> relTol, const DiscParams& disc, const bool compare_sens,
							const int simDataStride)
{
...
void testEOCReferenceBenchmark(const std::string& modelFileRelPath, const std::string& refFileRelPath,
							   const std::string& convFileRelPath, const std::string& unitID,
							   const std::vector<double> absTol, const std::vector<double> relTol,
							   const unsigned int nDisc, const DiscParams& startDisc, const bool compare_sens)
{
	const int unitOpID = std::stoi(unitID);
	...
	if (it != discZ.end())
		...
		throw std::out_of_range("discretization N_e^z = " + std::to_string(startNCol) +
								" not found in convergence reference data");
	if (startNPar > 0 && discP[discIdx] != static_cast<double>(startNPar))
		throw std::out_of_range("discretization N_e^p = " + std::to_string(startNPar) +
								" not found in convergence reference data at the same index as N_e^z");
	...
	for (int disc = 1; disc <= nDisc; disc++)
	{
		...
		CHECK((LinfErrors[0][disc - 1]) ==
			  cadet::test::makeApprox(LinfErrors_ref[discIdx + disc - 1], relTol[0], absTol[0]));

		if (disc > 1)
		{
			L1EOC[0][disc - 1] = std::log(L1Errors[0][disc - 1] / L1Errors[0][disc - 2]) /
								 std::log(std::pow(2, disc - 1) / std::pow(2, disc));
			LinfEOC[0][disc - 1] = std::log(LinfErrors[0][disc - 1] / LinfErrors[0][disc - 2]) /
								   std::log(std::pow(2, disc - 1) / std::pow(2, disc));
			CAPTURE(LinfEOC[0][disc - 1]);
			CAPTURE(LinfEOC_ref[discIdx + disc - 1]);
			CHECK((LinfEOC[0][disc - 1]) ==
				  cadet::test::makeApprox(LinfEOC_ref[discIdx + disc - 1], relTol[0], absTol[0]));
		}
		...
		if (nSens && compare_sens)
		{
			...
			{
				// get reference errors and EOC (to this end get sensitivity name to push convergence scope to reference
				// solution)
				pp_ref.pushScope("input");
				pp_ref.pushScope("sensitivity");
				std::string sensParam = std::to_string(sensID);
				...
				std::transform(approximation, approximation + ref_sens.size(), ref_sens.begin(), absErrors.begin(),
							   [](double a, double b) { return std::abs(a - b); });
				L1Errors[sensID + 1][disc - 1] =
					std::accumulate(absErrors.begin(), absErrors.end(), 0.0) / ref_sens.size();
				...
				CAPTURE(LinfErrors_ref[discIdx + disc - 1]);
				CHECK((LinfErrors[sensID + 1][disc - 1]) == cadet::test::makeApprox(LinfErrors_ref[discIdx + disc - 1],
																					relTol[1 + sensID],
																					absTol[1 + sensID]));

				if (disc > 1)
				{
					L1EOC[sensID + 1][disc - 1] =
						std::log(L1Errors[sensID + 1][disc - 1] / L1Errors[sensID + 1][disc - 2]) /
						std::log(std::pow(2, disc - 1) / std::pow(2, disc));
					LinfEOC[sensID + 1][disc - 1] =
						std::log(LinfErrors[sensID + 1][disc - 1] / LinfErrors[sensID + 1][disc - 2]) /
						std::log(std::pow(2, disc - 1) / std::pow(2, disc));
					CAPTURE(LinfEOC[sensID + 1][disc - 1]);
					CAPTURE(LinfEOC_ref[discIdx + disc - 1]);
					CHECK((LinfEOC[sensID + 1][disc - 1]) == cadet::test::makeApprox(LinfEOC_ref[discIdx + disc - 1],
																					 relTol[1 + sensID],
																					 absTol[1 + sensID]));
				}
				pp_ref.popScope();
...
void testForeignReferenceBenchmark(const std::string& configFileRelPath, const std::string& refFileRelPath,
								   const std::string& unitID, const double absTol, const double relTol,
								   const int compIdx)
{
	const int unitOpID = std::stoi(unitID);
	...
	// check if only one specific component is considered in the reference solution and set stride and offset
	// accordingly
