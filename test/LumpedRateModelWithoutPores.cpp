...
TEST_CASE("LRM non-binding linear pulse vs analytic solution",
		  "[LRM],[FV],[Simulation],[Reference],[Analytic],[NonBinding],[CI]")
{
	cadet::test::column::FVparams disc(1024);
...
TEST_CASE("LRM numerical Benchmark with parameter sensitivities for linear case",
		  "[LRM],[FV],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails on
															   // server
{
...
TEST_CASE("LRM numerical Benchmark with parameter sensitivities for SMA LWE case",
		  "[LRM],[FV],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails on
															   // server
{
	...
	const std::vector<double> absTol = {
		1e-12, 1e-0, 1e-12, 1e-12}; // todo: why is such a high tolerance required for first sensitivity parameter
...
// TEST_CASE("LRM forward sensitivity vs FD", "[LRM],[FV],[Sensitivity],[Simulation],[failedFDtestLRM],[FD]") // todo
// fix tolerances
//{
...
// }
//
// TEST_CASE("LRM forward sensitivity forward vs backward flow", "[LRM],[FV],[Sensitivity],[Simulation],[fixLRM]") //
// todo fix tolerances? why is there a pass ratio here, shouldnt this be precise?
//{
...
{
	cadet::test::column::testConsistentInitializationLinearBinding("LUMPED_RATE_MODEL_WITHOUT_PORES", "FV", 1e-12,
																   1e-12);
}
...
TEST_CASE("LRM consistent sensitivity initialization with linear binding",
		  "[LRM],[FV],[ConsistentInit],[Sensitivity],[CI]")
{
	...
	cadet::test::column::testConsistentInitializationSensitivity("LUMPED_RATE_MODEL_WITHOUT_PORES", "FV", y.data(),
																 yDot.data(), true, 1e-14);
}

TEST_CASE("LRM consistent sensitivity initialization with SMA binding",
		  "[LRM],[FV],[ConsistentInit],[Sensitivity],[CI]")
{
	// Fill state vector with given initial values
	const unsigned int numDofs = 4 + 16 * (4 + 4);
...
TEST_CASE("LRM transport Jacobian", "[LRM],[FV],[UnitOp],[Jacobian],[CI]")
{
	cadet::JsonParameterProvider jpp =
		createColumnLinearBenchmark(false, true, "LUMPED_RATE_MODEL_WITHOUT_PORES", "FV");
...
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("LUMPED_RATE_MODEL_WITHOUT_PORES", "FV", true, false,
															  false);
}
...
TEST_CASE("LRM dynamic reactions time derivative Jacobian vs FD bulk",
		  "[LRM],[FV],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("LUMPED_RATE_MODEL_WITHOUT_PORES", "FV", true,
