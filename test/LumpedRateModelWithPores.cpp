...
TEST_CASE("LRMP numerical Benchmark with parameter sensitivities for linear case",
		  "[LRMP],[FV],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails
																// on server
{
...
TEST_CASE("LRMP numerical Benchmark with parameter sensitivities for SMA LWE case",
		  "[LRMP],[FV],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails
																// on server
{
...
TEST_CASE("LRMP numerical EOC Benchmark with parameter sensitivities for linear case",
		  "[releaseCI],[EOC],[EOC_LRMP_FV]")
{
	const std::string& modelFilePath = std::string("/data/model_LRMP_dynLin_1comp_benchmark1.json");
	...
	cadet::test::column::testEOCReferenceBenchmark(modelFilePath, refFilePath, convFilePath, "001", absTol, relTol, 4,
												   disc, true);
}

TEST_CASE("LRMP numerical EOC Benchmark with parameter sensitivities for SMA LWE case",
		  "[releaseCI],[EOC],[EOC_LRMP_FV]")
{
	const std::string& modelFilePath = std::string("/data/model_LRMP_reqSMA_4comp_benchmark1.json");
...
// TEST_CASE("LRMP forward sensitivity forward vs backward flow", "[LRMP],[FV],[Sensitivity],[Simulation],[fixLRMP]") //
// todo fix
//{
...
TEST_CASE("LRMP consistent sensitivity initialization with linear binding",
		  "[LRMP],[FV],[ConsistentInit],[Sensitivity],[CI]")
{
	...
	cadet::test::column::testConsistentInitializationSensitivity("LUMPED_RATE_MODEL_WITH_PORES", "FV", y.data(),
																 yDot.data(), true, 1e-14);
}

TEST_CASE("LRMP consistent sensitivity initialization with SMA binding",
		  "[LRMP],[FV],[ConsistentInit],[Sensitivity],[CI]")
{
	// Fill state vector with given initial values
	const unsigned int numDofs = 4 + 4 * 16 + 16 * (4 + 4) + 4 * 16;
...
TEST_CASE("LRMP linear binding single particle matches particle distribution",
		  "[LRMP],[FV],[Simulation],[ParticleType],[CI]")
{
...
TEST_CASE("LRMP multiple particle types time derivative Jacobian vs FD",
		  "[LRMP],[FV],[UnitOp],[Residual],[Jacobian],[ParticleType],[CI],[FD]")
{
	cadet::test::particle::testTimeDerivativeJacobianMixedParticleTypesFD("LUMPED_RATE_MODEL_WITH_PORES", "FV", 1e-6,
																		  0.0, 9e-4);
}

TEST_CASE("LRMP multiple spatially dependent particle types Jacobian analytic vs AD",
		  "[LRMP],[FV],[Jacobian],[AD],[ParticleType],[CI]")
{
...
TEST_CASE("LRMP dynamic reactions Jacobian vs AD bulk and modified particle",
		  "[LRMP],[FV],[Jacobian],[AD],[ReactionModel],[CI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("LUMPED_RATE_MODEL_WITH_PORES", "FV", true, true, true);
}

TEST_CASE("LRMP dynamic reactions time derivative Jacobian vs FD bulk",
		  "[LRMP],[FV],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("LUMPED_RATE_MODEL_WITH_PORES", "FV", true,
...
TEST_CASE("LRMP dynamic reactions time derivative Jacobian vs FD particle",
		  "[LRMP],[FV],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("LUMPED_RATE_MODEL_WITH_PORES", "FV", false,
...
TEST_CASE("LRMP multi particle types dynamic reactions Jacobian vs AD bulk",
		  "[LRMP],[FV],[Jacobian],[AD],[ReactionModel],[ParticleType],[CI]")
{
...
TEST_CASE("LRMP multi particle types dynamic reactions time derivative Jacobian vs FD bulk",
		  "[LRMP],[FV],[Jacobian],[Residual],[ReactionModel],[ParticleType],[CI],[FD]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
