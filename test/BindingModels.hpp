...
/**
 * @brief Emits code for a test that checks whether Jacobian columns belonging to non-binding liquid phase components are all zero
		...
		{ \
			...
			{ \
				...
				{ \
					...
					{ \
						cadet::test::binding::testNonBindingConsistency(modelName, sizeof(nBound3) / sizeof(unsigned int), nBound3, isKinetic, "{" configSomeNon "}", useAD, state3); \
					} \
				} \
			} \
		} \
		...
		{ \
			...
			{ \
				...
				{ \
					...
					SECTION(std::string("Use AD ") + (useAD ? "yes" : "no")) \
					{ \
						cadet::test::binding::testNonbindingBindingConsistency(modelName, sizeof(nBound2) / sizeof(unsigned int), sizeof(nBound3) / sizeof(unsigned int), nBound2, nBound3, isKinetic, "{" configAll "}", "{" configSomeNon "}", useAD, state2, state3); \
					} \
				} \
			} \
		} \
	...
	{ \
		...
		{ \
			...
			{ \
				...
				{ \
					cadet::test::binding::testJacobianAD(modelName, sizeof(nBound2) / sizeof(unsigned int), nBound2, isKinetic, "{" configAll "}", state2); \
				} \
				SECTION("With nonbinding components") \
				{ \
					cadet::test::binding::testJacobianAD(modelName, sizeof(nBound3) / sizeof(unsigned int), nBound3, isKinetic, "{" configSomeNon "}", state3); \
				} \
			} \
		} \
	} \
	TEST_CASE(modelName " binding model time derivative Jacobian vs FD", "[Jacobian],[BindingModel]," tagName) \
	{ \
		const unsigned int nBound2[] = BRACED_INIT_LIST allBinding; \
		const unsigned int nBound3[] = BRACED_INIT_LIST someNonBinding; \
		for (int bindMode = 0; bindMode < 2; ++bindMode) \
		{ \
			...
			{ \
				SECTION("Without nonbinding components") \
				{ \
					cadet::test::binding::testTimeDerivativeJacobianFD(modelName, sizeof(nBound2) / sizeof(unsigned int), nBound2, isKinetic, "{" configAll "}"); \
				} \
				SECTION("With nonbinding components") \
				{ \
					cadet::test::binding::testTimeDerivativeJacobianFD(modelName, sizeof(nBound3) / sizeof(unsigned int), nBound3, isKinetic, "{" configSomeNon "}"); \
				} \
			} \
		} \
	} \
	TEST_CASE(modelName " binding model multiplyWithDerivativeJacobian() vs jacobianAddDiscretized()", "[Jacobian],[BindingModel]," tagName) \
	{ \
		const unsigned int nBound2[] = BRACED_INIT_LIST allBinding; \
		const unsigned int nBound3[] = BRACED_INIT_LIST someNonBinding; \
		for (int bindMode = 0; bindMode < 2; ++bindMode) \
		{ \
			...
			{ \
				SECTION("Without nonbinding components") \
				{ \
					cadet::test::binding::testTimeDerivativeJacobianMultiplyFunction(modelName, sizeof(nBound2) / sizeof(unsigned int), nBound2, isKinetic, "{" configAll "}"); \
				} \
				SECTION("With nonbinding components") \
				{ \
					cadet::test::binding::testTimeDerivativeJacobianMultiplyFunction(modelName, sizeof(nBound3) / sizeof(unsigned int), nBound3, isKinetic, "{" configSomeNon "}"); \
				} \
			} \
		} \
	} \
	TEST_CASE(modelName " binding model consistency of algebraic equation functions", "[BindingModel]," tagName) \
	{ \
		const unsigned int nBound2[] = BRACED_INIT_LIST allBinding; \
		const unsigned int nBound3[] = BRACED_INIT_LIST someNonBinding; \
		for (int bindMode = 0; bindMode < 2; ++bindMode) \
		{ \
			...
			{ \
				SECTION("Without nonbinding components") \
				{ \
					cadet::test::binding::testConsistencyOfAlgebraicEquationFunctions(modelName, sizeof(nBound2) / sizeof(unsigned int), nBound2, isKinetic, "{" configAll "}"); \
				} \
				SECTION("With nonbinding components") \
				{ \
					cadet::test::binding::testConsistencyOfAlgebraicEquationFunctions(modelName, sizeof(nBound3) / sizeof(unsigned int), nBound3, isKinetic, "{" configSomeNon "}"); \
				} \
			} \
		} \
	} \
	TEST_CASE(modelName " binding model consistent initialization", "[BindingModel],[ConsistentInit]," tagName) \
	{ \
		const unsigned int nBound2[] = BRACED_INIT_LIST allBinding; \
		const unsigned int nBound3[] = BRACED_INIT_LIST someNonBinding; \
		const double state2[] = BRACED_INIT_LIST stateAll; \
			...
			{ \
				...
				{ \
					...
					{ \
						SECTION("Without nonbinding components") \
						{ \
							cadet::test::binding::testConsistentInitialization(modelName, sizeof(nBound2) / sizeof(unsigned int), nBound2, isKinetic, "{" configAll "}", useAD, state2, consInitTol, consInitCheckTol); \
						} \
						SECTION("With nonbinding components") \
						{ \
							cadet::test::binding::testConsistentInitialization(modelName, sizeof(nBound3) / sizeof(unsigned int), nBound3, isKinetic, "{" configSomeNon "}", useAD, state3, consInitTol, consInitCheckTol); \
						} \
					} \
				} \
			} \
	...
	{ \
		...
		{ \
			...
			{ \
				SECTION("Without nonbinding components") \
				{ \
					cadet::test::binding::testNormalExternalConsistency(modelName, extModelName, sizeof(nBound2) / sizeof(unsigned int), nBound2, isKinetic, "{" configAll "," extConfigAll "}", state2); \
				} \
				SECTION("With nonbinding components") \
				{ \
					cadet::test::binding::testNormalExternalConsistency(modelName, extModelName, sizeof(nBound3) / sizeof(unsigned int), nBound3, isKinetic, "{" configSomeNon "," extConfigSomeNon "}", state3); \
				} \
			} \
		} \
	}
...
/**
 * @brief Emits tests for a binding model that can have external function dependence and does not allow non-binding components
 * @param modelName Identifier of the model as string (e.g. "LINEAR")
 * @param extModelName Identifier of the externally dependent model as string (e.g. "EXT_LINEAR")
 * @param allBinding Array with number of bound states in parentheses (e.g., (1, 1, 2))
 * @param stateAll Array with full state vector (liquid and solid phase) in parentheses
