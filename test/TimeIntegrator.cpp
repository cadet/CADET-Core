...
TimeIntegrator::TimeIntegrator()
	: _model(nullptr), _idaMemBlock(nullptr), _vecStateY(nullptr), _vecStateYdot(nullptr), _absTol(1, 1.0e-8),
	  _relTol(1.0e-6), _initStepSize(1, 1.0e-6), _maxSteps(10000), _maxStepSize(0.0), _maxNewtonIter(3),
	  _maxErrorTestFail(7), _maxConvTestFail(10), _curSec(0)
{
}
...
void TimeIntegrator::configureTimeIntegrator(double relTol, double absTol, double initStepSize, unsigned int maxSteps,
											 double maxStepSize)
{
	_absTol.clear();
...
void TimeIntegrator::configureTimeIntegrator(double relTol, double absTol, const std::vector<double>& initStepSizes,
...
void TimeIntegrator::setSectionTimes(const std::vector<double>& sectionTimes,
									 const std::vector<bool>& sectionContinuity)
{
...
void TimeIntegrator::integrate()
{
	...
	while (curT < tEnd)
	{
		...
		const double endTime =
			writeAtUserTimes ? std::min(_sectionTimes[_curSec + skip], tEnd) : _sectionTimes[_curSec + skip];
		curT = startTime;
