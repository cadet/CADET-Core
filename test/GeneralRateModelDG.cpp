...
TEST_CASE("GRM_DG linear pulse vs analytic solution", "[GRM],[DG],[Simulation],[Analytic],[CI]")
{
	cadet::test::column::DGparams disc;
	cadet::test::column::testAnalyticBenchmark("GENERAL_RATE_MODEL", "/data/grm-pulseBenchmark.data", true, true, disc,
											   6e-5, 1e-7);
	cadet::test::column::testAnalyticBenchmark("GENERAL_RATE_MODEL", "/data/grm-pulseBenchmark.data", true, false, disc,
											   6e-5, 1e-7);
	cadet::test::column::testAnalyticBenchmark("GENERAL_RATE_MODEL", "/data/grm-pulseBenchmark.data", false, true, disc,
											   6e-5, 1e-7);
	cadet::test::column::testAnalyticBenchmark("GENERAL_RATE_MODEL", "/data/grm-pulseBenchmark.data", false, false,
											   disc, 6e-5, 1e-7);
}

TEST_CASE("GRM_DG non-binding linear pulse vs analytic solution",
		  "[GRM],[DG],[Simulation],[Analytic],[NonBinding],[CI]")
{
	cadet::test::column::DGparams disc;
	cadet::test::column::testAnalyticNonBindingBenchmark("GENERAL_RATE_MODEL", "/data/grm-nonBinding.data", true, disc,
														 6e-5, 1e-7);
	cadet::test::column::testAnalyticNonBindingBenchmark("GENERAL_RATE_MODEL", "/data/grm-nonBinding.data", false, disc,
														 6e-5, 1e-7);
}
...
TEST_CASE("GRM_DG numerical Benchmark with parameter sensitivities for linear case",
		  "[GRM],[DG],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails on
															   // server
{
	const std::string& modelFilePath = std::string("/data/model_GRM_dynLin_1comp_benchmark1.json");
	const std::string& refFilePath =
		std::string("/data/ref_GRM_dynLin_1comp_sensbenchmark1_cDG_P3Z8_GSM_parP3parZ1.h5");
...
TEST_CASE("GRM_DG numerical Benchmark with parameter sensitivities for SMA LWE case",
		  "[GRM],[DG],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails on
															   // server
{
	const std::string& modelFilePath = std::string("/data/model_GRM_reqSMA_4comp_benchmark1.json");
	const std::string& refFilePath =
		std::string("/data/ref_GRM_reqSMA_4comp_sensbenchmark1_cDG_P3Z8_GSM_parP3parZ1.h5");
	const std::vector<double> absTol = {1e-12, 1e-12, 1e-12, 1e-12};
	const std::vector<double> relTol = {1.0, 1.0, 1.0, 1.0};
...
TEST_CASE("GRM_DG LWE DGSEM and GSM particle discretization yields similar accuracy", "[GRM],[DG],[Simulation],[CI]")
{
	...
	for (unsigned int i = 0; i < GSMData->numDataPoints() * GSMData->numInletPorts() * nComp;
		 ++i, ++GSMOutlet, ++DGSEMOutlet)
	{
...
TEST_CASE("GRM_DG time derivative Jacobian vs FD", "[GRM],[DG],[UnitOp],[Residual],[Jacobian],[CI],[FD]")
{
	cadet::test::column::testTimeDerivativeJacobianFD("GENERAL_RATE_MODEL", "DG", 1e-6, 0.0, 9e-4);
}

// TEST_CASE("GRM_DG dynamic binding with surf diff par dep Jacobian vs AD",
// "[GRM],[DG],[UnitOp],[Residual],[Jacobian],[ParameterDependence],[fix]")
//{
//	cadet::test::column::testJacobianADVariableParSurfDiff("GENERAL_RATE_MODEL", "DG", true);
// }

TEST_CASE("GRM_DG rapid-equilibrium binding with surf diff par dep Jacobian vs AD",
		  "[GRM],[DG],[UnitOp],[Residual],[Jacobian],[ParameterDependence]") // todo include in CI (runs locally but
																			 // fails on server with linux)
{
	cadet::test::column::testJacobianADVariableParSurfDiff("GENERAL_RATE_MODEL", "DG", false);
}

TEST_CASE("GRM_DG sensitivity Jacobians",
		  "[GRM],[DG],[UnitOp],[Sensitivity]") // todo does not run on CI but locally on windows
{
...
// TEST_CASE("GRM_DG forward sensitivity vs FD", "[GRM],[DG],[Sensitivity],[Simulation],[todo]")
//{
...
//	cadet::test::column::testFwdSensSolutionFD("GENERAL_RATE_MODEL", "DG", false, fdStepSize, absTols, relTols,
// passRatio);
// }

//// todo fix: not just adjust tolerances as in FV but theres an actual error here: access violation in densematrix
// TEST_CASE("GRM_DG forward sensitivity forward vs backward flow", "[GRM],[DG],[Sensitivity],[Simulation],[todo]")
//{
...
TEST_CASE("GRM_DG consistent initialization with linear binding", "[GRM],[DG],[ConsistentInit],[CI]")
{
...
// TEST_CASE("GRM_DG consistent initialization with SMA binding", "[GRM],[DG],[ConsistentInit],[todo]")
//{
...
//	cadet::test::util::populate(y.data(), [](unsigned int idx) { return std::abs(std::sin(idx * 0.13)) + 1e-4; }, 4 + 4
//* 16); 	cadet::test::util::repeat(y.data() + 4 + 4 * 16, bindingCell, 16, 4 * 16 / 2);
//	cadet::test::util::populate(y.data() + 4 + 4 * 16 + 16 * 4 * (4 + 4), [](unsigned int idx) { return
// std::abs(std::sin(idx * 0.13)) + 1e-4; }, 4 * 16);
//
...
TEST_CASE("GRM_DG consistent sensitivity initialization with linear binding",
		  "[GRM],[DG],[ConsistentInit],[Sensitivity],[CI]")
{
	...
	// cadet::test::column::testConsistentInitializationSensitivity("GENERAL_RATE_MODEL", "DG", y.data(), yDot.data(),
	// true, 1e-14, 0, 0);
	cadet::test::column::testConsistentInitializationSensitivity("GENERAL_RATE_MODEL", "DG", y.data(), yDot.data(),
																 true, 1e-14, 1, 0);
	// cadet::test::column::testConsistentInitializationSensitivity("GENERAL_RATE_MODEL", "DG", y.data(), yDot.data(),
...
// TEST_CASE("GRM_DG consistent sensitivity initialization with SMA binding",
// "[GRM],[DG],[ConsistentInit],[Sensitivity],[fffffffiujbnlk]")
//{
//	// Fill state vector with given initial values
//	const unsigned int numDofs = 4 + 4 * 16 + 16 * 4 * (4 + 4) + 4 * 16;
//	std::vector<double> y(numDofs, 0.0);
...
//	cadet::test::util::populate(y.data(), [](unsigned int idx) { return std::abs(std::sin(idx * 0.13)) + 1e-4; }, 4 + 4
//* 16); 	cadet::test::util::repeat(y.data() + 4 + 4 * 16, bindingCell, 8, 4 * 16);
// cadet::test::util::populate(y.data()
//+ 4 + 4 * 16 + 16 * 4 * (4 + 4), [](unsigned int idx) { return std::abs(std::sin(idx * 0.13)) + 1e-4; }, 4 * 16);
//
//	cadet::test::util::populate(yDot.data(), [](unsigned int idx) { return std::abs(std::sin(idx * 0.9)) + 1e-4; },
// numDofs);
//
//	cadet::test::column::testConsistentInitializationSensitivity("GENERAL_RATE_MODEL", "DG", y.data(), yDot.data(),
// false, 1e-9);
...
TEST_CASE("GRM_DG inlet DOF Jacobian", "[GRM],[DG],[UnitOp],[Jacobian],[Inlet],[CI]")
{
	cadet::test::column::testInletDofJacobian("GENERAL_RATE_MODEL", "DG");
}

TEST_CASE("GRM_DG transport Jacobian", "[GRM],[DG],[UnitOp],[Jacobian],[CI]")
{
...
// }
//
// TEST_CASE("GRM_DG linear binding single particle matches particle distribution",
// "[GRM],[DG],[Simulation],[ParticleType],[todo]")
//{
//	cadet::test::particle::testLinearMixedParticleTypes("GENERAL_RATE_MODEL", "DG", 5e-8, 5e-5);
// }
//
// TEST_CASE("GRM_DG multiple particle types Jacobian analytic vs AD",
// "[GRM],[DG],[Jacobian],[AD],[ParticleType],[todo]")
//{
//	cadet::test::particle::testJacobianMixedParticleTypes("GENERAL_RATE_MODEL", "DG");
// }
//
// TEST_CASE("GRM_DG multiple particle types time derivative Jacobian vs FD",
// "[GRM],[DG],[UnitOp],[Residual],[Jacobian],[ParticleType],[todo]")
//{
//	cadet::test::particle::testTimeDerivativeJacobianMixedParticleTypesFD("GENERAL_RATE_MODEL", "DG", 1e-6, 0.0, 9e-4);
// }
//
// TEST_CASE("GRM_DG multiple spatially dependent particle types Jacobian analytic vs AD",
// "[GRM],[DG],[Jacobian],[AD],[ParticleType],[todo]")
//{
//	cadet::test::particle::testJacobianSpatiallyMixedParticleTypes("GENERAL_RATE_MODEL", "DG");
// }
//
// TEST_CASE("GRM_DG linear binding single particle matches spatially dependent particle distribution",
// "[GRM],[DG],[Simulation],[ParticleType],[todo]")
//{
...
TEST_CASE("GRM_DG dynamic reactions Jacobian vs AD modified particle",
		  "[GRM],[DG],[Jacobian],[AD],[ReactionModel],[CI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("GENERAL_RATE_MODEL", "DG", false, true, true);
}

TEST_CASE("GRM_DG dynamic reactions Jacobian vs AD bulk and particle",
		  "[GRM],[DG],[Jacobian],[AD],[ReactionModel],[CI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("GENERAL_RATE_MODEL", "DG", true, true, false);
}

TEST_CASE("GRM_DG dynamic reactions Jacobian vs AD bulk and modified particle",
		  "[GRM],[DG],[Jacobian],[AD],[ReactionModel],[CI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("GENERAL_RATE_MODEL", "DG", true, true, true);
}

TEST_CASE("GRM_DG dynamic reactions time derivative Jacobian vs FD bulk",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "DG", true, false, false,
																		1e-6, 1e-14, 9e-4);
}

TEST_CASE("GRM_DG dynamic reactions time derivative Jacobian vs FD particle",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "DG", false, true, false,
																		1e-6, 1e-14, 9e-4);
}

TEST_CASE("GRM_DG dynamic reactions time derivative Jacobian vs FD modified particle",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "DG", false, true, true,
																		1e-6, 1e-14, 9e-4);
}

TEST_CASE("GRM_DG dynamic reactions time derivative Jacobian vs FD bulk and particle",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "DG", true, true, false,
																		1e-6, 1e-14, 9e-4);
}

TEST_CASE("GRM_DG dynamic reactions time derivative Jacobian vs FD bulk and modified particle",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[CI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("GENERAL_RATE_MODEL", "DG", true, true, true,
																		1e-6, 1e-14, 9e-4);
}
...
TEST_CASE("GRM_DG multi particle types dynamic reactions Jacobian vs AD bulk",
		  "[GRM],[DG],[Jacobian],[AD],[ReactionModel],[ParticleType],[CI]")
{
...
TEST_CASE("GRM_DG multi particle types dynamic reactions Jacobian vs AD particle",
		  "[GRM],[DG],[Jacobian],[AD],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("GRM_DG multi particle types dynamic reactions Jacobian vs AD modified particle",
		  "[GRM],[DG],[Jacobian],[AD],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("GRM_DG multi particle types dynamic reactions Jacobian vs AD bulk and particle",
		  "[GRM],[DG],[Jacobian],[AD],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("GRM_DG multi particle types dynamic reactions Jacobian vs AD bulk and modified particle",
		  "[GRM],[DG],[Jacobian],[AD],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("GRM_DG multi particle types dynamic reactions time derivative Jacobian vs FD bulk",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD(jpp, true, false, false, 1e-6, 1e-14, 9e-4);
}

TEST_CASE("GRM_DG multi particle types dynamic reactions time derivative Jacobian vs FD particle",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("GRM_DG multi particle types dynamic reactions time derivative Jacobian vs FD modified particle",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("GRM_DG multi particle types dynamic reactions time derivative Jacobian vs FD bulk and particle",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("GRM_DG multi particle types dynamic reactions time derivative Jacobian vs FD bulk and modified particle",
		  "[GRM],[DG],[Jacobian],[Residual],[ReactionModel],[ParticleType],[CI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
