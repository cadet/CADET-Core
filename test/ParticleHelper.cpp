...
void scrambleParticleTypeFractionsSpatially(cadet::JsonParameterProvider& jpp, unsigned int nParType)
{
	...
	unsigned int nRad = 1;
	{
		auto ds = cadet::test::util::makeOptionalGroupScope(jpp, "discretization");
		if (jpp.exists("SPATIAL_METHOD"))
			nCol = jpp.getString("SPATIAL_METHOD") == "DG" ? (jpp.getInt("POLYDEG") + 1) * jpp.getInt("NELEM")
														   : jpp.getInt("NCOL");
		else
...
void testTimeDerivativeJacobianMixedParticleTypesFD(const std::string& uoType, const std::string& spatialMethod,
