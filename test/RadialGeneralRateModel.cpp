...
TEST_CASE(
	"Radial GRM numerical Benchmark with parameter sensitivities for linear case",
	"[RadGRM],[Simulation],[Reference],[Sensitivity]") // todo CI flag: currently only runs locally but fails on server
{
...
TEST_CASE("Radial GRM rapid-equilibrium binding flux Jacobian vs FD",
		  "[RadGRM],[UnitOp],[Residual],[Jacobian],[ReleaseCI],[FD]")
{
	cadet::test::column::testArrowHeadJacobianFD("RADIAL_GENERAL_RATE_MODEL", false, 1e-6, 2e-9);
}

TEST_CASE("Radial GRM rapid-equilibrium binding with surf diff par dep flux Jacobian vs FD",
		  "[RadGRM],[UnitOp],[Residual],[Jacobian],[ParameterDependence],[ReleaseCI],[FD]")
{
	cadet::test::column::testArrowHeadJacobianFDVariableParSurfDiff("RADIAL_GENERAL_RATE_MODEL", 1e-6, 5e-9);
}

TEST_CASE("Radial GRM dynamic binding with surf diff par dep Jacobian vs AD",
		  "[RadGRM],[UnitOp],[Residual],[Jacobian],[ParameterDependence],[ReleaseCI]")
{
	cadet::test::column::testJacobianADVariableParSurfDiff("RADIAL_GENERAL_RATE_MODEL", "FV", true);
}

TEST_CASE("Radial GRM rapid-equilibrium binding with surf diff par dep Jacobian vs AD",
		  "[RadGRM],[UnitOp],[Residual],[Jacobian],[ParameterDependence],[ReleaseCI]")
{
...
// TEST_CASE("Radial GRM forward sensitivity vs FD", "[RadGRM],[Sensitivity],[Simulation],[failedFDtestGRM]") // todo
// fix. fails for SMA_KA for both binding modes
//{
//	// todo comment
...
//	cadet::test::column::testFwdSensSolutionFD("RADIAL_GENERAL_RATE_MODEL", false, fdStepSize, absTols, relTols,
// passRatio);
// }
//
// TEST_CASE("Radial GRM forward sensitivity forward vs backward flow", "[RadGRM],[Sensitivity],[Simulation],[fixGRM]")
// // todo fix. fails for COL_DISPERION for both binding modes
//{
...
TEST_CASE("Radial GRM consistent sensitivity initialization with linear binding",
		  "[RadGRM],[ConsistentInit],[Sensitivity],[ReleaseCI]")
{
	...
	cadet::test::column::testConsistentInitializationSensitivity("RADIAL_GENERAL_RATE_MODEL", "FV", y.data(),
																 yDot.data(), true, 1e-14);
}

TEST_CASE("Radial GRM consistent sensitivity initialization with SMA binding",
		  "[RadGRM],[ConsistentInit],[Sensitivity],[ReleaseCI]")
{
	// Fill state vector with given initial values
	const unsigned int numDofs = 4 + 4 * 16 + 16 * 4 * (4 + 4) + 4 * 16;
...
TEST_CASE("Radial GRM LWE one vs two identical particle types match",
		  "[RadGRM],[Simulation],[ParticleType],[ReleaseCI]")
{
...
TEST_CASE("Radial GRM linear binding single particle matches particle distribution",
		  "[RadGRM],[Simulation],[ParticleType],[ReleaseCI]")
{
	cadet::test::particle::testLinearMixedParticleTypes("RADIAL_GENERAL_RATE_MODEL", "FV", 5e-8, 5e-5);
}

TEST_CASE("Radial GRM multiple particle types Jacobian analytic vs AD",
		  "[RadGRM],[Jacobian],[AD],[ParticleType],[ReleaseCI]")
{
	cadet::test::particle::testJacobianMixedParticleTypes("RADIAL_GENERAL_RATE_MODEL", "FV");
}

TEST_CASE("Radial GRM multiple particle types time derivative Jacobian vs FD",
		  "[RadGRM],[UnitOp],[Residual],[Jacobian],[ParticleType],[ReleaseCI]")
{
	cadet::test::particle::testTimeDerivativeJacobianMixedParticleTypesFD("RADIAL_GENERAL_RATE_MODEL", "FV", 1e-6, 0.0,
																		  9e-4);
}

TEST_CASE("Radial GRM multiple spatially dependent particle types Jacobian analytic vs AD",
		  "[RadGRM],[Jacobian],[AD],[ParticleType],[ReleaseCI]")
{
	cadet::test::particle::testJacobianSpatiallyMixedParticleTypes("RADIAL_GENERAL_RATE_MODEL", "FV");
}

TEST_CASE("Radial GRM linear binding single particle matches spatially dependent particle distribution",
		  "[RadGRM],[Simulation],[ParticleType],[ReleaseCI]")
{
	cadet::test::particle::testLinearSpatiallyMixedParticleTypes("RADIAL_GENERAL_RATE_MODEL", "FV", 5e-8, 5e-5);
}

TEST_CASE("Radial GRM multiple spatially dependent particle types flux Jacobian vs FD",
		  "[RadGRM],[UnitOp],[Residual],[Jacobian],[ParticleType],[ReleaseCI],[FD]")
{
	cadet::test::particle::testArrowHeadJacobianSpatiallyMixedParticleTypes("RADIAL_GENERAL_RATE_MODEL", 1e-6, 1e-8,
																			1e-5);
}
...
TEST_CASE("Radial GRM dynamic reactions Jacobian vs AD particle",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ReleaseCI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("RADIAL_GENERAL_RATE_MODEL", "FV", false, true, false);
}

TEST_CASE("Radial GRM dynamic reactions Jacobian vs AD modified particle",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ReleaseCI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("RADIAL_GENERAL_RATE_MODEL", "FV", false, true, true);
}

TEST_CASE("Radial GRM dynamic reactions Jacobian vs AD bulk and particle",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ReleaseCI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("RADIAL_GENERAL_RATE_MODEL", "FV", true, true, false);
}

TEST_CASE("Radial GRM dynamic reactions Jacobian vs AD bulk and modified particle",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ReleaseCI]")
{
	cadet::test::reaction::testUnitJacobianDynamicReactionsAD("RADIAL_GENERAL_RATE_MODEL", "FV", true, true, true);
}

TEST_CASE("Radial GRM dynamic reactions time derivative Jacobian vs FD bulk",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ReleaseCI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("RADIAL_GENERAL_RATE_MODEL", "FV", true, false,
																		false, 1e-6, 1e-14, 9e-4);
}

TEST_CASE("Radial GRM dynamic reactions time derivative Jacobian vs FD particle",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ReleaseCI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("RADIAL_GENERAL_RATE_MODEL", "FV", false, true,
																		false, 1e-6, 1e-14, 9e-4);
}

TEST_CASE("Radial GRM dynamic reactions time derivative Jacobian vs FD modified particle",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ReleaseCI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("RADIAL_GENERAL_RATE_MODEL", "FV", false, true,
																		true, 1e-6, 1e-14, 9e-4);
}

TEST_CASE("Radial GRM dynamic reactions time derivative Jacobian vs FD bulk and particle",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ReleaseCI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("RADIAL_GENERAL_RATE_MODEL", "FV", true, true,
																		false, 1e-6, 1e-14, 9e-4);
}

TEST_CASE("Radial GRM dynamic reactions time derivative Jacobian vs FD bulk and modified particle",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ReleaseCI],[FD]")
{
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD("RADIAL_GENERAL_RATE_MODEL", "FV", true, true,
																		true, 1e-6, 1e-14, 9e-4);
...
TEST_CASE("Radial GRM multi particle types dynamic reactions Jacobian vs AD bulk",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ParticleType],[ReleaseCI]")
{
...
TEST_CASE("Radial GRM multi particle types dynamic reactions Jacobian vs AD particle",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ParticleType],[ReleaseCI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("Radial GRM multi particle types dynamic reactions Jacobian vs AD modified particle",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ParticleType],[ReleaseCI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("Radial GRM multi particle types dynamic reactions Jacobian vs AD bulk and particle",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ParticleType],[ReleaseCI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("Radial GRM multi particle types dynamic reactions Jacobian vs AD bulk and modified particle",
		  "[RadGRM],[Jacobian],[AD],[ReactionModel],[ParticleType],[ReleaseCI]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("Radial GRM multi particle types dynamic reactions time derivative Jacobian vs FD bulk",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ParticleType],[ReleaseCI],[FD]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
	cadet::test::reaction::testTimeDerivativeJacobianDynamicReactionsFD(jpp, true, false, false, 1e-6, 1e-14, 9e-4);
}

TEST_CASE("Radial GRM multi particle types dynamic reactions time derivative Jacobian vs FD particle",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ParticleType],[ReleaseCI],[FD]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("Radial GRM multi particle types dynamic reactions time derivative Jacobian vs FD modified particle",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ParticleType],[ReleaseCI],[FD]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("Radial GRM multi particle types dynamic reactions time derivative Jacobian vs FD bulk and particle",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ParticleType],[ReleaseCI],[FD]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
...
TEST_CASE("Radial GRM multi particle types dynamic reactions time derivative Jacobian vs FD bulk and modified particle",
		  "[RadGRM],[Jacobian],[Residual],[ReactionModel],[ParticleType],[ReleaseCI],[FD]")
{
	cadet::JsonParameterProvider jpp = createColumnWithTwoCompLinearBindingThreeParticleTypes();
