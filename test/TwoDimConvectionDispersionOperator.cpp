...
void testBulk2DJacobianWenoForwardBackward(int wenoOrder)
{
	...
	SECTION("Forward vs backward flow Jacobian (WENO=" + std::to_string(wenoOrder) + ")")
	{
		...
		convDispOp.residual(DummyModel(), 0.0, 0u, y.data(), nullptr, jacCol1.data(), true,
		...
		convDispOp.residual(DummyModel(), 0.0, 1u, y.data(), nullptr, jacCol1.data(), true,
							cadet::WithoutParamSensitivity());
...
void testBulk2DJacobianSparsityWeno(int wenoOrder, bool forwardFlow)
{
	...
	SECTION("WENO=" + std::to_string(wenoOrder))
	{
		...
		for (int col = 0; col < nPureDof; ++col)
		{
			...
			// Central finite differences
			y[nInletDof + col] = ref * (1.0 + h);
			convDispOp.residual(DummyModel(), 0.0, 0u, y.data(), nullptr, jacCol1.data(), false,
								cadet::WithoutParamSensitivity());

			y[nInletDof + col] = ref * (1.0 - h);
			convDispOp.residual(DummyModel(), 0.0, 0u, y.data(), nullptr, jacCol2.data(), false,
								cadet::WithoutParamSensitivity());
