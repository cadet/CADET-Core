...
struct FVparams : public DiscParams
{
	...
	FVparams(int nCol) : nAxCells(nCol), nParCells(0), wenoOrder(0), nRadCells(0)
	{
	}
	FVparams(int nCol, int nPar) : nAxCells(nCol), nParCells(nPar), wenoOrder(0), nRadCells(0)
	{
	}
	FVparams(int nCol, int nPar, int wenoOrder) : nAxCells(nCol), nParCells(nPar), wenoOrder(wenoOrder), nRadCells(0)
	{
	}
	...
	int getNAxCells() const override
	{
		return nAxCells;
	}
	int getNParCells() const override
	{
		return nParCells;
	}
...
struct DGparams : public DiscParams
{
	...
	{
		exactIntegration = integrationMode;
	}
 ...
 */
void testJacobianADVariableParSurfDiff(const std::string& uoType, const std::string& spatialMethod,
									   bool dynamicBinding);
...
void testJacobianWenoForwardBackwardFD(const std::string& uoType, const std::string& spatialMethod, int wenoOrder,
									   double h = 1e-6, double absTol = 0.0,
 ...
 * @param [in] relTol Relative error tolerance
 */
void testTimeDerivativeJacobianFD(const std::string& uoType, const std::string& spatialMethod, double h = 1e-6,
 ...
 * @param [in] relTol Relative error tolerance
 */
void testFwdSensJacobians(cadet::JsonParameterProvider jpp, double h = 1e-6, double absTol = 0.0,
						  double relTol = std::numeric_limits<float>::epsilon() * 100.0, const bool hasBinding = true);
...
void testConsistentInitializationLinearBinding(const std::string& uoType, const std::string& spatialMethod,
											   double consTol, double absTol, const int reqBnd = -1,
											   const int useAD = -1);
...
void testConsistentInitializationSMABinding(const std::string& uoType, const std::string& spatialMethod,
											...
											const int reqBnd = -1, const int useAD = -1);
...
void testConsistentInitializationSensitivity(const std::string& uoType, const std::string& spatialMethod,
											 ...
											 double absTol, const int reqBnd = -1, const int useAD = -1);
...
void testReferenceBenchmark(const std::string& modelFileRelPath, const std::string& refFileRelPath,
							...
							const std::vector<double> relTol, const cadet::test::column::DiscParams& disc,
							const bool compare_sens = false, const int simDataStride = 1);

/**
 * @brief Runs an EOC test comparing against numerical reference data (outlet data)
 * @param [in] setupFileRelPath Path to the setup data file from the directory of this file. Model configuration is
 * sufficient, rest will be copied from reference file.
 * @param [in] refFileRelPath Path to the reference data file from the directory of this file
...
void testEOCReferenceBenchmark(const std::string& modelFileRelPath, const std::string& refFileRelPath,
							   ...
							   const unsigned int nDisc, const cadet::test::column::DiscParams& disc,
							   const bool compare_sens = false);

/**
 * @brief Runs a simulation test comparing against numerical reference data (outlet data), generated by a foreign source
 * @detail Reference data from the foreign source needs to be stored in an h5 file with hierarchy
 * output->solution->SOLUTION_OUTLET
 * @param [in] setupFileRelPath Path to the setup data file from the directory of this file. Full CADET configuration is
 * required
 * @param [in] refFileRelPath Path to the reference data file from the directory of this file
 * @param [in] unitID ID of the unit of interest
 * @param [in] absTol Absolute error tolerance
