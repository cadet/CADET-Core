...
//------------------------------------------------------------------------------
//
// saves a gnuplot session to an encapsulated postscript file
//
Gnuplot& Gnuplot::savetoeps(const std::string &filename)
{
    cmd("set terminal postscript eps enhanced 14 color solid lw 3 size 6,4");
                             ...
                             const std::string &title)
{
    ...
    //
    int iIndex = 0;
    for(int iRow = 0; iRow < static_cast<int>(iHeight); iRow++)
    {
        for(int iColumn = 0; iColumn < static_cast<int>(iWidth); iColumn++)
        {
            tmp << iColumn << " " << iRow  << " "
