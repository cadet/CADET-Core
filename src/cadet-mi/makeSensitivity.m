function sens = makeSensitivity(unitOpIdx, name, comp, reaction, boundPhase, section, absTol, factors)
%MAKESENSITIVITY Creates a struct that represents a parameter sensitivity
%   SENS = MAKESENSITIVITY(UNITOPIDX, NAME, COMP, REACTION, BOUNDPHASE, SECTION) creates a parameter
...
%   vector with 0-based component indices, REACTION is a vector with 0-based reaction indices,
%   BOUNDPHASE is a vector with 0-based bound phase indices, and SECTION is a vector with
%   0-based section indices. If a single parameter does not depend on component, reaction,
%   bound phase, or section, the respective index is set to -1. All single parameter sensitivities
%   are summed up to form the joined sensitivity.
%   Returns the joined parameter as a struct of arrays with the fields SENS_NAME, SENS_UNIT,
%   SENS_COMP, SENS_REACTION, SENS_SECTION, SENS_BOUNDPHASE, SENS_FACTOR. The field SENS_ABSTOL
%   is a scalar in the struct.
%
