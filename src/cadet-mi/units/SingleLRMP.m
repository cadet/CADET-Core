...
classdef SingleLRMP < LumpedRateModelWithPores & SingleUnitOpSystem
	...
	%
	% See also LUMPEDRATEMODELWITHPORES, SINGLEUNITOPSYSTEM, PIECEWISECUBICPOLYPROFILE
	...
	methods
		...
		function obj = SingleLRMP()
			%SINGLELRMP Constructs a SingleLRMP object

			obj = obj@LumpedRateModelWithPores();
		...
		function S = saveobj(obj)
			S = obj.saveobj@LumpedRateModelWithPores();
		...
		function res = validate(obj, sectionTimes, subModels)
			...
			%   SECTIONTIMES to validate the LRMP in this system, the inlet model, and the
			...
			if subModels
				res = res && obj.inlet.validate(sectionTimes, obj.nComponents) && ...
				   obj.validate@LumpedRateModelWithPores(sectionTimes);
			end
		end
		...
		function res = assembleConfig(obj, unitOp)
			...
			if isempty(unitOp)
				...
				res = obj.assembleConfigBase(obj.assembleConfig@LumpedRateModelWithPores(), inletConfig);
			else
				switch unitOp
					case -1
						res = obj.assembleConfigBase([], []);
					case 0
						res = obj.assembleConfig@LumpedRateModelWithPores();
					case 1
		...
		function res = assembleReturnConfig(obj)
			...
			res = obj.assembleReturnConfigBase(obj.assembleReturnConfig@LumpedRateModelWithPores());
		end
		...
		function res = assembleInitialConditions(obj)
			...
			res = obj.assembleInitialConditionsBase(obj.assembleInitialConditions@LumpedRateModelWithPores());
		end
		...
		function val = getParameterValue(obj, param)
			...
			%
			% See also MODEL.GETPARAMETERVALUE, MEXSIMULATOR.GETPARAMETERVALUE, SINGLELRMP.SETPARAMETERVALUE,
			%   MAKESENSITIVITY
			...
			if param.SENS_UNIT == 0
				val = obj.getParameterValue@LumpedRateModelWithPores(param);
		...
		function oldVal = setParameterValue(obj, param, newVal)
			...
			%
			% See also MODEL.SETPARAMETERVALUE, MEXSIMULATOR.SETPARAMETERVALUE, SINGLELRMP.GETPARAMETERVALUE,
			%   MAKESENSITIVITY
			...
			if param.SENS_UNIT == 0
				oldVal = obj.setParameterValue@LumpedRateModelWithPores(param, newVal);
			elseif param.SENS_UNIT == 1
		...
		function notifySync(obj, systemOnly)
			...
			if ~systemOnly
				obj.notifySync@LumpedRateModelWithPores();
	...
	methods (Access = 'protected')
		...
		function loadobjInternal(obj, S)
			obj.loadobjInternal@LumpedRateModelWithPores(S);
