...
classdef SingleCSTR < StirredTankModel & SingleUnitOpSystem
	%SingleCSTR Represents a system with one stirred tank model
	%   This class behaves like one single stirred tank (CSTR) fed by a
	%   piecewise cubic polynomial inlet profile.
	%
	%   The stirred tank model is assigned the id 0 and the implicit inlet model
	%   is assigned id 1.
	%
	% See also STIRREDTANKMODEL, SINGLEUNITOPSYSTEM, PIECEWISECUBICPOLYPROFILE
	...
	properties (Dependent, Transient)
		% Constant coefficient for each component and section in [mol / m^3];
		% each row corresponds to a section, each column to a component such that the
		% format is nSections x nComponents.
		constant;
		% Linear coefficient for each component and section in [mol / (m^3 * s)];
		% each row corresponds to a section, each column to a component such that the
		% format is nSections x nComponents.
		linear;
		% Quadratic coefficient for each component and section in [mol / (m^3 * s^2)];
		% each row corresponds to a section, each column to a component such that the
		% format is nSections x nComponents.
		quadratic;
		% Cubic coefficient for each component and section in [mol / (m^3 * s^3)];
		% each row corresponds to a section, each column to a component such that the
		% format is nSections x nComponents.
		cubic;
	end
	...
	methods
		...
		function obj = SingleCSTR()
			%SINGLECSTR Constructs a SingleCSTR object

			obj = obj@StirredTankModel();
		...
		function S = saveobj(obj)
			S = obj.saveobj@StirredTankModel();
		...
		function res = validate(obj, sectionTimes, subModels)
			...
			%   SECTIONTIMES to validate the CSTR in this system, the inlet model, and the
			...
			if subModels
				res = res && obj.inlet.validate(sectionTimes, obj.nComponents) && ...
				   obj.validate@StirredTankModel(sectionTimes);
			end
		end
		...
		function res = assembleConfig(obj, unitOp)
			...
			%   system (including CSTR and inlet) as detailed in the CADET file format spec.
			%
			...
			%   stirred tank model (UNITOP = 'cstr' or 0), or just the inlet (UNITOP = 'inlet'
			...
			if isempty(unitOp)
				...
				res = obj.assembleConfigBase(obj.assembleConfig@StirredTankModel(), inletConfig);
			else
				switch unitOp
					case -1
						res = obj.assembleConfigBase([], []);
					case 0
						res = obj.assembleConfig@StirredTankModel();
					case 1
		...
		function res = assembleReturnConfig(obj)
			...
			%   configuration (return group in the file format spec) of the full system including the stirred
			%   tank model and the inlet.
			%
			% See also MODEL.ASSEMBLERETURNCONFIG, MODELSYSTEM.ASSEMBLERETURNCONFIG, MEXSIMULATOR.ASSEMBLERETURNCONFIG

			res = obj.assembleReturnConfigBase(obj.assembleReturnConfig@StirredTankModel());
		end
		...
		function res = assembleInitialConditions(obj)
			...
			%   initial conditions part of the system, the stirred tank model, and the inlet as detailed in
			...
			res = obj.assembleInitialConditionsBase(obj.assembleInitialConditions@StirredTankModel());
		end
		...
		function val = getParameterValue(obj, param)
			...
			%
			% See also MODEL.GETPARAMETERVALUE, MEXSIMULATOR.GETPARAMETERVALUE, SINGLECSTR.SETPARAMETERVALUE,
			%   MAKESENSITIVITY
			...
			if param.SENS_UNIT == 0
				val = obj.getParameterValue@StirredTankModel(param);
		...
		function oldVal = setParameterValue(obj, param, newVal)
			...
			%
			% See also MODEL.SETPARAMETERVALUE, MEXSIMULATOR.SETPARAMETERVALUE, SINGLECSTR.GETPARAMETERVALUE,
			%   MAKESENSITIVITY
			...
			if param.SENS_UNIT == 0
				oldVal = obj.setParameterValue@StirredTankModel(param, newVal);
			elseif param.SENS_UNIT == 1
		...
		function notifySync(obj, systemOnly)
			...
			if ~systemOnly
				obj.notifySync@StirredTankModel();
	...
	methods (Access = 'protected')
		...
		function loadobjInternal(obj, S)
			obj.loadobjInternal@StirredTankModel(S);
