...
classdef SingleGRM < GeneralRateModel & SingleUnitOpSystem
	%SingleGRM Represents a system with one general rate model of liquid column chromatography
	%   This class behaves like one single column (general rate model) fed by a
	%   piecewise cubic polynomial inlet profile.
	%
	%   The general rate model is assigned the id 0 and the implicit inlet model
	%   is assigned id 1.
	%
	...
	methods
		...
		function obj = SingleGRM()
			%SINGLEGRM Constructs a SingleGRM object

			obj = obj@GeneralRateModel();
		...
		function S = saveobj(obj)
			S = obj.saveobj@GeneralRateModel();
		...
		function res = validate(obj, sectionTimes, subModels)
			...
			%   SECTIONTIMES to validate the GRM in this system, the inlet model, and the
			...
			if subModels
				res = res && obj.inlet.validate(sectionTimes, obj.nComponents) && ...
				   obj.validate@GeneralRateModel(sectionTimes);
			end
		end
		...
		function res = assembleConfig(obj, unitOp)
			...
			%   system (including GRM and inlet) as detailed in the CADET file format spec.
			%
			...
			%   general rate model (UNITOP = 'grm' or 0), or just the inlet (UNITOP = 'inlet'
			...
			if isempty(unitOp)
				...
				res = obj.assembleConfigBase(obj.assembleConfig@GeneralRateModel(), inletConfig);
			else
				switch unitOp
					case -1
						res = obj.assembleConfigBase([], []);
					case 0
						res = obj.assembleConfig@GeneralRateModel();
					case 1
		...
		function res = assembleReturnConfig(obj)
			...
			res = obj.assembleReturnConfigBase(obj.assembleReturnConfig@GeneralRateModel());
		end
		...
		function res = assembleInitialConditions(obj)
			...
			res = obj.assembleInitialConditionsBase(obj.assembleInitialConditions@GeneralRateModel());
		end
		...
		function notifySync(obj, systemOnly)
			...
			if ~systemOnly
				obj.notifySync@GeneralRateModel();
	...
	methods (Access = 'protected')
		...
		function loadobjInternal(obj, S)
			obj.loadobjInternal@GeneralRateModel(S);
