function [params, residual, success] = runFitting(initParams)
%RUNFITTING Performs a parameter estimation for a three component SMA system
%
...
%   parameters. To simplify post-processing, the parameters are sorted by
%   characteristic charge in ascending order after the optimization has
%   finished.
%
...
%
%   RUNFITTING(INITPARAMS) runs the parameter fit from the initial parameters
%   given in INITPARAMS. The order of the parameters is
...
%
%   [PARAMS, RESIDUAL, SUCCESS] = RUNFITTING(...) returns a vector PARAMS
%   with the parameters at the end of the optimization, the residual RESIDUAL
%   at this point, and the logical SUCCESS indicating whether the optimizer
%   claims a solution. Returns -1 in RESIDUAL if something went wrong.
...
%            See the license note at the end of the file.
	...
	% Sort components in ascending characteristic charges
	idxBefore = [2 5 8];
	[~, idxSorted] = sort(params(idxBefore));
	idxAfter = idxBefore(idxSorted);
	% Permute in-place (KA, NU, and SIGMA)
	params(idxBefore-1) = params(idxAfter-1);
	params(idxBefore) = params(idxAfter);
	params(idxBefore+1) = params(idxAfter+1);
