function [gradientShape, overlaps, optResult, constr] = bilinearGradientProcessOverlapMax(initGradient, colLength, absMode, elutionConstraint, algorithm)
%BILINEARGRADIENTPROCESSOVERLAPMAX Optimizes the separation of a three component SMA process by minimizing the maximum peak overlap
%
%   The optimizer can vary the gradient shape of the elution phase (bilinear
%   gradient). The maximum of the three overlaps is minimized.
%  
...
%           \int_0^{t_end} R(c_1(t) * c_2(t)) dt <= m   Overlap of comp 1, comp 2
%           \int_0^{t_end} R(c_1(t) * c_3(t)) dt <= m   Overlap of comp 1, comp 3
%           \int_0^{t_end} R(c_2(t) * c_3(t)) dt <= m   Overlap of comp 2, comp 3
...
%
%   BILINEARGRADIENTPROCESSOVERLAPMAX(INITGRADIENT) minimizes the maximum peak
...
%
%   BILINEARGRADIENTPROCESSOVERLAPMAX(..., COLLENGTH) sets the column length to
%   COLLENGTH. Defaults to 0.014m = 1.4cm.
%
%   BILINEARGRADIENTPROCESSOVERLAPMAX(..., COLLENGTH, ABSMODE) additionally sets
%   the absolute function R(x) to one of the three functions described above.
%
%   BILINEARGRADIENTPROCESSOVERLAPMAX(..., COLLENGTH, ABSMODE, ELUTIONCONSTRAINT) 
...
%
%   BILINEARGRADIENTPROCESSOVERLAPMAX(..., COLLENGTH, ABSMODE, ELUTIONCONSTRAINT, ALGORITHM) 
%   additionally sets the algorithm used by FMINCON (e.g., 'interior-point', 'sqp').
%
%   [GRADIENTSHAPE, OVERLAPS, OPTRESULT, CONSTR] = BILINEARGRADIENTPROCESSOVERLAPMAX(...)
...
%            See the license note at the end of the file.
	...
	% Layout: start, slope1, len, max (m in formulation above)
	lb = [bp.initialSalt, 0.001, 120, -inf];
	ub = [500, 10, bp.endTime - bp.startTime - 120, inf];

	% Add the maximum m if not given
	if length(initGradient) == 3
		initGradient(4) = 1;
	end
	...
	%   y = A*x   <=>   y^T = x^T * A^T
	lb(1:end-1) = lb(1:end-1) * transform';
	ub(1:end-1) = ub(1:end-1) * transform';
	initGradient(1:end-1) = initGradient(1:end-1) * transform';
	...
	try
		...
		gradientShape = optSol(1:end-1) * invTransform';
	catch E
	...
	function [f, grad] = overlap(x)
		% Returns the m value (see problem formulation above)
	...
	function [c,ceq,gradc,gradceq] = constraints(x)
		...
		if nargout > 2
			...
			if elutionConstraint == 1
				gradc = [squeeze(jac(end, 2:end, :)).'; zeros(1, size(jac, 3))];
			elseif elutionConstraint == 2
				gradc = [-cell2mat(arrayfun(@(idx) squeeze(jac(:, idx, :)).' * numIntRule, 2:size(jac, 2), 'UniformOutput', false)); zeros(1, size(jac, 3))];
			end
			...
			gradSalt = [1; x(3); x(2); 0];
			
			% Assemble all gradients into Jacobian
			gradc = [gradc, [grad1, grad2, grad3 ; -ones(1, 3)], gradSalt];
			...
			gradc(1:end-1,:) = invTransform' * gradc(1:end-1,:);
		end
	end
	...
	% is computed using finite differences (FD).
		...
		rightRes = sim.runWithParameters(newLen + bp.startTime);

		% Simulate with slightly decreased length
		newLen = len * (1 - fdFactor);
		leftRes = sim.runWithParameters(newLen + bp.startTime);
	...
	function stop = progressMonitor(x, optimValues, state)
		...
		if isempty(hdPlot)			
			...
			set(hdTemp, 'Location','NorthWest');
			grid on;
			hdPlot.titleRight = title(sprintf('Start %g Slope1 %g Length %g MaxOverlap %g', [x(:); mOverlap]));
			...
			subplot(1, 2, 2);
			hdPlot.param = semilogy(optimValues.iteration, [x(:); mOverlap], 'x-');
			...
			set(hdPlot.titleRight, 'String', sprintf('Start %g Slope1 %g Length %g MaxOverlap %g', [x(:); mOverlap]));
			...
			set(hdPlot.param(end), 'YData', [get(hdPlot.param(end), 'YData'), mOverlap], 'XData', [get(hdPlot.param(end), 'XData'), optimValues.iteration]);
		end
		drawnow;
	end
