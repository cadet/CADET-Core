function [solution] = breakthroughParticleCoreRadius()
%BREAKTHROUGHPARTICLECORERADIUS Breakthrough curve with 1 component Langmuir model and varying particle core radius
%
...
%
%   A breakthrough curve is computed for varying particle core radius. The results
%   are plotted to show the effect of increasing particle core radius.
%
...
%            See the license note at the end of the file.
	...
	% Set particleCoreRadius to 10%, 50%, 75% of particleRadius
	for factor = [0.1, 0.5, 0.75]
		% Change parameter
		mGrm.particleCoreRadius = factor * mGrm.particleRadius;

		% Run simulation again with changed parameter
		result = sim.run();
	...
	% Plot the solution
	plot(solution(:, 1), solution(:, 2:end));
	legend('Core radius 0%', 'Core radius 10%', 'Core radius 50%', 'Core radius 75%', 'Location', 'southeast');
	grid on;
	xlabel('Time [s]');
