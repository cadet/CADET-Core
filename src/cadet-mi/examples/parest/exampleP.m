function exampleP()
%EXAMPLEP Two fits with artificial data. Both the peaks and experiments are
% weighted.
%
...
%            See the license note at the end of the file.
    ...
    fit.weight = 0.8; % Place 80% weight on the first experiment
    ...
    fit.weightComponent = fit.weightComponent ./ sum(fit.weightComponent);
    ...
    fit.outMeas = generateArtificialData(fit);
    fit.outMeas = fit.outMeas(:, fit.idxComp); % Take components, ignore salt
    fit.weight = 0.2; % Place 20% weight on the second experiment
...
function [sim] = createModel(tOut, n)
    model = ModelGRM();
