function singleExpMultipleParamTransform()
%SINGLEEXPMULTIPLEPARAMTRANSFORM Fits adsorption rates in a single experiment with multiple parameter transformations
%
...
%      K_EQ = KA / KD.
%   The parameters are then transformed to a logarithmic scale which helps the
%   optimizer to cope with the varying orders of magnitude.
%
%   See also LOADWASHELUTIONSMASINGLE, SINGLEEXPSEPARATECOMPONENTS, SINGLEEXPSINGLEPARAMTRANSFORM,
%      SINGLEEXPCUSTOMPARAMTRANSFORM
...
%            See the license note at the end of the file.
	...
	params{2} = makeSensitivity([0], {'SMA_KD'}, [1], [-1], [0], [-1]);
	params{3} = makeSensitivity([0], {'SMA_KA'}, [2], [-1], [0], [-1]);
	params{4} = makeSensitivity([0], {'SMA_KD'}, [2], [-1], [0], [-1]);
	...
	% Create logarithmic transformation that applies to all parameters
	logTransform = LogParameterTransformation(initParams, true(4, 1));	

	% First, apply (KA, KD) -> KEQ, then log scaling
	pf.parameterTransform = [keqTransform, logTransform];
	...
	loBound = logTransform.transform([loBound(1) / upBound(2), loBound(2), loBound(3) / upBound(4), loBound(4)]);
	upBound = logTransform.transform([upBound(1) / loBound(2), upBound(2), upBound(3) / loBound(4), upBound(4)]);
