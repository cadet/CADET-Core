function resumeResetTime()
%RESUMERESETTIME Shows how to resume a previous simulation with time resetted to 0
%
...
%
%   In this example, the time in the second half is reset to start from 0.0
%   again. Another option is to just let the time continue. This approach
%   is presented in RESUMECONTINUOUSTIME.
%
...
%            See the license note at the end of the file.
	...
	%   - Since time is resetted, we will simulate from 0s to 750s again
	...
	% Shift the time by 750s for a continuous output
	sol2 = [res2.solution.time + 750, squeeze(res2.solution.outlet{1})];
	sens2 = [res2.solution.time + 750, squeeze(res2.sensitivity.jacobian{1})];
...
%   The inlet profile is shifted in time such that the excerpt begins at 0s.
	...
	% Update simulator
	sim.sectionTimes = max(min(nomSecTimes(secStart:secEnd), tEnd) - tStart, 0);
