...
classdef LangmuirBinding < KineticQuasiStationaryBindingModel
	%LangmuirBinding Langmuir binding model
	...
	methods
		...
		function obj = LangmuirBinding(kA, kD, qMax)
			%LANGMUIRBINDING Constructs a LangmuirBinding object
			%   OBJ = LANGMUIRBINDING(KA) creates a LangmuirBinding model with
			%   the given adsorption rates KA.
			%
			%   OBJ = LANGMUIRBINDING(..., KD) also sets the desorption rates to KD.
			%
			%   OBJ = LANGMUIRBINDING(..., KD, QMAX) also sets the desorption rates
		...
		function val = get.qMax(obj)
			val = obj.data.MCL_QMAX;
		end

		function set.qMax(obj, val)
			validateattributes(val, {'double'}, {'vector', 'nonempty', '>', 0.0, 'finite', 'real'}, '', 'qMax');
			obj.data.MCL_QMAX = val;
			obj.hasChanged = true;
		end
		
		function val = get.MCL_KA(obj)
			val = obj.kA;
		end
		function set.MCL_KA(obj, val)
			obj.kA = val;
		end
		function val = get.MCL_KD(obj)
			val = obj.kD;
		end
		function set.MCL_KD(obj, val)
			obj.kD = val;
		end
		function val = get.MCL_QMAX(obj)
			val = obj.qMax;
		end
		function set.MCL_QMAX(obj, val)
			obj.qMax = val;
		end
