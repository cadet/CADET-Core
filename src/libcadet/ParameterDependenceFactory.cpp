...
template <class ParamDep_t> void ParameterDependenceFactory::registerStateDependence(const std::string& name)
{
	_paramStateDeps[name] = []() { return new ParamDep_t(); };
}

template <class ParamDep_t> void ParameterDependenceFactory::registerStateDependence()
{
	registerStateDependence<ParamDep_t>(ParamDep_t::identifier());
}

template <class ParamDep_t> void ParameterDependenceFactory::registerParameterDependence(const std::string& name)
{
	_paramParamDeps[name] = []() { return new ParamDep_t(); };
}

template <class ParamDep_t> void ParameterDependenceFactory::registerParameterDependence()
{
...
model::IParameterParameterDependence* ParameterDependenceFactory::createParameterDependence(
...
void ParameterDependenceFactory::registerModel(const std::string& name,
											   std::function<model::IParameterStateDependence*()> factory)
{
	if (_paramStateDeps.find(name) == _paramStateDeps.end())
		_paramStateDeps[name] = factory;
	else
		throw InvalidParameterException("IParameterStateDependence implementation with the name " + name +
										" is already registered and cannot be overwritten");
}

void ParameterDependenceFactory::registerModel(const std::string& name,
											   std::function<model::IParameterParameterDependence*()> factory)
{
	if (_paramParamDeps.find(name) == _paramParamDeps.end())
		_paramParamDeps[name] = factory;
	else
		throw InvalidParameterException("IParameterParameterDependence implementation with the name " + name +
										" is already registered and cannot be overwritten");
}
