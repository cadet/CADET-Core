...
int residualDaeWrapper(double t, N_Vector y, N_Vector yDot, N_Vector res, void* userData)
{
	...
	if (sim->_modifiedNewton)
		return sim->_model->residual(cadet::SimulationTime{t, secIdx},
									 cadet::ConstSimulationState{NVEC_DATA(y), NVEC_DATA(yDot)}, NVEC_DATA(res));
						  ...
						  N_Vector tempv3)
{
	...
	return sim->_model->jacobian(
		cadet::SimulationTime{t, secIdx}, cadet::ConstSimulationState{NVEC_DATA(y), NVEC_DATA(yDot)}, NVEC_DATA(tempv1),
		cadet::AdJacobianParams{sim->_vecADres, sim->_vecADy, sim->numSensitivityAdDirections()});
}
						...
						N_Vector* resS, void* userData, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
	...
	*/
	...
	if (sim->_modifiedNewton)
	{
		return sim->_model->residualSensFwdWithJacobian(
			ns, cadet::SimulationTime{t, secIdx}, cadet::ConstSimulationState{NVEC_DATA(y), NVEC_DATA(yDot)},
			NVEC_DATA(res), sensY, sensYdot, sensRes,
			cadet::AdJacobianParams{sim->_vecADres, sim->_vecADy, sim->numSensitivityAdDirections()}, NVEC_DATA(tmp1),
			NVEC_DATA(tmp2), NVEC_DATA(tmp3));
	}
...
Simulator::Simulator()
	: _model(nullptr), _solRecorder(nullptr), _idaMemBlock(nullptr), _vecStateY(nullptr), _vecStateYdot(nullptr),
	  ...
	  _maxNewtonIter(4), _maxErrorTestFail(10), _maxConvTestFail(10), _maxNewtonIterSens(4), _curSec(0),
