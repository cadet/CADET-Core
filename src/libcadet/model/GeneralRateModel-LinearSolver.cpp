...
int GeneralRateModel<ConvDispOperator>::linearSolve(double t, double alpha, double outerTol, double* const rhs,
...
// rhs is passed twice but due to the values in jacA the writes happen to a different area of the rhs than the reads.
	 ...
	 */
	template <typename ConvDispOperator>
	void GeneralRateModel<ConvDispOperator>::assembleDiscretizedJacobianParticleBlock(unsigned int parType,
																					  unsigned int pblk, double alpha,
																					  const Indexer& idxr)
	{
	 ...
	 */
	template <typename ConvDispOperator>
	void GeneralRateModel<ConvDispOperator>::addTimeDerivativeToJacobianParticleShell(
