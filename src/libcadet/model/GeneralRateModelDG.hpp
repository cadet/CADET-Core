...
\end{align} @f]
 * Methods are described in @cite Breuer2023 (DGSEM discretization), @cite Puttmann2013 @cite Puttmann2016 (forward
...
protected:
	...
	void addTimeDerivativeToJacobianParticleShell(linalg::BandedEigenSparseRowIterator& jac, const Indexer& idxr,
	...
	struct Discretization
	{
		...
		unsigned int*
			nParPointsBeforeType; //!< Array with total number of radial points before a particle type (cumulative sum
								  //!< of nParPoints), additional last element contains total number of particle shells
		...
		Eigen::MatrixXd*
			minus_InvMM_ST; //!< equals minus inverse mass matrix times transposed stiffness matrix. Required solely for
							//!< exact integration DG discretization of particle equation
		Eigen::VectorXd* parInvWeights; //!< Array with weights for LGL quadrature of size nNodes for each particle
		Eigen::MatrixXd*
			parInvMM; //!< dense inverse mass matrix for exact integration of integrals with metrics, for each particle
		Eigen::MatrixXd* parInvMM_Leg; //!< dense inverse mass matrix (Legendre) for exact integration of integral
									   //!< without metric, for each particle
		...
		Eigen::Vector<active, Dynamic>*
			Ir; //!< metric part for each particle type and cell, particle type major ordering
		Eigen::MatrixXd*
			Dr; //!< derivative matrices including metrics for each particle type and cell, particle type major ordering
		Eigen::VectorXi offsetMetric; //!< offset required to access metric dependent DG operator storage of Ir, Dr ->
									  //!< summed up nCells of all previous parTypes
		...
		/**
		 * @brief allocates memory for DG operators and computes those that are metric independent. Also allocates
		 * required containers needed for the DG discretization.
		 */
		void initializeDG()
		{
			...
			/* compute metric independent DG operators for bulk and particles. Note that metric dependent DG operators
			 * are computet in updateRadialDisc(). */
		...
		void initializeDGjac(std::vector<double> parGeomSurfToVol)
		{
			...
			for (unsigned int type = 0; type < nParType; type++)
			{
				...
				{
					if (parGSM[type])
						DGjacParDispBlocks[offsetMetric[type] + block] =
							GSMjacobianParDispBlock(type, parGeomSurfToVol[type]);
					else
						DGjacParDispBlocks[offsetMetric[type] + block] =
							DGjacobianParDispBlock(block + 1u, type, parGeomSurfToVol[type]);
				}
			}
		}
	...
	private:
		...
		MatrixXd getParGBlock(unsigned int cellIdx, unsigned int parType)
		{
			...
			if (parExactInt[parType])
			{
				...
				{ // cellIdx out of bounds
					return MatrixXd::Zero(nParNode[parType], nParNode[parType] + 2);
				}
				if (cellIdx != 1 && cellIdx != nParCell[parType])
				{ // inner cell
					gBlock.block(0, 0, nParNode[parType], 1) -=
						0.5 * parInvMM_Leg[parType].block(0, 0, nParNode[parType], 1);
					gBlock.block(0, 1, nParNode[parType], 1) +=
						0.5 * parInvMM_Leg[parType].block(0, 0, nParNode[parType], 1);
					gBlock.block(0, nParNode[parType], nParNode[parType], 1) -=
						0.5 * parInvMM_Leg[parType].block(0, nParNode[parType] - 1, nParNode[parType], 1);
					gBlock.block(0, nParNode[parType] + 1, nParNode[parType], 1) +=
						0.5 * parInvMM_Leg[parType].block(0, nParNode[parType] - 1, nParNode[parType], 1);
				}
				else if (cellIdx == 1u)
				{                                     // left boundary cell
				...
				else if (cellIdx == nParCell[parType])
				{ // right boundary cell
					gBlock.block(0, 0, nParNode[parType], 1) -=
						0.5 * parInvMM_Leg[parType].block(0, 0, nParNode[parType], 1);
					gBlock.block(0, 1, nParNode[parType], 1) +=
			...
			{
				// inexact integration not maintained due to inferior performance. Code is part of
				// calcParticleCollocationDGSEMJacobian()
			}
		 ...
		 */
		Eigen::MatrixXd parAuxBlockGstar(unsigned int cellIdx, unsigned int parType, MatrixXd leftG, MatrixXd middleG,
										 MatrixXd rightG)
		{

			// auxiliary block [ d g^* / d c ], depends on whole previous and subsequent cell plus first entries of
			// subsubsequent cells
			MatrixXd gStarDC = MatrixXd::Zero(nParNode[parType], 3 * nParNode[parType] + 2);
			// NOTE: N = polyDeg
			// indices  gStarDC    :     0   ,   1   , ..., nNodes; nNodes+1, ..., 2 * nNodes;	2*nNodes+1, ..., 3 *
			// nNodes; 3*nNodes+1 derivative index j  : -(N+1)-1, -(N+1),... ,  -1   ;   0     , ...,		N	 ;	  N
			// + 1	  , ..., 2N + 2    ; 2(N+1) +1 auxiliary block [d g^* / d c]
			if (cellIdx != 1)
			{
				gStarDC.block(0, nParNode[parType], 1, nParNode[parType] + 2) +=
					middleG.block(0, 0, 1, nParNode[parType] + 2);
				gStarDC.block(0, 0, 1, nParNode[parType] + 2) +=
					leftG.block(nParNode[parType] - 1, 0, 1, nParNode[parType] + 2);
			}
			if (cellIdx != nParCell[parType])
			{
				gStarDC.block(nParNode[parType] - 1, nParNode[parType], 1, nParNode[parType] + 2) +=
					middleG.block(nParNode[parType] - 1, 0, 1, nParNode[parType] + 2);
				gStarDC.block(nParNode[parType] - 1, 2 * nParNode[parType], 1, nParNode[parType] + 2) +=
					rightG.block(0, 0, 1, nParNode[parType] + 2);
			}
			gStarDC *= 0.5;
		...
		Eigen::MatrixXd getParBMatrix(int parType, int cell, double parGeomSurfToVol)
		{
			...
			else
			{
				...
				B(nParNode[parType] - 1, nParNode[parType] - 1) =
					static_cast<double>(Ir[offsetMetric[parType] + (cell - 1)][nParNode[parType] - 1]);
			}
		...
		Eigen::MatrixXd GSMjacobianParDispBlock(unsigned int parType, double parGeomSurfToVol)
		{
			...
			// We have to match the DGSEM interface, where the dispersion block [ d RHS_disp / d c ] depends on whole
			// previous and subsequent cell plus first entries of subsubsequent cells
			dispBlock = MatrixXd::Zero(nParNode[parType], 3 * nParNode[parType] + 2);

			dispBlock.block(0, nParNode[parType] + 1, nParNode[parType], nParNode[parType]) =
				minus_parInvMM_Ar[parType];
			dispBlock *= 2.0 / static_cast<double>(deltaR[offsetMetric[parType]]) * 2.0 /
						 static_cast<double>(deltaR[offsetMetric[parType]]);
		...
		Eigen::MatrixXd DGjacobianParDispBlock(unsigned int cellIdx, unsigned int parType, double parGeomSurfToVol)
		{
			...
			MatrixXd dispBlock;
			// Inner dispersion block [ d RHS_disp / d c ], depends on whole previous and subsequent cell plus first
			// entries of subsubsequent cells
			dispBlock = MatrixXd::Zero(nParNode[parType], 3 * nParNode[parType] + 2);
			...
			if (parExactInt[parType])
			{
				...
				MatrixXd gStarDC = parAuxBlockGstar(cellIdx, parType, getParGBlock(cellIdx - 1, parType), gBlock,
													getParGBlock(cellIdx + 1, parType)); // Numerical flux block

				if (parGeomSurfToVol != SurfVolRatioSlab) // weak form DGSEM required
					dispBlock.block(0, nParNode[parType], nParNode[parType], nParNode[parType] + 2) =
						minus_InvMM_ST[offsetMetric[parType] + (cellIdx - 1)] * gBlock;
				else // strong form DGSEM
					dispBlock.block(0, nParNode[parType], nParNode[parType], nParNode[parType] + 2) =
						(parPolyDerM[parType] - parInvMM[offsetMetric[parType] + (cellIdx - 1)] * B) * gBlock;
			...
			{
				// inexact integration is not maintained due to inferior performance. Code is in
				// calcParticleCollocationDGSEMJacobian
			}
	...
	parts::AxialConvectionDispersionOperatorBaseDG
		_convDispOp;                         //!< Convection dispersion operator base for interstitial volume transport
	...
	std::vector<IParameterStateDependence*>
		_parDepSurfDiffusion;        //!< Parameter dependencies for particle surface diffusion
	...
	public:
		...
		// Strides
		inline int strideColNode() const CADET_NOEXCEPT
		{
			return static_cast<int>(_disc.nComp);
		}
		inline int strideColCell() const CADET_NOEXCEPT
		{
			return static_cast<int>(_disc.nNodes * strideColNode());
		}
		...
		inline int strideParNode(int parType) const CADET_NOEXCEPT
		{
			return strideParLiquid() + strideParBound(parType);
		}
		inline int strideParShell(int parType) const CADET_NOEXCEPT
		{
			return strideParNode(parType) * _disc.nParNode[parType];
		}
		inline int strideParBlock(int parType) const CADET_NOEXCEPT
		{
			return static_cast<int>(_disc.nParPoints[parType]) * strideParNode(parType);
		}
		...
		{
			return _disc.nComp * _disc.nPoints + offsetC();
		}
		...
		template <typename real_t> inline real_t& c(real_t* const data, unsigned int point, unsigned int comp) const
		{
			return data[offsetC() + comp + point * strideColNode()];
		}
		template <typename real_t>
		inline const real_t& c(real_t const* const data, unsigned int point, unsigned int comp) const
		{
			return data[offsetC() + comp + point * strideColNode()];
		}
	...
	{
	public:
		Exporter(const Discretization& disc, const GeneralRateModelDG& model, double const* data)
		...
		/**
		 * @brief calculates, writes the physical axial/column coordinates of the DG discretization with double!
		 * interface nodes
		 */
		virtual int writePrimaryCoordinates(double* coords) const
		{
			for (unsigned int i = 0; i < _disc.nCol; i++)
			{
				...
				{
					// mapping
					coords[i * _disc.nNodes + j] =
						_model._convDispOp.cellLeftBound(i) +
						0.5 *
							(static_cast<double>(_model._convDispOp.columnLength()) / static_cast<double>(_disc.nCol)) *
		...
		/**
		 * @brief calculates, writes the physical radial/particle coordinates of the DG discretization with double!
		 * interface nodes
		 */
		virtual int writeParticleCoordinates(unsigned int parType, double* coords) const
		{
			...
			for (unsigned int par = 0; par < _disc.nParPoints[parType]; par++)
			{
				...
				double r_L = static_cast<double>(pcr[cell]) -
							 0.5 * static_cast<double>(_disc.deltaR[_disc.offsetMetric[parType] + cell]);
				coords[par] = r_L + 0.5 * static_cast<double>(_disc.deltaR[_disc.offsetMetric[parType] + cell]) *
										(1.0 + _disc.parNodes[parType][par % _disc.nParNode[parType]]);
			}
	...
	void applyParInvMap(Eigen::Map<Vector<ResidualType, Dynamic>, 0, InnerStride<>>& state, unsigned int parType)
	{
		...
		{
			state.segment(cell * _disc.nParNode[parType], _disc.nParNode[parType]) *=
				2.0 / static_cast<ParamType>(_disc.deltaR[_disc.offsetMetric[parType] + cell]) * 2.0 /
				static_cast<ParamType>(_disc.deltaR[_disc.offsetMetric[parType] + cell]);
		}
	}

	template <typename StateType, typename ResidualType>
	void parGSMVolumeIntegral(const int parType,
							  ...
							  Eigen::Map<Vector<ResidualType, Dynamic>, 0, InnerStride<Dynamic>>& stateDer)
	{
		...
		stateDer.segment(0, nNodes) -=
			(_disc.minus_parInvMM_Ar[parType].template cast<StateType>() * state.segment(0, nNodes))
				.template cast<ResidualType>();
	}

	template <typename StateType, typename ResidualType>
	void parVolumeIntegral(const int parType, const bool aux,
						   Eigen::Map<const Vector<StateType, Dynamic>, 0, InnerStride<Dynamic>>& state,
						   Eigen::Map<Vector<ResidualType, Dynamic>, 0, InnerStride<Dynamic>>& stateDer)
	{
		...
		if (aux || (_disc.parExactInt[parType] && _parGeomSurfToVol[parType] == _disc.SurfVolRatioSlab))
		{
			...
			for (unsigned int Cell = 0; Cell < _disc.nParCell[parType]; Cell++)
			{
				stateDer.segment(Cell * nNodes, nNodes) -=
					(_disc.parPolyDerM[parType].template cast<StateType>() * state.segment(Cell * nNodes, nNodes))
						.template cast<ResidualType>();
			}
		}
		else if (_disc.parExactInt[parType] && _parGeomSurfToVol[parType] != _disc.SurfVolRatioSlab)
		{
			...
			{
				stateDer.segment(Cell * nNodes, nNodes) -=
					(_disc.minus_InvMM_ST[_disc.offsetMetric[parType] + Cell].template cast<StateType>() *
					 state.segment(Cell * nNodes, nNodes))
						.template cast<ResidualType>();
			}
		}
		/* include metrics for main particle equation -> res = - D * (d_p * c^p + invBeta_p sum_mi d_s c^s) */
		else
		{ // inexact integration, main equation
			...
			if (_parGeomSurfToVol[parType] != _disc.SurfVolRatioSlab && _parCoreRadius[parType] == 0.0)
			{
				...
				stateDer.segment(1, nNodes - 1) -= (_disc.Dr[_disc.offsetMetric[parType]]
														.block(1, 1, nNodes - 1, nNodes - 1)
														.template cast<StateType>() *
													state.segment(1, nNodes - 1))
													   .template cast<ResidualType>();
			...
			{
				stateDer.segment(cell * nNodes, nNodes) -=
					(_disc.Dr[_disc.offsetMetric[parType] + cell].template cast<StateType>() *
					 state.segment(cell * nNodes, nNodes))
						.template cast<ResidualType>();
			}
		}
	...
	/*
	 * @brief calculates the interface fluxes g* of particle mass balance equation and implements the respective
	 * boundary conditions
	...
	void InterfaceFluxParticle(int parType,
							   Eigen::Map<const Vector<StateType, Dynamic>, 0, InnerStride<Dynamic>>& state,
							   const unsigned int strideCell, const unsigned int strideNode, const bool aux,
							   const int comp, const bool addParDisc = false)
	{

		Eigen::Map<Vector<StateType, Dynamic>, 0, InnerStride<>> _surfFluxPar(
			reinterpret_cast<StateType*>(&_disc.surfaceFluxParticle[parType][0]), _disc.nParCell[parType] + 1,
			InnerStride<>(1));
		...
		if (aux)
		{ // ghost nodes given by state^- := state^+ for auxiliary equation
		...
		else
		{
			...
			// film diffusion BC
			_surfFluxPar[_disc.nParCell[parType]] =
				static_cast<StateType>(_disc.localFlux[comp]) /
				(static_cast<double>(_parPorosity[parType]) *
				 static_cast<double>(_poreAccessFactor[parType * _disc.nComp + comp])) *
				(2.0 / static_cast<double>(
						   _disc.deltaR[_disc.offsetMetric[parType]])); // inverse squared mapping was also applied, so
																		// we apply Map * invMap^2 = invMap
	...
	void parSurfaceIntegral(int parType, Eigen::Map<const Vector<StateType, Dynamic>, 0, InnerStride<Dynamic>>& state,
							Eigen::Map<Vector<ResidualType, Dynamic>, 0, InnerStride<Dynamic>>& stateDer,
							unsigned const int strideCell, unsigned const int strideNode, const bool aux,
							const int comp = 0, const bool addParDisc = false)
	{
		...
		{                  // inexact integration approach -> diagonal mass matrix
			int Cell0 = 0; // auxiliary variable to distinguish special case
			...
			if (_parGeomSurfToVol[parType] != _disc.SurfVolRatioSlab && _parCoreRadius[parType] == 0.0)
			{
				...
				stateDer[_disc.parPolyDeg[parType] * strideNode] // last cell node
					+= _disc.parInvWeights[parType][_disc.parPolyDeg[parType]] *
					   (state[_disc.parPolyDeg[parType] * strideNode] - _surfFluxPar[1]);
			}
			...
			for (unsigned int Cell = Cell0; Cell < _disc.nParCell[parType]; Cell++)
			{
				...
				stateDer[Cell * strideCell + _disc.parPolyDeg[parType] * strideNode] // last cell node
					+= _disc.parInvWeights[parType][_disc.parPolyDeg[parType]] *
					   (state[Cell * strideCell + _disc.parPolyDeg[parType] * strideNode] - _surfFluxPar[Cell + 1u]);
			}
		}
		else
		{ // exact integration approach -> dense mass matrix
			for (unsigned int Cell = 0; Cell < _disc.nParCell[parType]; Cell++)
			{
				...
				{
					if (aux)
					{ // strong surface integral -> M^-1 B [state - state*] and B has two non-zero entries, -1 and 1
						stateDer[Cell * strideCell + Node * strideNode] -=
							_disc.parInvMM_Leg[parType](Node, 0) * (state[Cell * strideCell] - _surfFluxPar[Cell]) -
							_disc.parInvMM_Leg[parType](Node, _disc.parPolyDeg[parType]) *
								(state[Cell * strideCell + _disc.parPolyDeg[parType] * strideNode] -
								 _surfFluxPar[Cell + 1u]);
					}
					else
					{
						if (_parGeomSurfToVol[parType] == _disc.SurfVolRatioSlab)
						{ // strong surface integral -> M^-1 B [state - state*] and B has two non-zero entries, -1 and 1
							stateDer[Cell * strideCell + Node * strideNode] -= static_cast<ResidualType>(
								_disc.parInvMM[parType](Node, 0) * (state[Cell * strideCell] - _surfFluxPar[Cell]) -
								_disc.parInvMM[parType](Node, _disc.parPolyDeg[parType]) *
									(state[Cell * strideCell + _disc.parPolyDeg[parType] * strideNode] -
									 _surfFluxPar[Cell + 1u]));
						}
						else if (_parGeomSurfToVol[parType] == _disc.SurfVolRatioCylinder)
						{ // weak surface integral -> M^-1 B [- state*] and B has two non-zero entries, which depend on
						  // metrics
							stateDer[Cell * strideCell + Node * strideNode] -= static_cast<ResidualType>(
								_disc.Ir[_disc.offsetMetric[parType] + Cell][0] *
									_disc.parInvMM[_disc.offsetMetric[parType] + Cell](Node, 0) *
									(-_surfFluxPar[Cell]) +
								_disc.Ir[_disc.offsetMetric[parType] + Cell][_disc.nParNode[parType] - 1] *
									_disc.parInvMM[_disc.offsetMetric[parType] + Cell](Node,
																					   _disc.parPolyDeg[parType]) *
									_surfFluxPar[Cell + 1u]);
						}
	 ...
	 */
	template <typename StateType>
	void solve_auxiliary_DG(int parType, Eigen::Map<const Vector<StateType, Dynamic>, 0, InnerStride<>>& conc,
							unsigned int strideCell, unsigned int strideNode, int comp)
	{

		Eigen::Map<Vector<StateType, Dynamic>, 0, InnerStride<>> g_p(
			reinterpret_cast<StateType*>(&_disc.g_p[parType][0]), _disc.nParPoints[parType], InnerStride<>(1));
		Eigen::Map<Vector<StateType, Dynamic>, 0, InnerStride<>> _surfFluxPar(
			reinterpret_cast<StateType*>(&_disc.surfaceFluxParticle[parType][0]), _disc.nParCell[parType] + 1,
			InnerStride<>(1));
		...
		parSurfaceIntegral<StateType>(parType, conc, g_p, strideCell, strideNode, true,
									  comp); // surface integral in strong DG form: M^-1 B [c - c^*]
	...
	/**
	 * @brief calculates the particle dispersion jacobian Pattern of the exact/inexact integration DG scheme for the
	 * given particle type and bead
	 */
	void calcParticleJacobianPattern(std::vector<T>& tripletList, unsigned int parType, unsigned int colNode,
									 unsigned int secIdx)
	{

		// Ordering of particle surface diffusion:
		// bnd0comp0, bnd0comp1, bnd0comp2, bnd1comp0, bnd1comp1, bnd1comp2
		active const* const _parSurfDiff =
		...
		if (_disc.nParCell[parType] == 1)
		{
			...
			// fill the jacobian: add dispersion block for each unbound and bound component, adjusted for the respective
			// coefficients
				...
				for (unsigned int i = 0; i < nNodes; i++)
				{
					for (unsigned int j = 0; j < nNodes; j++)
					{
						...
						tripletList.push_back(
							T(offset + comp * sComp + i * sNode, offset + comp * sComp + j * sNode, 0.0));
						...
						if (_hasSurfaceDiffusion[parType])
						{
							...
							for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
							{
								...
								{
									// row: add current component offset and go node strides from there for each
									// dispersion block entry col: jump oover liquid states, add current bound state
									// offset and go node strides from there for each dispersion block entry
									tripletList.push_back(
										T(offset + comp * sComp + i * sNode,
										  offset + idxr.strideParLiquid() +
											  idxr.offsetBoundComp(ParticleTypeIndex{parType}, ComponentIndex{comp}) +
											  bnd + j * sNode,
										  0.0));

									/* add surface diffusion dispersion block to solid */
									if (!qsReaction[idxr.offsetBoundComp(ParticleTypeIndex{parType},
																		 ComponentIndex{comp}) +
													bnd])
									{
										// row: jump over liquid states, add current bound state offset and go node
										// strides from there for each dispersion block entry col: jump over liquid
										// states, add current bound state offset and go node strides from there for
										// each dispersion block entry
		...
		else
		{
			...
			if (!_disc.parExactInt[parType])
			{
				...
				// fill the jacobian: add dispersion block for each unbound and bound component, adjusted for the
				...
				for (unsigned int comp = 0; comp < _disc.nComp; comp++)
				{
					for (unsigned int i = 0; i < nNodes; i++)
					{
						for (unsigned int j = nNodes; j < 3 * nNodes; j++)
						{
							...
							if ((j >= nNodes - 1 && j <= 2 * nNodes) || (i == 0 && j <= 2 * nNodes) ||
								(i == nNodes - 1 && j >= nNodes - 1))
							{
								// handle liquid state
								// row: add component offset and go node strides from there for each dispersion block
								// entry col: add component offset and go node strides from there for each dispersion
								// block entry
								tripletList.push_back(T(offset + comp * sComp + i * sNode,
														offset + comp * sComp + (j - nNodes) * sNode, 0.0));
								...
								if (_hasSurfaceDiffusion[parType])
								{
									...
									for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
									{
										...
										{
											// row: add current component offset and go node strides from there for each
											// dispersion block entry col: jump over liquid states, add current bound
											// state offset and go node strides from there for each dispersion block
											// entry
											tripletList.push_back(T(offset + comp * sComp + i * sNode,
																	offset + idxr.strideParLiquid() +
																		idxr.offsetBoundComp(ParticleTypeIndex{parType},
																							 ComponentIndex{comp}) +
																		bnd + (j - nNodes) * sNode,
																	0.0));
											...
											if (!qsReaction[idxr.offsetBoundComp(ParticleTypeIndex{parType},
															...
															bnd])
											{
												// row: jump over liquid states, add current bound state offset and go
												// node strides from there for each dispersion block entry col: jump
												// over liquid states, add current bound state offset and go node
												// strides from there for each dispersion block entry
												tripletList.push_back(
													T(offset + idxr.strideParLiquid() +
														  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																			   ComponentIndex{comp}) +
														  bnd + i * sNode,
													  offset + idxr.strideParLiquid() +
				...
				for (unsigned int comp = 0; comp < _disc.nComp; comp++)
				{
					for (unsigned int i = 0; i < nNodes; i++)
					{
						for (unsigned int j = 0; j < 2 * nNodes; j++)
						{
							...
							{
								// handle liquid state
								// row: add component offset and jump over previous cells. Go node strides from there
								// for each dispersion block entry col: add component offset and jump over previous
								// cells. Go back one cell and go node strides from there for each dispersion block
								// entry
								tripletList.push_back(
									T(offset + comp * sComp + (_disc.nParCell[parType] - 1) * sCell + i * sNode,
								...
								if (_hasSurfaceDiffusion[parType])
								{
									...
									for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
									{
										if (_parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)] != 0.0)
										{
											// row: add component offset and jump over previous cells. Go node strides
											// from there for each dispersion block entry col: jump over liquid states,
											// add current bound state offset and jump over previous cells. Go back one
											// cell and go node strides from there for each dispersion block entry
											tripletList.push_back(
												T(offset + comp * sComp + (_disc.nParCell[parType] - 1) * sCell +
													  i * sNode,
												  offset + idxr.strideParLiquid() +
													  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																		   ComponentIndex{comp}) +
													  bnd + (_disc.nParCell[parType] - 2) * sCell + j * sNode,
												  0.0));
											...
											if (!qsReaction[idxr.offsetBoundComp(ParticleTypeIndex{parType},
															...
															bnd])
											{
												// row: jump over previous cells and liquid states, add current bound
												// state offset and go node strides from there for each dispersion block
												// entry col: jump over previous cells and liquid states, go back one
												// cell, add current bound state offset and go node strides from there
												// for each dispersion block entry
												tripletList.push_back(
													T(offset + (_disc.nParCell[parType] - 1) * sCell +
														  idxr.strideParLiquid() +
														  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																			   ComponentIndex{comp}) +
														  bnd + i * sNode,
													  offset + (_disc.nParCell[parType] - 2) * sCell +
														  idxr.strideParLiquid() +
														  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																			   ComponentIndex{comp}) +
														  bnd + j * sNode,
					...
					for (unsigned int comp = 0; comp < _disc.nComp; comp++)
					{
						for (unsigned int i = 0; i < nNodes; i++)
						{
							for (unsigned int j = 0; j < 3 * nNodes; j++)
							{
								...
								{
									// handle liquid state
									// row: add component offset and jump over previous cells. Go node strides from
									// there for each dispersion block entry col: add component offset and jump over
									// previous cells. Go back one cell and go node strides from there for each
									// dispersion block entry
									...
									if (_hasSurfaceDiffusion[parType])
									{
										...
										for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp];
											 bnd++)
										{
											if (_parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)] != 0.0)
											{
												// row: add component offset and jump over previous cells. Go node
												// strides from there for each dispersion block entry col: jump over
												// liquid states, add current bound state offset and jump over previous
												// cells. Go back one cell and go node strides from there for each
												// dispersion block entry
												tripletList.push_back(
													T(offset + comp * sComp + cell * sCell + i * sNode,
													  offset + idxr.strideParLiquid() +
														  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																			   ComponentIndex{comp}) +
														  bnd + (cell - 1) * sCell + j * sNode,
													  0.0));
												...
												if (!qsReaction[idxr.offsetBoundComp(ParticleTypeIndex{parType},
																...
																bnd])
												{
													// row: jump over previous cells and liquid states, add current
													// bound state offset and go node strides from there for each
													// dispersion block entry col: jump over previous cells and liquid
													// states, go back one cell, add current bound state offset and go
													// node strides from there for each dispersion block entry
													tripletList.push_back(
														T(offset + cell * sCell + idxr.strideParLiquid() +
															  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																				   ComponentIndex{comp}) +
															  bnd + i * sNode,
														  offset + (cell - 1) * sCell + idxr.strideParLiquid() +
															  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																				   ComponentIndex{comp}) +
															  bnd + j * sNode,
														  0.0));
			...
			{ // exact integration
				...
				if (_disc.nParCell[parType] < 3u)
					special = 1u; // limits the iterator for special case nCells = 3 (dependence on additional entry)
				// fill the jacobian: add dispersion block for each unbound and bound component, adjusted for the
				// respective coefficients
				for (unsigned int comp = 0; comp < _disc.nComp; comp++)
				{
					for (unsigned int i = 0; i < nNodes; i++)
					{
						for (unsigned int j = nNodes + 1; j < 3 * nNodes + 2 - special; j++)
						{
							...
							// adjust for j start
							tripletList.push_back(T(offset + comp * sComp + i * sNode,
													offset + comp * sComp + j * sNode - (nNodes + 1) * sNode, 0.0));
							...
							if (_hasSurfaceDiffusion[parType])
							{
								...
								for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
								{
									...
									{
										// row: add current component offset and go node strides from there for each
										// dispersion block entry col: jump over liquid states, add current bound state
										// offset and go node strides from there for each dispersion block entry. adjust
										// for j start
										tripletList.push_back(T(
											offset + comp * sComp + i * sNode,
											offset + idxr.strideParLiquid() +
												idxr.offsetBoundComp(ParticleTypeIndex{parType}, ComponentIndex{comp}) +
												bnd + j * sNode - (nNodes + 1) * sNode,
											0.0));
														...
														bnd])
										{
											...
											// strides from there for each dispersion block entry col: jump over liquid
											// states, add current bound state offset and go node strides from there for
											// each dispersion block entry. adjust for j start
											tripletList.push_back(T(offset + idxr.strideParLiquid() +
																		idxr.offsetBoundComp(ParticleTypeIndex{parType},
																							 ComponentIndex{comp}) +
																		bnd + i * sNode,
																	offset + idxr.strideParLiquid() +
				...
				for (unsigned int comp = 0; comp < _disc.nComp; comp++)
				{
					for (unsigned int i = 0; i < nNodes; i++)
					{
						...
						{
							// handle liquid state
							// row: add component offset and jump over previous cells. Go node strides from there for
							// each dispersion block entry col: add component offset and jump over previous cells. Go
							// back one cell (and node or adjust for start) and go node strides from there for each
							// dispersion block entry.
							tripletList.push_back(
								T(offset + comp * sComp + (_disc.nParCell[parType] - 1) * sCell + i * sNode,
								  offset + comp * sComp + (_disc.nParCell[parType] - 1) * sCell - sCell - sNode +
									  j * sNode,
								  0.0));
							...
							if (_hasSurfaceDiffusion[parType])
							{
								...
								for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
								{
									...
									{
										// row: add component offset and jump over previous cells. Go node strides from
										// there for each dispersion block entry col: jump over liquid states, add
										// current bound state offset and jump over previous cells. Go back one cell
										// (and node or adjust for start) and go node strides from there for each
										// dispersion block entry.
										tripletList.push_back(T(
											offset + comp * sComp + (_disc.nParCell[parType] - 1) * sCell + i * sNode,
											offset + idxr.strideParLiquid() +
												idxr.offsetBoundComp(ParticleTypeIndex{parType}, ComponentIndex{comp}) +
												bnd + (_disc.nParCell[parType] - 2) * sCell - sNode + j * sNode,
											0.0));
										...
										if (!qsReaction[idxr.offsetBoundComp(ParticleTypeIndex{parType},
														...
														bnd])
										{
											// row: jump over previous cells and over liquid states, add current bound
											// state offset. go node strides from there for each dispersion block entry
											// col: jump over previous cells and over liquid states, add current bound
											// state offset. go back one cell (and node or adjust for start) and go node
											// strides from there for each dispersion block entry.
											tripletList.push_back(T(offset + (_disc.nParCell[parType] - 1) * sCell +
																		idxr.strideParLiquid() +
																		idxr.offsetBoundComp(ParticleTypeIndex{parType},
																							 ComponentIndex{comp}) +
																		bnd + i * sNode,
				...
				if (_disc.nParCell[parType] == 3)
				{
					for (unsigned int comp = 0; comp < _disc.nComp; comp++)
					{
						for (unsigned int i = 0; i < nNodes; i++)
						{
							...
							{
								// handle liquid state
								// row: add component offset and jump over previous cell. Go node strides from there for
								// each dispersion block entry col: add component offset. Go node strides from there for
								// each dispersion block entry. adjust for j start
								tripletList.push_back(T(offset + comp * sComp + sCell + i * sNode,
														offset + comp * sComp + j * sNode - sNode, 0.0));
								...
								if (_hasSurfaceDiffusion[parType])
								{
									...
									for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
									{
										if (_parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)] != 0.0)
										{
											// row: add component offset and jump over previous cells. Go node strides
											// from there for each dispersion block entry col: jump over liquid states,
											// add current bound state offset and jump over previous cell. go back one
											// cell and go node strides from there for each dispersion block entry.
											// adjust for j start
											tripletList.push_back(T(offset + comp * sComp + sCell + i * sNode,
																	offset + idxr.strideParLiquid() +
											...
											if (!qsReaction[idxr.offsetBoundComp(ParticleTypeIndex{parType},
															...
															bnd])
											{
												// row: jump over previous cells and over liquid states, add current
												// bound state offset. go node strides from there for each dispersion
												// block entry col: jump over liquid states, add current bound state
												// offset and jump over previous cell. go node strides from there for
												// each dispersion block entry. adjust for j start
												tripletList.push_back(
													T(offset + sCell + idxr.strideParLiquid() +
														  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																			   ComponentIndex{comp}) +
														  bnd + i * sNode,
													  offset + idxr.strideParLiquid() +
									...
									for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
									{
										if (_parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)] != 0.0)
										{
															...
															bnd])
											{
												// row: jump over previous cells and over liquid states, add current
												// bound state offset. go node strides from there for each dispersion
												// block entry col: jump over previous cells and over liquid states, add
												// current bound state offset. go back one cell and go node strides from
												// there for each dispersion block entry. adjust for j start
												tripletList.push_back(
													T(offset + sCell + idxr.strideParLiquid() +
														  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																			   ComponentIndex{comp}) +
					...
					for (unsigned int comp = 0; comp < _disc.nComp; comp++)
					{
						for (unsigned int i = 0; i < nNodes; i++)
						{
							for (unsigned int j = 0; j < 3 * nNodes + 2 - 1; j++)
							{
								...
								// row: add component offset and jump over previous cells. Go node strides from there
								// for each dispersion block entry col: add component offset and jump over previous
								// cells. Go back one cell and node. Go node strides from there for each dispersion
								// block entry.
								tripletList.push_back(
									T(offset + comp * sComp + (_disc.nParCell[parType] - 2) * sCell + i * sNode,
									  offset + comp * sComp + (_disc.nParCell[parType] - 2) * sCell - sCell - sNode +
										  j * sNode,
									  0.0));
								...
								if (_hasSurfaceDiffusion[parType])
								{
									...
									for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
									{
										if (_parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)] != 0.0)
										{
											...
											// from there for each dispersion block entry col: jump over liquid states,
											// add current bound state offset and jump over previous cells. Go back one
											// cell and node and go node strides from there for each dispersion block
											// entry
											tripletList.push_back(T(offset + comp * sComp +
																		(_disc.nParCell[parType] - 2) * sCell +
																		i * sNode,
																	offset + idxr.strideParLiquid() +
																		idxr.offsetBoundComp(ParticleTypeIndex{parType},
																							 ComponentIndex{comp}) +
																		bnd + (_disc.nParCell[parType] - 2) * sCell -
																		sCell - sNode + j * sNode,
																	0.0));
															...
															bnd])
											{
												...
												// bound state offset. go node strides from there for each dispersion
												// block entry col: jump over previous cells and over liquid states, add
												// current bound state offset. go back one cell and node and go node
												// strides from there for each dispersion block entry
												tripletList.push_back(
													T(offset + (_disc.nParCell[parType] - 2) * sCell +
														  idxr.strideParLiquid() +
														  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																			   ComponentIndex{comp}) +
														  bnd + i * sNode,
						...
						for (unsigned int comp = 0; comp < _disc.nComp; comp++)
						{
							for (unsigned int i = 0; i < nNodes; i++)
							{
								for (unsigned int j = 0; j < 3 * nNodes + 2; j++)
								{
									...
									// row: add component offset and jump over previous cells. Go node strides from
									// there for each dispersion block entry col: add component offset and jump over
									// previous cells. Go back one cell and node. Go node strides from there for each
									// dispersion block entry.
									tripletList.push_back(
										T(offset + comp * sComp + cell * sCell + i * sNode,
										  offset + comp * sComp + cell * sCell - sCell - sNode + j * sNode, 0.0));
									...
									if (_hasSurfaceDiffusion[parType])
									{
											 ...
											 bnd++)
										{
											if (_parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)] != 0.0)
											{
												...
												// strides from there for each dispersion block entry col: jump over
												// liquid states, add current bound state offset and jump over previous
												// cells. Go back one cell and node and go node strides from there for
												// each dispersion block entry
												tripletList.push_back(
													T(offset + comp * sComp + cell * sCell + i * sNode,
													  offset + idxr.strideParLiquid() +
														  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																			   ComponentIndex{comp}) +
														  bnd + cell * sCell - sCell - sNode + j * sNode,
													  0.0));
												...
												{
													// row: jump over previous cells and over liquid states, add current
													// bound state offset. go node strides from there for each
													// dispersion block entry col: jump over previous cells and over
													// liquid states, add current bound state offset. go back one cell
													// and node and go node strides from there for each dispersion block
													// entry
													tripletList.push_back(
														T(offset + cell * sCell + idxr.strideParLiquid() +
															  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																				   ComponentIndex{comp}) +
															  bnd + i * sNode,
														  offset + cell * sCell + idxr.strideParLiquid() +
															  idxr.offsetBoundComp(ParticleTypeIndex{parType},
																				   ComponentIndex{comp}) +
															  bnd - sCell - sNode + j * sNode,
														  0.0));
												}
											}
										}
	...
	unsigned int calcParDispNNZ(int parType)
	{
		...
		{
			return _disc.nComp *
				   ((3u * _disc.nParCell[parType] - 2u) * _disc.nParNode[parType] * _disc.nParNode[parType] +
					(2u * _disc.nParCell[parType] - 3u) * _disc.nParNode[parType]);
		}
		else
		{
			return _disc.nComp * (_disc.nParCell[parType] * _disc.nParNode[parType] * _disc.nParNode[parType] +
		...
		for (int parNode = 0; parNode < _disc.nParPoints[parType]; parNode++)
		{
			for (int bnd = 0; bnd < _disc.strideBound[parType]; bnd++)
			{
				for (int conc = 0; conc < idxr.strideParNode(parType); conc++)
				{
					...
					tripletList.push_back(
						T(offset + parNode * idxr.strideParNode(parType) + idxr.strideParLiquid() + bnd,
	 ...
	 *@detail since the main diagonal entries are already set, we actually only set the solid phase time derivative
	 *entries for the discretized particle mass balance equations
	 */
								  ...
								  unsigned int secIdx)
	{
		...
		for (unsigned int parNode = 0; parNode < _disc.nParPoints[parType]; parNode++)
		{
			...
			if (!_disc.parExactInt[parType] && parNode == 0u && _parGeomSurfToVol[parType] != _disc.SurfVolRatioSlab &&
			...
			for (unsigned int comp = 0; comp < _disc.nComp; comp++)
			{
				...
				for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
				{
					...
					tripletList.push_back(T(offset + parNode * idxr.strideParNode(parType) + comp,
											offset + parNode * idxr.strideParNode(parType) + idxr.strideParLiquid() +
												idxr.offsetBoundComp(ParticleTypeIndex{parType}, ComponentIndex{comp}) +
												bnd,
											0.0));
				}
	...
	/**
	 * @brief returns the offset between state order and parameter storage (bnd0comp0, bnd0comp1, bnd0comp2, bnd1comp0,
	 * bnd1comp1, bnd1comp2, ...) for one components bound state for a certain particle type
	...
	unsigned int getOffsetSurfDiff(unsigned int parType, unsigned int comp, unsigned int bnd)
	{
		...
		{
			if (_comp < comp) // if its a component that occurs before comp, add all bound states of that component up
							  // to bnd + 1 (note that bound index starts at 0 -> +1).
				offNextBound += std::min(bnd + 1u, _disc.nBound[parType * _disc.nComp + _comp]);
			else // Otherwise, only add all previous (i.e. up to bnd) bound states of that component. This includes the
				 // current component itself (comp == _comp).
	...
	void orderSurfDiff()
	{
		...
		for (unsigned int type = 0; type < _disc.nParType; type++)
		{
			for (unsigned int comp = 0; comp < _disc.nComp; comp++)
			{
				...
				{
					_disc.offsetSurfDiff[idxr.offsetBoundComp(ParticleTypeIndex{type}, ComponentIndex{comp}) + bnd] =
						getOffsetSurfDiff(type, comp, bnd);
				}
			}
		}
	}
	/**
	 * @brief analytically calculates the particle dispersion jacobian of the DGSEM (exact integration) for a single
	 * particle type and bead
	 */
	int calcParticleDGSEMJacobian(unsigned int parType, unsigned int colNode, const active* const parDiff,
								  const active* const parSurfDiff, const double* const invBetaP)
	{
		...
		if (_disc.nParCell[parType] == 1)
		{
			...
			linalg::BandedEigenSparseRowIterator jacIt(
				_globalJac,
				idxr.offsetCp(ParticleTypeIndex{parType},
							  ParticleIndex{colNode})); // row iterator starting at first cell, first component

			insertParJacBlock(
				_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + 0].block(0, nNodes + 1, nNodes, nNodes), jacIt,
				idxr, parDiff, parSurfDiff, invBetaP, _binding[parType]->reactionQuasiStationarity(), parType, 1u, 0);
			return 1;
		}
		...
		if (_disc.nParCell[parType] == 2)
		{
			...
			insertParJacBlock(
				_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + 0].block(0, nNodes + 1, nNodes, 2 * nNodes),
				jacIt, idxr, parDiff, parSurfDiff, invBetaP, _binding[parType]->reactionQuasiStationarity(), parType,
				1u, 0);
			// right Bacobian block, iterator is already moved to second cell
			insertParJacBlock(_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + 1].block(0, 1, nNodes, 2 * nNodes),
							  jacIt, idxr, parDiff, parSurfDiff, invBetaP,
							  _binding[parType]->reactionQuasiStationarity(), parType, 1u,
		...
		if (_disc.nParCell[parType] == 3)
		{
			...
			linalg::BandedEigenSparseRowIterator jacIt(
				_globalJac, idxr.offsetCp(ParticleTypeIndex{parType}, ParticleIndex{colNode}) +
								idxr.strideParShell(parType)); // row iterator starting at first cell, first component

			insertParJacBlock(_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + 1].block(0, 1, nNodes, 3 * nNodes),
							  jacIt, idxr, parDiff, parSurfDiff, invBetaP,
							  _binding[parType]->reactionQuasiStationarity(), parType, 1u,
							  -idxr.strideParShell(parType));
		}
		...
		if (_disc.nParCell[parType] >= 5)
		{
			...
			linalg::BandedEigenSparseRowIterator jacIt(
				...
				idxr.offsetCp(ParticleTypeIndex{parType}, ParticleIndex{colNode}) +
					idxr.strideParShell(parType) * 2); // row iterator starting at third cell, first component

			// insert all (nCol - 4) inner cell blocks
			for (unsigned int cell = 2; cell < _disc.nParCell[parType] - 2; cell++)
				insertParJacBlock(_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + cell], jacIt, idxr, parDiff,
								  parSurfDiff, invBetaP, _binding[parType]->reactionQuasiStationarity(), parType, 1u,
								  -(idxr.strideParShell(parType) + idxr.strideParNode(parType)));
		}
		...
		if (_disc.nParCell[parType] >= 4)
		{
			...
			linalg::BandedEigenSparseRowIterator jacIt(
				_globalJac, idxr.offsetCp(ParticleTypeIndex{parType}, ParticleIndex{colNode}) +
								idxr.strideParShell(parType)); // row iterator starting at second cell, first component

			insertParJacBlock(
				_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + 1].block(0, 1, nNodes, 3 * nNodes + 1), jacIt,
				idxr, parDiff, parSurfDiff, invBetaP, _binding[parType]->reactionQuasiStationarity(), parType, 1u,
				-idxr.strideParShell(parType));

			jacIt += (_disc.nParCell[parType] - 4) *
					 idxr.strideParShell(parType); // move iterator to preultimate cell (already at third cell)
			insertParJacBlock(
				_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + _disc.nParCell[parType] - 2u].block(
					0, 0, nNodes, 3 * nNodes + 1),
				jacIt, idxr, parDiff, parSurfDiff, invBetaP, _binding[parType]->reactionQuasiStationarity(), parType,
				1u, -(idxr.strideParShell(parType) + idxr.strideParNode(parType)));
		}
		...
		if (_disc.nParCell[parType] >= 3)
		{
			...
			insertParJacBlock(
				_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + 0].block(0, nNodes + 1, nNodes, 2 * nNodes + 1),
				jacIt, idxr, parDiff, parSurfDiff, invBetaP, _binding[parType]->reactionQuasiStationarity(), parType,
				1u, 0);

			jacIt += (_disc.nParCell[parType] - 2) *
					 idxr.strideParShell(parType); // move iterator to last cell (already at second cell)
			insertParJacBlock(
				_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + _disc.nParCell[parType] - 1u].block(
					0, 0, nNodes, 2 * nNodes + 1),
				jacIt, idxr, parDiff, parSurfDiff, invBetaP, _binding[parType]->reactionQuasiStationarity(), parType,
				1u, -(idxr.strideParShell(parType) + idxr.strideParNode(parType)));
		}
	...
	/**
	 * @brief analytically calculates the particle dispersion jacobian of the collocation DGSEM (inexact integration)
	 * for one particle type and bead
	 * @note deprecated, not further development
	 */
	int calcParticleCollocationDGSEMJacobian(unsigned int parType, unsigned int colNode, const active* const parDiff,
											 const active* const parSurfDiff, const double* const invBetaP)
	{
		...
		if (_disc.nParCell[parType] == 1)
		{
			...
			if (_parGeomSurfToVol[parType] == _disc.SurfVolRatioSlab || _parCoreRadius[parType] != 0.0)
				dispBlock = invMap * invMap * (_disc.Dr[parType] - _disc.parInvWeights[parType].asDiagonal() * B) *
							_disc.parPolyDerM[parType];

			else
			{ // special treatment of inner boundary node for spherical and cylindrical particles without particle core
				...
				dispBlock.block(1, 0, nNodes - 1, nNodes) =
					(_disc.Dr[parType].block(1, 1, nNodes - 1, nNodes - 1) -
					 _disc.parInvWeights[parType].segment(1, nNodes - 1).asDiagonal() *
						 B.block(1, 1, nNodes - 1, nNodes - 1)) *
				...
				dispBlock.block(0, 0, 1, nNodes) =
					-(_disc.Ir[parType]
						  ...
						  .cwiseProduct(_disc.parInvWeights[parType].segment(1, nNodes - 1).cwiseInverse())
						  .cwiseProduct(_disc.parPolyDerM[parType].block(1, 0, nNodes - 1, 1)))
						 .transpose() *
					_disc.parPolyDerM[parType].block(1, 0, nNodes - 1, nNodes);
			...
			for (unsigned int comp = 0; comp < _disc.nComp; comp++)
			{
				for (unsigned int i = 0; i < dispBlock.rows(); i++)
				{
					for (unsigned int j = 0; j < dispBlock.cols(); j++)
					{
						...
						// row: add component offset and go node strides from there for each dispersion block entry
						// col: add component offset and go node strides from there for each dispersion block entry
						_globalJac.coeffRef(offset + comp * sComp + i * sNode, offset + comp * sComp + j * sNode) =
							-(static_cast<double>(parDiff[comp])) *
							dispBlock(i, j); // - D_p * (Delta r / 2)^2 * (D_r D - M^-1 B D)
								...
								if (static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) != 0.0)
								{
									...
									_globalJac.coeffRef(
										...
										offset + idxr.strideParLiquid() +
											idxr.offsetBoundComp(ParticleTypeIndex{parType}, ComponentIndex{comp}) +
											bnd + j * sNode) =
										-(static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) *
										  invBetaP[comp]) *
										dispBlock(i, j); // -  D_s * (1 / Beta_p) * (Delta r / 2)^2 * (D_r D - M^-1 B D)
									...
									if (!qsReaction[idxr.offsetBoundComp(ParticleTypeIndex{parType},
													...
													bnd])
									{
										// row: jump oover liquid states, add current bound state offset and go node
										// strides from there for each dispersion block entry col: jump oover liquid
										// states, add current bound state offset and go node strides from there for
										// each dispersion block entry
										_globalJac.coeffRef(
											offset + idxr.strideParLiquid() +
											...
											offset + idxr.strideParLiquid() +
												...
												bnd + j * sNode) =
											-(static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)])) *
											dispBlock(i, j); // -  D_s * (Delta r / 2)^2 * (D_r D - M^-1 B D)
									}
								}
							}
						}
		...
		else
		{
			...
			if (_parGeomSurfToVol[parType] == _disc.SurfVolRatioSlab || _parCoreRadius[parType] != 0.0)
			{
				...
				bnd_dispBlock.block(0, 0, nNodes, nNodes + 1) =
					(_disc.Dr[_disc.offsetMetric[parType]] - _disc.parInvWeights[parType].asDiagonal() * B) * GBlock_l;
				bnd_dispBlock.block(0, 0, nNodes, 2 * nNodes) +=
					_disc.parInvWeights[parType].asDiagonal() * B * bnd_gStarDC;
			...
			{ // special treatment of inner boundary node for spherical and cylindrical particles without particle core
				...
				bnd_dispBlock.block(0, 0, 1, nNodes + 1) =
					-(_disc.Ir[_disc.offsetMetric[parType]]
						  .template cast<double>()
						  .segment(1, nNodes - 1)
						  .cwiseProduct(_disc.parInvWeights[parType].segment(1, nNodes - 1).cwiseInverse())
						  .cwiseProduct(_disc.parPolyDerM[parType].block(1, 0, nNodes - 1, 1)))
				...
				bnd_dispBlock.block(1, 0, nNodes - 1, 2 * nNodes) +=
					_disc.parInvWeights[parType].segment(1, nNodes - 1).asDiagonal() *
					B.block(1, 1, nNodes - 1, nNodes - 1) * bnd_gStarDC.block(1, 0, nNodes - 1, 2 * nNodes);
				...
				for (unsigned int i = 0; i < bnd_dispBlock.rows(); i++)
				{
					for (unsigned int j = 0; j < bnd_dispBlock.cols(); j++)
					{
						...
						if ((j <= nNodes) || (i == nNodes - 1))
						{
							...
							_globalJac.coeffRef(offset + comp * sComp + i * sNode, offset + comp * sComp + j * sNode) =
								-static_cast<double>(parDiff[comp]) *
								bnd_dispBlock(i, j); // dispBlock <- D_p * [ M^-1 * M_r * G_l +  invMap * ( D * G_l -
													 // M^-1 * B * [G_l - g^*] ) ]
								...
								for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
								{
									if (static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) != 0.0)
									{
										...
										_globalJac.coeffRef(
											...
											-static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) *
											invBetaP[comp] *
											bnd_dispBlock(i, j); // dispBlock <- D_s * invBeta * [ M^-1 * M_r * G_l +
																 // invMap * ( D * G_l - M^-1 * B * [G_l - g^*] ) ]
														...
														bnd])
										{
											...
											// states, add current bound state offset and go node strides from there for
											// each dispersion block entry
											_globalJac.coeffRef(offset + idxr.strideParLiquid() +
																	idxr.offsetBoundComp(ParticleTypeIndex{parType},
																						 ComponentIndex{comp}) +
																	bnd + i * sNode,
																offset + idxr.strideParLiquid() +
																	...
																	bnd + j * sNode) =
												-(static_cast<double>(
													parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)])) *
												bnd_dispBlock(i, j); // -  D_s * (Delta r / 2)^2 * (D_r D - M^-1 B D)
										}
									}
								}
							}
			...
			/*			 right boundary cell				*/
			invMap =
				(2.0 / static_cast<double>(_disc.deltaR[_disc.offsetMetric[parType] + _disc.nParCell[parType] - 1]));
			...
			bnd_dispBlock.block(0, nNodes - 1, nNodes, nNodes + 1) =
				(_disc.Dr[_disc.offsetMetric[parType] + _disc.nParCell[parType] - 1] -
				 _disc.parInvWeights[parType].asDiagonal() * B) *
				GBlock_r;
			bnd_dispBlock.block(0, 0, nNodes, 2 * nNodes) +=
				_disc.parInvWeights[parType].asDiagonal() * B * bnd_gStarDC;
			bnd_dispBlock *= invMap * invMap;
				...
				for (unsigned int i = 0; i < bnd_dispBlock.rows(); i++)
				{
					for (unsigned int j = 0; j < bnd_dispBlock.cols(); j++)
					{
						...
						if ((j <= nNodes) || (i == nNodes - 1))
						{
							// row: add component offset and jump over previous cells. Go node strides from there for
							// each dispersion block entry col: add component offset and jump over previous cells. Go
							// back one cell and go node strides from there for each dispersion block entry
							_globalJac.coeffRef(
								offset + comp * sComp + (_disc.nParCell[parType] - 1) * sCell + i * sNode,
							...
							if (_hasSurfaceDiffusion[parType])
							{
								...
								for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
								{
									if (static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) != 0.0)
									{
										// row: add component offset and jump over previous cells. Go node strides from
										// there for each dispersion block entry col: jump over liquid states, add
										// current bound state offset and jump over previous cells. Go back one cell and
										// go node strides from there for each dispersion block entry
										_globalJac.coeffRef(
											offset + comp * sComp + (_disc.nParCell[parType] - 1) * sCell + i * sNode,
											offset + idxr.strideParLiquid() +
												idxr.offsetBoundComp(ParticleTypeIndex{parType}, ComponentIndex{comp}) +
														...
														bnd])
										{
											...
											// state offset. go node strides from there for each dispersion block entry
											// col: jump over previous cells and over liquid states, add current bound
											// state offset. go back one cell and go node strides from there for each
											// dispersion block entry
											_globalJac.coeffRef(offset + (_disc.nParCell[parType] - 1) * sCell +
																	idxr.strideParLiquid() +
																	idxr.offsetBoundComp(ParticleTypeIndex{parType},
																						 ComponentIndex{comp}) +
																	bnd + i * sNode,
			...
			dispBlock.block(0, nNodes - 1, nNodes, nNodes + 2) =
				-1.0 * _disc.parInvWeights[parType].asDiagonal() * B * GBlock;
			...
			for (int cell = 1; cell < _disc.nParCell[parType] - 1; cell++)
			{
				...
				dispBlock.block(0, nNodes - 1, nNodes, nNodes + 2) +=
					_disc.Dr[_disc.offsetMetric[parType] + cell] * GBlock * invMap * invMap;
						...
						for (unsigned int j = 0; j < dispBlock.cols(); j++)
						{
								...
								(i == nNodes - 1 && j >= nNodes - 1))
							{
								...
								_globalJac.coeffRef(offset + comp * sComp + cell * sCell + i * sNode,
													offset + comp * sComp + (cell - 1) * sCell + j * sNode) =
									-static_cast<double>(parDiff[comp]) *
									dispBlock(i, j); // dispBlock <- D_p * [ M^-1 * M_r * G_l +  invMap * ( D * G_l -
													 // M^-1 * B * [G_l - g^*] ) ]
								...
								if (_hasSurfaceDiffusion[parType])
								{
									...
									for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++)
									{
										if (static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) !=
											0.0)
										{
											// row: add component offset and jump over previous cells. Go node strides
											// from there for each dispersion block entry col: jump over liquid states,
											...
											_globalJac.coeffRef(offset + comp * sComp + cell * sCell + i * sNode,
												...
												-static_cast<double>(
													parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) *
												invBetaP[comp] *
												dispBlock(i, j); // dispBlock <- D_s * invBeta * [ M^-1 * M_r * G_l +
																 // invMap * ( D * G_l - M^-1 * B * [G_l - g^*] ) ]
															...
															bnd])
											{
												...
												// state offset and go node strides from there for each dispersion block
												// entry col: jump over previous cells and liquid states, go back one
												// cell and add current bound state offset and go node strides from
												// there for each dispersion block entry
												_globalJac.coeffRef(offset + cell * sCell + idxr.strideParLiquid() +
																		idxr.offsetBoundComp(ParticleTypeIndex{parType},
																							 ComponentIndex{comp}) +
																		bnd + i * sNode,
																	offset + (cell - 1) * sCell +
	...
	/**
	 * @brief adds jacobian entries which have been overwritten by the binding kernel (only use for surface diffusion
	 * combined with kinetic binding)
	...
	int addSolidDGentries(unsigned int parType, const active* const parSurfDiff)
	{
		...
		for (unsigned int col = 0; col < _disc.nPoints; col++)
		{
			...
			linalg::BandedEigenSparseRowIterator jac(
				_globalJac, idxr.offsetCp(ParticleTypeIndex{parType}, ParticleIndex{col}) + idxr.strideParLiquid());

			for (unsigned int cell = 0; cell < _disc.nParCell[parType]; cell++)
				addDiagonalSolidJacobianEntries(
					_disc.DGjacParDispBlocks[_disc.offsetMetric[parType] + cell].block(
						0, _disc.nParNode[parType] + 1, _disc.nParNode[parType], _disc.nParNode[parType]),
	...
	int addSolidDGentries_inexInt(unsigned int parType, const active* const parSurfDiff)
	{
				...
				for (unsigned int node = 0; node < _disc.nParNode[parType]; node++, jac += idxr.strideParLiquid())
				{
					for (unsigned int comp = 0; comp < _disc.nComp; comp++)
					{
						for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++, ++jac)
						{
							if (static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) != 0.0)
							{
								jac[0] += -(static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)])) *
										  dispBlock(node, node);
							}
						}
					}
		...
		else
		{
			...
			bnd_dispBlock.setZero();
			bnd_dispBlock.block(0, nNodes - 1, nNodes, nNodes + 1) =
				(_disc.Dr[_disc.offsetMetric[parType] + _cell] - _disc.parInvWeights[parType].asDiagonal() * B) *
				GBlock_r;
			bnd_dispBlock.block(0, 0, nNodes, 2 * nNodes) +=
				_disc.parInvWeights[parType].asDiagonal() * B * bnd_gStarDC;
			bnd_dispBlock *= invMap * invMap;
			...
			for (unsigned int colNode = 0; colNode < _disc.nPoints; colNode++)
			{
				...
				linalg::BandedEigenSparseRowIterator jac_right(
					_globalJac, offset + (_disc.nParCell[parType] - 1) * sCell + idxr.strideParLiquid());
					...
					for (unsigned int comp = 0; comp < _disc.nComp; comp++)
					{
						for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; bnd++, ++jac_right)
						{
							if (static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) != 0.0)
							{
								jac_right[0] +=
									-(static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)])) *
									bnd_dispBlock(node, _disc.nParNode[parType] + node);
							}
						}
					}
				}
			...
			for (int cell = 1; cell < _disc.nParCell[parType] - 1; cell++)
			{
				...
				// add metric part, dependent on current cell
				dispBlock.block(0, nNodes - 1, nNodes, nNodes + 2) +=
					_disc.Dr[_disc.offsetMetric[parType] + cell] * GBlock;
				...
				for (unsigned int colNode = 0; colNode < _disc.nPoints; colNode++)
				{
					...
					linalg::BandedEigenSparseRowIterator jac_inner(_globalJac,
																   offset + cell * sCell + idxr.strideParLiquid());

					for (unsigned int node = 0; node < _disc.nParNode[parType];
						 node++, jac_inner += idxr.strideParLiquid())
					{
						for (unsigned int comp = 0; comp < _disc.nComp; comp++)
						{
							for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp];
								 bnd++, ++jac_inner)
							{
								if (static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) != 0.0)
								{
									jac_inner[0] +=
										-(static_cast<double>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)])) *
										dispBlock(node, _disc.nParNode[parType] + node);
								}
							}
						}
					}
	 ...
	 */
	void addJacBlock(Eigen::MatrixXd block, linalg::BandedEigenSparseRowIterator& jac, int offRowToCol, Indexer& idxr,
					 unsigned int nCells, unsigned int strideNode, unsigned int nStates, unsigned int strideDead = 0)
	{
	...
	void addDiagonalSolidJacobianEntries(Eigen::MatrixXd block, linalg::BandedEigenSparseRowIterator& jac,
										 Indexer& idxr, ParamType* surfDiff, const int* nonKinetic, unsigned int type)
	{
		...
		for (unsigned int i = 0; i < block.rows(); i++, jac += idxr.strideParLiquid())
		{
			for (unsigned int comp = 0; comp < _disc.nComp; comp++)
			{
				...
				{
					if (static_cast<double>(surfDiff[_disc.offsetSurfDiff[idxr.offsetBoundComp(ParticleTypeIndex{type},
																							   ComponentIndex{comp}) +
																		  bnd]]) != 0.0 &&
						!nonKinetic[idxr.offsetBoundComp(ParticleTypeIndex{type}, ComponentIndex{comp}) + bnd])
					{
						...
						jac[0] += block(i, i) *
								  static_cast<double>(
									  surfDiff[_disc.offsetSurfDiff[idxr.offsetBoundComp(ParticleTypeIndex{type},
																						 ComponentIndex{comp}) +
																	bnd]]);
					}
				}
	 ...
	 * @param [in] offRowToCol column to row offset (i.e. start at upper left corner of block)
	 */
	void insertParJacBlock(Eigen::MatrixXd block, linalg::BandedEigenSparseRowIterator& jac, Indexer& idxr,
						   const active* const parDiff, const active* const surfDiff, const double* const beta_p,
						   const int* nonKinetic, unsigned int type, unsigned int nBlocks, int offRowToCol)
	{
			...
			for (unsigned int i = 0; i < block.rows(); i++)
			{
				for (unsigned int comp = 0; comp < _disc.nComp; comp++, ++jac)
				{
					for (unsigned int j = 0; j < block.cols(); j++)
					{
						...
						jac[(j - i) * idxr.strideParNode(type) + offRowToCol] =
							block(i, j) * static_cast<double>(parDiff[comp]);
					}
	 ...
	 */
	int calcStaticAnaParticleDispJacobian(unsigned int parType, unsigned int colNode, const active* const parDiff,
										  const active* const parSurfDiff, const double* const invBetaP)
	{
		...
		return _globalJac.isCompressed(); // if matrix lost its compressed storage, the calculation did not fit the
										  // pre-defined pattern.
	}
	...
	void setJacobianPattern_GRM(SparseMatrix<double, RowMajor>& globalJ, unsigned int secIdx, bool hasBulkReaction)
	{
		...
		if (hasBulkReaction)
			bulkEntries += _disc.nPoints * _disc.nComp *
						   _disc.nComp; // add nComp entries for every component at each discrete bulk point
		...
		{
			isothermNNZ = (idxr.strideParNode(type)) * _disc.nParPoints[type] *
						  _disc.strideBound[type]; // every bound satte might depend on every bound and liquid state
		...
		if (hasBulkReaction)
		{
			for (unsigned int colNode = 0; colNode < _disc.nPoints; colNode++)
			{
				for (unsigned int comp = 0; comp < _disc.nComp; comp++)
				{
					...
					{
						tripletList.push_back(
							T(idxr.offsetC() + colNode * idxr.strideColNode() + comp * idxr.strideColComp(),
							  idxr.offsetC() + colNode * idxr.strideColNode() + toComp * idxr.strideColComp(), 0.0));
					}
				}
			}
		}
		...
		for (unsigned int type = 0; type < _disc.nParType; type++)
		{
			for (unsigned int colNode = 0; colNode < _disc.nPoints; colNode++)
			{
				for (unsigned int comp = 0; comp < _disc.nComp; comp++)
				{
					...
					tripletList.push_back(
						T(idxr.offsetC() + colNode * idxr.strideColNode() + comp * idxr.strideColComp(),
						  idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{colNode}) +
							  (_disc.nParPoints[type] - 1) * idxr.strideParNode(type) + comp * idxr.strideParComp(),
						  0.0));

					// add Cp on Cl entries
					if (!_disc.parExactInt[type])
						// row: add particle offset to current parType and particle, go to last node and add component
						// offset col: add flux offset to current component, jump over previous nodes and components
						tripletList.push_back(T(idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{colNode}) +
													(_disc.nParPoints[type] - 1) * idxr.strideParNode(type) +
													comp * idxr.strideParComp(),
												idxr.offsetC() + colNode * idxr.strideColNode() + comp, 0.0));
					else
					{
						for (unsigned int node = 0; node < _disc.nParNode[type]; node++)
						{
							// row: add particle offset to current parType and particle, go to last cell and current
							// node and add component offset col: add flux offset to current component, jump over
							// previous nodes and components
							tripletList.push_back(
								T(idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{colNode}) +
									  (_disc.nParCell[type] - 1) * _disc.nParNode[type] * idxr.strideParNode(type) +
	...
	int calcFluxJacobians(unsigned int secIdx, bool outliersOnly = false)
	{
		...
		for (unsigned int type = 0; type < _disc.nParType; type++)
		{
			...
			double exIntLiftContribution = static_cast<double>(
				_disc.Ir[_disc.offsetMetric[type] + _disc.nParCell[type] - 1][_disc.nParNode[type] - 1]);
			...
			linalg::BandedEigenSparseRowIterator jacCp(_globalJac,
													   idxr.offsetCp(ParticleTypeIndex{type}) +
														   (_disc.nParPoints[type] - 1) * idxr.strideParNode(type));
			...
			for (unsigned int colNode = 0; colNode < _disc.nPoints; colNode++)
			{
				for (unsigned int comp = 0; comp < _disc.nComp; comp++, ++jacCp, ++jacCl)
				{
					...
					if (!outliersOnly)
						jacCl[0] += static_cast<double>(filmDiff[comp]) * (1.0 - static_cast<double>(_colPorosity)) /
									static_cast<double>(_colPorosity) * _parGeomSurfToVol[type] /
					...
					jacCl[jacCp.row() - jacCl.row()] = -static_cast<double>(filmDiff[comp]) *
													   (1.0 - static_cast<double>(_colPorosity)) /
													   static_cast<double>(_colPorosity) * _parGeomSurfToVol[type] /
													   static_cast<double>(_parRadius[type]) *
													   _parTypeVolFrac[type + colNode * _disc.nParType].getValue();
					...
					if (!_disc.parExactInt[type])
					{
						...
						if (!outliersOnly) // Cp on Cb
							jacCp[0] += static_cast<double>(filmDiff[comp]) * 2.0 /
										static_cast<double>(_disc.deltaR[_disc.offsetMetric[type]]) *
										_disc.parInvWeights[type][0] / static_cast<double>(_parPorosity[type]) /
										static_cast<double>(_poreAccessFactor[type * _disc.nComp + comp]);
						// row: already at particle. already at current node and liquid state.
						// col: go to current bulk phase.
						jacCp[jacCl.row() - jacCp.row()] =
							-static_cast<double>(filmDiff[comp]) * 2.0 /
							static_cast<double>(_disc.deltaR[_disc.offsetMetric[type]]) * _disc.parInvWeights[type][0] /
						...
						for (int node = _disc.parPolyDeg[type]; node >= 0; node--, jacCp -= idxr.strideParNode(type))
						{
							...
							if (!outliersOnly) // Cp on Cb
								jacCp[entry - jacCp.row()] +=
									static_cast<double>(filmDiff[comp]) * 2.0 /
									static_cast<double>(_disc.deltaR[_disc.offsetMetric[type]]) *
									_disc.parInvMM[_disc.offsetMetric[type] + _disc.nParCell[type] - 1](
										node, _disc.nParNode[type] - 1) *
									exIntLiftContribution / static_cast<double>(_parPorosity[type]) /
									static_cast<double>(_poreAccessFactor[type * _disc.nComp + comp]);
							// row: already at particle. Already at current node and liquid state.
							// col: go to current bulk phase.
							jacCp[jacCl.row() - jacCp.row()] =
				...
				if (colNode <
					_disc.nPoints - 1) // execute iteration statement only when condition is true in next loop.
	...
	int calcStaticAnaJacobian_GRM(unsigned int secIdx)
	{
		...
		for (int colNode = 0; colNode < _disc.nPoints; colNode++)
		{
			for (int type = 0; type < _disc.nParType; type++)
			{
				...
				{
					invBetaP[comp] = (1.0 - static_cast<double>(_parPorosity[type])) /
									 (static_cast<double>(_poreAccessFactor[_disc.nComp * type + comp]) *
									  static_cast<double>(_parPorosity[type]));
				}
	 ...
	 */
	MatrixXd calcFDJacobian(const double* y_, const double* yDot_, const SimulationTime simTime,
							util::ThreadLocalStorage& threadLocalMem, double alpha)
	{
		...
		{
			y[dof] += epsilon;
			residualImpl<double, double, double, false>(simTime.t, simTime.secIdx, yPtr, yDotPtr, resPtr,
														threadLocalMem);
			y[dof] -= epsilon;
