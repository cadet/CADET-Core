...
//	IExternalFunction* _extFun; //!< External function (owned by library user)
	...
	public:
		...
		{
			return _model._parDiffOp.writeParticleCoordinates(parType, coords);
		}
	...
	void setJacobianPattern_GRM(SparseMatrix<double, RowMajor>& globalJ, unsigned int secIdx, bool hasBulkReaction) {
		...
		for (int type = 0; type < _disc.nParType; type++) {
			...
			particleEntries += _parDiffOp.calcParDiffNNZ(type) + addTimeDer + isothermNNZ;
		}
		...
		for (int colNode = 0; colNode < _disc.nPoints; colNode++) {
			for (int type = 0; type < _disc.nParType; type++) {
				_parDiffOp.setParJacPattern(tripletList, type, colNode, secIdx, idxr.offsetCp(ParticleTypeIndex{ static_cast<unsigned int>(type) },ParticleIndex{ static_cast<unsigned int>(colNode) }), _binding[type]->reactionQuasiStationarity());
			}
		}
				...
				for (unsigned int comp = 0; comp < _disc.nComp; comp++, ++jacCp, ++jacCl) {
					...
					if (!_disc.parExactInt[type]) {
						...
						if (!outliersOnly) // Cp on Cb
							jacCp[0] += static_cast<double>(filmDiff[comp]) * 2.0 / static_cast<double>(_parDiffOp._deltaR[_parDiffOp._offsetMetric[type]]) * _parDiffOp._parInvWeights[type][0] / static_cast<double>(_parPorosity[type]) / static_cast<double>(_poreAccessFactor[type * _disc.nComp + comp]);
						// row: already at particle. already at current node and liquid state.
						// col: go to current bulk phase.
						jacCp[jacCl.row() - jacCp.row()] = -static_cast<double>(filmDiff[comp]) * 2.0 / static_cast<double>(_parDiffOp._deltaR[_parDiffOp._offsetMetric[type]]) * _parDiffOp._parInvWeights[type][0] / static_cast<double>(_parPorosity[type]) / static_cast<double>(_poreAccessFactor[type * _disc.nComp + comp]);
					}
					else {
	...
	int calcStaticAnaJacobian_GRM(unsigned int secIdx)
	{
		...
		// particle jacobian (without isotherm, which is handled in residualKernel due to non-linearities)
		for (int colNode = 0; colNode < _disc.nPoints; colNode++)
		{
			for (int type = 0; type < _disc.nParType; type++)
			{
				...
				_parDiffOp.calcStaticAnaParticleDiffJacobian(type, colNode, parDiff, parSurfDiff, invBetaP, _binding[type]->reactionQuasiStationarity(), idxr.offsetCp(ParticleTypeIndex{ static_cast<unsigned int>(type) }, ParticleIndex{ static_cast<unsigned int>(colNode) }), _globalJac);
			}
		}
