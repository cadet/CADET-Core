...
public:
	...
	{
		return "LUMPED_RATE_MODEL_WITH_PORES_DG";
	}
...
protected:
	...
	void addTimeDerivativeToJacobianParticleBlock(linalg::BandedEigenSparseRowIterator& jac, const Indexer& idxr,
	...
	public:
		...
		{
			return offsetCp(pti) + strideParBlock(pti.value) * pi.value;
		}
	...
	{
	public:
		Exporter(const Discretization& disc, const LumpedRateModelWithPoresDG& model, double const* data)
		...
		{
			return _disc.nComp * _disc.nPoints * _disc.nParType;
		}
		virtual unsigned int numSolidPhaseDofs(unsigned int parType) const CADET_NOEXCEPT
		{
			return _disc.strideBound[parType] * _disc.nPoints;
		}
		...
		/**
		 * @brief calculates and writes the physical node coordinates of the DG discretization with double! interface
		 * nodes
		 */
	...
	int particlePattern(std::vector<T>& tripletList)
	{
		...
		for (unsigned int parType = 0; parType < _disc.nParType; parType++)
		{
			for (unsigned int nCol = 0; nCol < _disc.nPoints; nCol++)
			{
				...
				int offset = idxr.offsetCp(ParticleTypeIndex{parType}, ParticleIndex{nCol}) -
							 idxr.offsetC(); // inlet DOFs not included in Jacobian
				...
				{
					for (unsigned int toParState = 0; toParState < _disc.nComp + _disc.strideBound[parType];
						 toParState++)
					{
	...
	int fluxPattern(std::vector<T>& tripletList)
	{
		...
		for (unsigned int parType = 0; parType < _disc.nParType; parType++)
		{
			...
			int offP =
				idxr.offsetCp(ParticleTypeIndex{parType}) - idxr.offsetC(); // inlet DOFs not included in Jacobian
			...
			for (unsigned int nCol = 0; nCol < _disc.nPoints; nCol++)
			{
				for (unsigned int comp = 0; comp < _disc.nComp; comp++)
				{
					...
					tripletList.push_back(T(offC + nCol * _disc.nComp + comp,
											offP + nCol * idxr.strideParBlock(parType) + comp, 0.0)); // c^b on c^p
					// c^p on c^p entry already set
					tripletList.push_back(T(offP + nCol * idxr.strideParBlock(parType) + comp,
											offC + nCol * _disc.nComp + comp, 0.0)); // c^p on c^b
				}
			}
		}
		return 1;
	...
	int calcFluxJacobians(const unsigned int secIdx, const bool crossDepsOnly = false)
	{
		...
		for (unsigned int type = 0; type < _disc.nParType; type++)
		{
			...
			linalg::BandedEigenSparseRowIterator jacP(_globalJac,
													  idxr.offsetCp(ParticleTypeIndex{type}) - idxr.offsetC());
			...
			for (unsigned int colNode = 0; colNode < _disc.nPoints; colNode++, jacP += _disc.strideBound[type])
			{
				for (unsigned int comp = 0; comp < _disc.nComp; comp++, ++jacC, ++jacP)
				{
					...
					if (!crossDepsOnly)
						jacC[0] += jacCF_val * static_cast<double>(filmDiff[comp]) *
								   static_cast<double>(_parTypeVolFrac[type + _disc.nParType * colNode]);
					// add Cl on Cp entries
					// row: already at bulk phase. already at current node and component.
					// col: already at bulk phase. already at current node and component.
					jacC[jacP.row() - jacC.row()] =
					...
					// col: go to flux of current parType and adjust for offsetC. jump over previous colNodes and add
					// component offset
					if (!crossDepsOnly)
						jacP[0] =
	...
	unsigned int nJacEntries(const bool hasBulkReaction, const bool pureNNZ = false)
	{
		...
		if (hasBulkReaction)
			nEntries += _disc.nPoints * _disc.nComp *
