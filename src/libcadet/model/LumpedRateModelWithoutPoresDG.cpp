...
LumpedRateModelWithoutPoresDG::LumpedRateModelWithoutPoresDG(UnitOpIdx unitOpIdx)
	: UnitOperationBase(unitOpIdx),
	  /*_jacInlet(),*/ _analyticJac(true), _jacobianAdDirs(0), _factorizeJacobian(false), _tempState(nullptr),
	  _initC(0), _initQ(0), _initState(0), _initStateDot(0)
{
...
bool LumpedRateModelWithoutPoresDG::configureModelDiscretization(IParameterProvider& paramProvider,
																 const IConfigHelper& helper)
{
	...
	const bool transportSuccess = _convDispOp.configureModelDiscretization(
		paramProvider, helper, _disc.nComp, polynomial_integration_mode, _disc.nCol, _disc.polyDeg, strideNode);
	...
	_jacDisc.resize((_disc.nComp + _disc.strideBound) * _disc.nPoints,
					(_disc.nComp + _disc.strideBound) * _disc.nPoints);
...
void LumpedRateModelWithoutPoresDG::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx,
...
void LumpedRateModelWithoutPoresDG::extractJacobianFromAD(active const* const adRes, unsigned int adDirOffset)
{
	...
	ad::extractBandedBlockEigenJacobianFromAd(adVec, adDirOffset, diagDir, lowerBandwidth, upperBandwidth, colOffset,
											  nRows, _jac);
}
 ...
 * @param [in] adDirOffset Number of AD directions used for non-Jacobian purposes (e.g., parameter sensitivities)
 */
void LumpedRateModelWithoutPoresDG::checkAnalyticJacobianAgainstAd(active const* const adRes,
																   unsigned int adDirOffset) const
{
	...
	const double maxDiff = ad::compareBandedEigenJacobianWithAd(
		adRes + idxr.offsetC(), adDirOffset, lowerBandwidth, lowerBandwidth, upperBandwidth, 0, _jac.rows(), _jac, 0);
...
#endif
int LumpedRateModelWithoutPoresDG::jacobian(const SimulationTime& simTime, const ConstSimulationState& simState,
...
int LumpedRateModelWithoutPoresDG::residual(const SimulationTime& simTime, const ConstSimulationState& simState,
...
int LumpedRateModelWithoutPoresDG::residualWithJacobian(const SimulationTime& simTime,
...
int LumpedRateModelWithoutPoresDG::residualImpl(double t, unsigned int secIdx, StateType const* const y_,
												double const* const yDot_, ResidualType* const res_,
												util::ThreadLocalStorage& threadLocalMem)
{
	Indexer idxr(_disc);
	...
	for (unsigned int blk = 0; blk < _disc.nPoints; ++blk)
		...
		if (wantRes)
		{
			...
			parts::cell::residualKernel<StateType, ResidualType, ParamType, parts::cell::CellParameters,
										linalg::BandedEigenSparseRowIterator, wantJac, false>(
				t, secIdx, ColumnPosition{z, 0.0, 0.0}, localY, localYdot, localRes, jacIt, cellResParams,
				threadLocalMem.get());
		}
		else
		{
			parts::cell::residualKernel<StateType, ResidualType, ParamType, parts::cell::CellParameters,
				...
				t, secIdx, ColumnPosition{z, 0.0, 0.0}, localY, nullptr, nullptr, jacIt, cellResParams,
				threadLocalMem.get());
		}
			} CADET_PARFOR_END;
...
int LumpedRateModelWithoutPoresDG::residualSensFwdWithJacobian(const SimulationTime& simTime,
															   const ConstSimulationState& simState,
															   const AdJacobianParams& adJac,
															   util::ThreadLocalStorage& threadLocalMem)
{
...
int LumpedRateModelWithoutPoresDG::residualSensFwdAdOnly(const SimulationTime& simTime,
...
int LumpedRateModelWithoutPoresDG::residualSensFwdCombine(const SimulationTime& simTime,
														  const ConstSimulationState& simState,
 ...
 */
void LumpedRateModelWithoutPoresDG::multiplyWithJacobian(const SimulationTime& simTime,
 ...
 */
void LumpedRateModelWithoutPoresDG::multiplyWithDerivativeJacobian(const SimulationTime& simTime,
																   const ConstSimulationState& simState,
...
void LumpedRateModelWithoutPoresDG::expandErrorTol(double const* errorSpec, unsigned int errorSpecSize,
												   double* expandOut)
{
 ...
 */
int LumpedRateModelWithoutPoresDG::linearSolve(double t, double alpha, double outerTol, double* const rhs,
 ...
 */
void LumpedRateModelWithoutPoresDG::addTimeDerivativeToJacobianNode(linalg::BandedEigenSparseRowIterator& jac,
 ...
 */
void LumpedRateModelWithoutPoresDG::consistentInitialState(const SimulationTime& simTime, double* const vecStateY,
														   ...
														   util::ThreadLocalStorage& threadLocalMem)
{
	...
	for (unsigned int point = 0; point < _disc.nPoints; point++)
		...
		linalg::DenseMatrixView fullJacobianMatrix(_jacDisc.valuePtr() + point * _disc.strideBound * _disc.strideBound,
		...
		linalg::DenseMatrixView jacobianMatrix(jacobianMem, _jacDisc.innerIndexPtr() + point * _disc.strideBound,
											   probSize, probSize);
		const parts::cell::CellParameters cellResParams{
			_disc.nComp,
			_disc.nBound,
 ...
 */
void LumpedRateModelWithoutPoresDG::consistentInitialTimeDerivative(const SimulationTime& simTime,
 ...
 * @param [in] errorTol Error tolerance for algebraic equations
 */
void LumpedRateModelWithoutPoresDG::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY,
															   const AdJacobianParams& adJac, double errorTol,
															   util::ThreadLocalStorage& threadLocalMem)
{
}
 ...
 */
void LumpedRateModelWithoutPoresDG::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY,
 ...
 * consistent with the sensitivity DAE. This function updates the initial sensitivity\f$ s_0 \f$ and overwrites the time
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void LumpedRateModelWithoutPoresDG::consistentInitialSensitivity(
	...
	std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	...
	for (std::size_t param = 0; param < vecSensY.size(); ++param)
	{
		...
		if (_binding[0]->hasQuasiStationaryReactions())
		{
			...
			for (unsigned int node = 0; node < _disc.nPoints; ++node)
				...
				const unsigned int jacRowOffset =
					idxr.strideColNode() * node + static_cast<unsigned int>(idxr.strideColLiquid());
				...
				linalg::DenseMatrixView jacobianMatrix(
					_jacDisc.valuePtr() + node * _disc.strideBound * _disc.strideBound,
					_jacDisc.innerIndexPtr() + node * _disc.strideBound, probSize, probSize);
				...
				rhsUnmasked_vec = _jac.block(jacRowOffset, jacRowOffset - static_cast<int>(_disc.nComp),
											 _disc.strideBound, idxr.strideColNode()) *
								  maskedMultiplier_vec;
		...
		for (unsigned int node = 0; node < _disc.nPoints; ++node)
		{
			...
			{
				// Get iterators to beginning of solid phase
				linalg::BandedEigenSparseRowIterator jacSolidOrig(_jac,
																  idxr.strideColNode() * node + idxr.strideColLiquid());
				linalg::BandedEigenSparseRowIterator jacSolid = jac - idxr.strideColLiquid();

				int const* const mask = _binding[0]->reactionQuasiStationarity();
				double* const qShellDot =
					sensYdot + idxr.offsetC() + idxr.strideColNode() * node + idxr.strideColLiquid();
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void LumpedRateModelWithoutPoresDG::leanConsistentInitialSensitivity(
	const SimulationTime& simTime, const ConstSimulationState& simState, std::vector<double*>& vecSensY,
	std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
...
bool LumpedRateModelWithoutPoresDG::setSensitiveParameter(const ParameterId& pId, unsigned int adDirection,
														  double adValue)
{
