...
int GeneralRateModelDG::multiplexInitialConditions(const cadet::ParameterId& pId, unsigned int adDirection,
												   double adValue)
{
	if (_singleBinding)
	{
 ...
 */
void GeneralRateModelDG::consistentInitialState(const SimulationTime& simTime, double* const vecStateY,
												...
												util::ThreadLocalStorage& threadLocalMem)
{
	...
	// initialization for inexact integration DG discretization of particle mass balance not supported. This would
	// require the consideration of the additional algebraic constraints, but the general performance (stiffness due to
	// the additional algebraic constraints) of this scheme does not justify the effort here.
	for (unsigned int type = 0; type < _disc.nParType; type++)
	{
		if (_disc.parExactInt[type] == false)
		{
			LOG(Error)
				<< "No consistent initialization for inexact integration DG discretization in particles (cf. "
				   "par_exact_integration). If consistent initialization is required, change to exact integration.";
			return;
		}
	}
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
...
#endif
		{
			...
			linalg::DenseMatrixView fullJacobianMatrix(
				_globalJacDisc.valuePtr() +
					_globalJacDisc.outerIndexPtr()[idxr.offsetCp(ParticleTypeIndex{type},
																 ParticleIndex{static_cast<unsigned int>(pblk)})],
				nullptr, mask.len, mask.len);

			// z coordinate (column length normed to 1) of current node - needed in externally dependent adsorption
			// kinetic
			...
			for (unsigned int node = 0; node < _disc.nParPoints[type]; ++node)
			{
				...
				const double r = static_cast<double>(_disc.deltaR[type]) * std::floor(node / _disc.nParNode[type]) +
								 0.5 * static_cast<double>(_disc.deltaR[type]) *
									 (1 + _disc.parNodes[type][node % _disc.nParNode[type]]);
				...
				if (localAdY && localAdRes)
				{
					jacFunc = [&](double const* const x, linalg::detail::DenseMatrixBase& mat) {
						...
						//						parts::cell::residualKernel<double, double, double,
						// parts::cell::CellParameters, linalg::DenseBandedRowIterator, true, true>(
						...
						//							localAdRes - localOffsetInParticle, localOffsetInParticle,
						// adJac.adDirOffset, _jacP[type * _disc.nPoints].lowerBandwidth(),
						// _jacP[type * _disc.nPoints].lowerBandwidth(), _jacP[type * _disc.nPoints].upperBandwidth(),
						// fullJacobianMatrix
						//						);
 ...
 */
void GeneralRateModelDG::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY,
														 ...
														 util::ThreadLocalStorage& threadLocalMem)
{
...
#endif
	{
		...
		// Assemble
		linalg::BandedEigenSparseRowIterator jacPar(_globalJacDisc,
													idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}));
		...
		for (unsigned int j = 0; j < _disc.nParPoints[type]; ++j)
		{
			...
			linalg::BandedEigenSparseRowIterator jacSolidOrig(
				_globalJac, idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}) +
								j * static_cast<unsigned int>(idxr.strideParNode(type)) +
			...
			double* const qShellDot = vecStateYdot + idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}) +
									  static_cast<int>(j) * idxr.strideParNode(type) + idxr.strideParLiquid();

			// Obtain derivative of fluxes wrt. time
			std::fill_n(dFluxDt, _disc.strideBound[type], 0.0);
			if (_binding[type]->dependsOnTime())
			{
				// r (particle) coordinate of current node (particle radius normed to 1) - needed in externally
				// dependent adsorption kinetic
				const double r = (static_cast<double>(_disc.deltaR[type]) * std::floor(j / _disc.nParNode[type]) +
								  0.5 * static_cast<double>(_disc.deltaR[type]) *
									  (1 + _disc.parNodes[type][j % _disc.nParNode[type]])) /
				...
				_binding[type]->timeDerivativeQuasiStationaryFluxes(simTime.t, simTime.secIdx,
																	ColumnPosition{z, 0.0, r}, qShellDot - _disc.nComp,
																	qShellDot, dFluxDt, tlmAlloc);
			}
 ...
 * @param [in] errorTol Error tolerance for algebraic equations
 */
void GeneralRateModelDG::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY,
													const AdJacobianParams& adJac, double errorTol,
													util::ThreadLocalStorage& threadLocalMem)
{
...
#endif
			{
				...
				for (std::size_t shell = 0; shell < static_cast<std::size_t>(_disc.nParPoints[type]); ++shell)
				{
					...
					const int localOffsetInParticle =
						static_cast<int>(shell) * idxr.strideParNode(type) + idxr.strideParLiquid();
					double* const qShell = vecStateY + localOffsetToParticle + localOffsetInParticle;
					// r (particle) coordinate of current node
					const double r =
						static_cast<double>(_disc.deltaR[type]) * std::floor(shell / _disc.nParNode[type]) +
						0.5 * static_cast<double>(_disc.deltaR[type]) *
							(1 + _disc.parNodes[type][shell % _disc.nParNode[type]]);
					const ColumnPosition colPos{z, 0.0, r};
 ...
 */
void GeneralRateModelDG::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY,
 ...
 * reaction equilibria). No need to solve for fluxes @f$ j_{f,i} @f$ (since they are not part of the state for DG). Let
 * @f$ \mathcal{I}_a @f$ be the index set of algebraic equations, then, at this point, we have \f[ \left( \frac{\partial
 * F}{\partial y}(t, y_0, \dot{y}_0) s + \frac{\partial F}{\partial p}(t, y_0, \dot{y}_0) \right)_{\mathcal{I}_a} = 0.
 * \f]</li> <li>Compute the time derivatives of the sensitivity @f$ \dot{s} @f$ such that the differential equations
 * hold. However, because of the algebraic equations, we need additional conditions to fully determine
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void GeneralRateModelDG::consistentInitialSensitivity(const SimulationTime& simTime,
	...
	for (std::size_t param = 0; param < vecSensY.size(); ++param)
	{
		...
		for (unsigned int type = 0; type < _disc.nParType; ++type)
		{
...
#endif
			{
				...
				linalg::DenseMatrixView jacobianMatrix(
					_globalJacDisc.valuePtr() +
						_globalJacDisc.outerIndexPtr()[idxr.offsetCp(ParticleTypeIndex{type},
																	 ParticleIndex{static_cast<unsigned int>(pblk)}) -
													   idxr.offsetC()] +
						pblk * probSize * probSize,
					nullptr, probSize, probSize);
				...
				for (unsigned int shell = 0; shell < _disc.nParPoints[type]; ++shell)
				{
					const int jacRowOffset =
						idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{static_cast<unsigned int>(pblk)}) +
						static_cast<int>(shell) * idxr.strideParNode(type) + _disc.nComp;
					...
					std::copy_n(sensY + localQOffset - idxr.strideParLiquid(), idxr.strideParNode(type),
								maskedMultiplier);
					...
					rhsUnmasked_eigen = _globalJac.block(jacRowOffset, jacColOffset, idxr.strideParBlock(type),
														 idxr.strideParBlock(type)) *
										maskedMultiplier_eigen;
...
#endif
		{
			...
			// Assemble
			linalg::BandedEigenSparseRowIterator jacPar(
				_globalJacDisc, idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(type)},
											  ParticleIndex{static_cast<unsigned int>(pblk)}));
			...
			for (unsigned int j = 0; j < _disc.nParPoints[type]; ++j)
			{
				...
				if (_binding[type]->hasQuasiStationaryReactions())
				{
					// Get iterators to beginning of solid phase
					linalg::BandedEigenSparseRowIterator jacSolidOrig(
						_globalJac, idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(type)},
												  ParticleIndex{static_cast<unsigned int>(pblk)}) +
										j * idxr.strideParNode(type) + idxr.strideParLiquid());
					linalg::BandedEigenSparseRowIterator jacSolid = jacPar - idxr.strideParBound(type);
 ...
 * solve algebraic equations). No need to solve for fluxes @f$ j_{f,i} @f$ (since they are not part of the state for
 * DG).</li> <li>Compute the time derivatives of the sensitivity @f$ \dot{s} @f$ such that the differential equations
 * hold. However, because of the algebraic equations, we need additional conditions to fully determine
 *                 @f$ \dot{s}@f$. By differentiating the algebraic equations with respect to time, we get the
 *                 missing linear equations (recall that the sensitivity vector @f$ s @f$ is fixed). The resulting
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void GeneralRateModelDG::leanConsistentInitialSensitivity(const SimulationTime& simTime,
														  const ConstSimulationState& simState,
