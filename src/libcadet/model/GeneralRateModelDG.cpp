...
bool GeneralRateModelDG::configureModelDiscretization(IParameterProvider& paramProvider, const IConfigHelper& helper)
{
	...
	{
		particleConfSuccess = particleConfSuccess && _parDiffOp[parType].configureModelDiscretization(paramProvider, helper, _disc.nComp, parType, _disc.nParType, idxr.strideColComp());
	}
	paramProvider.pushScope("discretization");
	...
	// legacy code for particle reactions
...
unsigned int GeneralRateModelDG::threadLocalMemorySize() const CADET_NOEXCEPT
{
	...
	for (unsigned int i = 0; i < _disc.nParType; ++i)
	{
		if (_parDiffOp[i]._binding && _parDiffOp[i]._binding->requiresWorkspace())
			lms.fitBlock(_parDiffOp[i]._binding->workspaceSize(_disc.nComp, _disc.strideBound[i], _disc.nBound + i * _disc.nComp));

		if (_parDiffOp[i]._dynReaction && _parDiffOp[i]._dynReaction->requiresWorkspace())
			lms.fitBlock(_parDiffOp[i]._dynReaction->workspaceSize(_disc.nComp, _disc.strideBound[i], _disc.nBound + i * _disc.nComp));
	}
...
void GeneralRateModelDG::extractJacobianFromAD(active const* const adRes, unsigned int adDirOffset)
{
	...
	{
		_parDiffOp[parType].calcFilmDiffJacobian(_disc.curSection, idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(parType)}), idxr.offsetC(), _disc.nPoints, _disc.nParType, static_cast<double>(_colPorosity), &_parTypeVolFrac[0], _globalJac, true);
	}
}
...
int GeneralRateModelDG::residualFlux(double t, unsigned int secIdx, StateType const* yBase, double const* yDotBase, ResidualType* resBase)
{
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
		...
		const ParamType epsP = static_cast<ParamType>(_parDiffOp[type]._parPorosity);
		...
		const ParamType surfaceToVolumeRatio = _parDiffOp[type].surfaceToVolumeRatio<ParamType>();
