...
namespace model
{
	...
	{
		model::getAllParameterValues(data, std::vector<IParameterStateDependence*>{_parDiffOp->getParDepSurfDiffusion()}, _parDiffOp->parDepSurfDiffParTypeIndep());
	...
	double GeneralRateParticle::getParameterDouble(const ParameterId& pId) const
	{
		...
		if (model::getParameterDouble(pId, std::vector<IParameterStateDependence*>{_parDiffOp->getParDepSurfDiffusion()}, _parDiffOp->parDepSurfDiffParTypeIndep(), val))
			return val;
		else
	...
	{
		if (model::hasParameter(pId, std::vector<IParameterStateDependence*>{_parDiffOp->getParDepSurfDiffusion()}, _parDiffOp->parDepSurfDiffParTypeIndep()))
			return true;
