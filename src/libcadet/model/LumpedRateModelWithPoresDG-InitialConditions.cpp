...
int LumpedRateModelWithPoresDG::multiplexInitialConditions(const cadet::ParameterId& pId, unsigned int adDirection,
														   double adValue)
{
	if (_singleBinding)
	{
 ...
 */
void LumpedRateModelWithPoresDG::consistentInitialState(const SimulationTime& simTime, double* const vecStateY,
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
		...
		for (unsigned int pblk = 0; pblk < _disc.nPoints; ++pblk)
			...
			linalg::DenseMatrixView jacobianMatrix(static_cast<double*>(jacobianMemBuffer),
												   _globalJacDisc.outerIndexPtr() + pblk * probSize, probSize,
												   probSize);
			...
			if (localAdY && localAdRes) // todo fix AD consistent initialization with req. binding
			{
				jacFunc = [&](double const* const x, linalg::detail::DenseMatrixBase& mat) {
					...
					//								adJac.adRes + idxr.offsetCp(ParticleTypeIndex{ type }), pblk *
					// idxr.strideParBlock(type), adJac.adDirOffset, _jacP[type].lowerBandwidth(),
					//								_jacP[type].lowerBandwidth(), _jacP[type].upperBandwidth(),
					// fullJacobianMatrix
					//							);
					...
					for (unsigned int type = 0; type < _disc.nParType; type++)
					{
						for (unsigned int par = 0; par < _disc.nPoints; par++)
						{
							...
							const int eqOffset_mat =
								idxr.offsetCp(ParticleTypeIndex{type}, ParticleIndex{par}) - idxr.offsetC();
 ...
 */
void LumpedRateModelWithPoresDG::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY,
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
		...
		for (unsigned int pblk = 0; pblk < _disc.nPoints; ++pblk)
		{
			...
			linalg::BandedEigenSparseRowIterator jacPar(
				_globalJacDisc, idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(type)}, ParticleIndex{pblk}) -
									idxr.offsetC());
			...
			linalg::BandedEigenSparseRowIterator jacSolidOrig(
				_globalJac, idxr.offsetCp(ParticleTypeIndex{static_cast<unsigned int>(type)}, ParticleIndex{pblk}) -
								idxr.offsetC() + idxr.strideParLiquid());
 ...
 * @param [in] errorTol Error tolerance for algebraic equations
 */
void LumpedRateModelWithPoresDG::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY,
															const AdJacobianParams& adJac, double errorTol,
															util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
 ...
 */
void LumpedRateModelWithPoresDG::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY,
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void LumpedRateModelWithPoresDG::consistentInitialSensitivity(
	...
	for (std::size_t param = 0; param < vecSensY.size(); ++param)
	{
		...
		for (unsigned int type = 0; type < _disc.nParType; ++type)
		{
			...
			for (unsigned int pblk = 0; pblk < _disc.nPoints; ++pblk)
				...
				rhsUnmasked_eigen =
					_globalJac.block(jacRowOffset, jacColOffset, idxr.strideParBlock(type), idxr.strideParBlock(type)) *
		...
		// Step 2: Compute the correct time derivative of the state vector: Assemble, factorize, and solve diagonal
		// blocks of linear system
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void LumpedRateModelWithPoresDG::leanConsistentInitialSensitivity(
	const SimulationTime& simTime, const ConstSimulationState& simState, std::vector<double*>& vecSensY,
	std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
