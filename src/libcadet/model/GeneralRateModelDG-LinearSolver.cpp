 ...
 */
int GeneralRateModelDG::linearSolve(double t, double alpha, double outerTol, double* const rhs,
...
void GeneralRateModelDG::assembleDiscretizedGlobalJacobian(double alpha, Indexer idxr)
{
	...
	for (unsigned int parType = 0; parType < _disc.nParType; parType++)
	{
		for (unsigned int colNode = 0; colNode < _disc.nPoints; colNode++)
		{
			...
			// Do not add time derivative to particle mass balance equation at inner particle boundary for mass
			// balance(s)
			if (!_disc.parExactInt[parType] && _parGeomSurfToVol[parType] != _disc.SurfVolRatioSlab &&
				_parCoreRadius[parType] == 0.0)
			{
				...
				for (unsigned int comp = 0; comp < _disc.nComp; ++comp)
				{
					for (unsigned int bnd = 0; bnd < _disc.nBound[parType * _disc.nComp + comp]; ++bnd, ++jac)
					{
						...
						if (_binding[parType]->reactionQuasiStationarity()
								[idxr.offsetBoundComp(ParticleTypeIndex{parType}, ComponentIndex{comp}) + bnd])
							continue;

						// surface diffusion + kinetic binding -> additional DG-discretized mass balance equation for
						// solid, for which the (inexact integration) discretization special case also holds
						else if (_hasSurfaceDiffusion[parType] &&
								 static_cast<double>(getSectionDependentSlice(
									 _parSurfDiffusion, _disc.strideBound[_disc.nParType],
									 _disc.curSection)[_disc.nBoundBeforeType[parType] +
													   getOffsetSurfDiff(parType, comp, bnd)]) != 0.0)
							continue;
			...
			{ // else, treat boundary node "normally"
 ...
 */
void GeneralRateModelDG::addTimeDerivativeToJacobianParticleShell(linalg::BandedEigenSparseRowIterator& jac,
																  const Indexer& idxr, double alpha,
																  unsigned int parType)
{
	parts::cell::addTimeDerivativeToJacobianParticleShell<linalg::BandedEigenSparseRowIterator, true>(
