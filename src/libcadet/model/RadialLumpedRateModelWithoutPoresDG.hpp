...
/**
 * @file
 * Defines the radial lumped rate model without pores (RLRM) using a Discontinous Galerkin (DG) scheme.
 */

#ifndef LIBCADET_RADIALLUMPEDRATEMODELWITHOUTPORESDG_HPP_
#define LIBCADET_RADIALLUMPEDRATEMODELWITHOUTPORESDG_HPP_
...
namespace cadet
{
	...
	namespace model
	{
		...
		/**
		* @brief Radial lumped rate model of liquid column chromatography without pores
		* @details Implements the radial transport equation with DG discretization:
		*
		* @f[\begin{align}
				\frac{\partial c_i}{\partial t} + \frac{1 - \varepsilon_t}{\varepsilon_t} \frac{\partial q_{i}}{\partial t} &= - \frac{u}{\rho} \frac{\partial c_i}{\partial \rho} + \frac{1}{\rho} \frac{\partial}{\partial \rho}\left( D_{\rho,i} \rho \frac{\partial c_i}{\partial \rho} \right) \\
		...
		* @f[ \begin{align}
			...
			\end{align} @f]
		*/
		class RadialLumpedRateModelWithoutPoresDG : public UnitOperationBase
		{
		public:

			RadialLumpedRateModelWithoutPoresDG(UnitOpIdx unitOpIdx);
			virtual ~RadialLumpedRateModelWithoutPoresDG() CADET_NOEXCEPT;
			...
			static const char* identifier() { return "RADIAL_LUMPED_RATE_MODEL_WITHOUT_PORES_DG"; }
		...
		protected:
			...
			public:
				...
				Exporter(const Discretization& disc, const RadialLumpedRateModelWithoutPoresDG& model, double const* data) : _disc(disc), _idx(disc), _model(model), _data(data) { }
				Exporter(const Discretization&& disc, const RadialLumpedRateModelWithoutPoresDG& model, double const* data) = delete;
			...
			protected:
				const Discretization& _disc;
				const Indexer _idx;
				const RadialLumpedRateModelWithoutPoresDG& _model;
...
#endif  // LIBCADET_RADIALLUMPEDRATEMODELWITHOUTPORESDG_HPP_
