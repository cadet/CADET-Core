...
bool GeneralRateModel2D::configureModelDiscretization(IParameterProvider& paramProvider, const IConfigHelper& helper)
{
	...
	const bool transportSuccess = _convDispOp.configureModelDiscretization(paramProvider, helper, _disc.nComp,
																		   _disc.nCol, _disc.nRad, _dynReactionBulk);
...
int GeneralRateModel2D::jacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res,
...
void registerGeneralRateModel2D(
	std::unordered_map<std::string, std::function<IUnitOperation*(UnitOpIdx, IParameterProvider&)>>& models)
{
	models[GeneralRateModel2D::identifier()] = [](UnitOpIdx uoId, IParameterProvider&) {
