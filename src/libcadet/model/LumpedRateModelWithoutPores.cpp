...
bool LumpedRateModelWithoutPores<ConvDispOperator>::configure(IParameterProvider& paramProvider)
{
	...
	if (paramProvider.exists("liquid_reaction_000"))
...
#endif
	{
		...
		if (_reaction.getDynReactionVector("cross_phase").size() > 0 || _reaction.getDynReactionVector("solid").size() > 0 || _reaction.getDynReactionVector("liquid").size() > 0)
		{
				...
				if (wantRes)
				{
					...
					for (unsigned int comp = 0; comp < _disc.nComp; ++comp)
					{
						...
						for (unsigned int bnd = 0; bnd < nBound[comp]; ++bnd, ++idx)
						{
							...
							localRes[-static_cast<int>(_disc.nComp) + static_cast<int>(comp)] += static_cast<typename DoubleActiveDemoter<ParamType, ResidualType>::type>(invBetaP)* fluxSolid[idx];
			...
			for (unsigned int reac = 0; reac < _reaction.getnReactionOfParType("liquid", 0); ++reac)
			{
				...
				if (wantRes)
				{
					_reaction.getDynReactionVector("liquid")[reac]->residualFluxAdd(t, secIdx, colPos, _disc.nComp, localY - _disc.nComp, localRes - _disc.nComp, -1.0, buffer);

					if (wantJac)
					{
						// static_cast should be sufficient here, but this statement is also analyzed when wantJac = false
						_reaction.getDynReactionVector("liquid")[reac]->analyticJacobianAdd(t, secIdx, colPos, _disc.nComp, reinterpret_cast<double const*>(localY - _disc.nComp), -1.0, jacBase, buffer);
			...
			for (unsigned int reac = 0; reac < _reaction.getnReactionOfParType("solid", 0); ++reac)
			{
				...
				if (wantRes)
				{
					...
					std::fill_n(static_cast<ResidualType*>(fluxSolid), nTotalBound, 0.0);
					_reaction.getDynReactionVector("solid")[reac]->residualFluxAdd(t, secIdx, colPos, _disc.strideBound, localY, static_cast<ResidualType*>(fluxSolid), -1.0, buffer);
					...
					if (wantJac)
					{
						...
						// static_cast should be sufficient here, but this statement is also analyzed when wantJac = false
						_reaction.getDynReactionVector("solid")[reac]->analyticJacobianAdd(t, secIdx, colPos, _disc.strideBound, reinterpret_cast<double const*>(localY), -1.0, dmv.row(0, _disc.nComp), buffer);
