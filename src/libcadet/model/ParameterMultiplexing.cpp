...
/* For the new particle type interface */
...
MultiplexMode newIF_readAndRegisterSingleTypeMultiplexBndCompTypeSecParam(IParameterProvider& paramProvider, std::unordered_map<ParameterId, active*>& parameters, std::vector<active>& values, const std::string& name, unsigned int nTotalBound, unsigned int nComp, unsigned int strideBound, unsigned int const* nBound, unsigned int const* nBoundBeforeType, const unsigned int parTypeIdx, UnitOpIdx uoi)
{
	if (nTotalBound == 0)
		return MultiplexMode::Component;
