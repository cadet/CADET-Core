...
int LumpedRateModelWithPores<ConvDispOperator>::residualBulk(double t, unsigned int secIdx, StateType const* yBase, double const* yDotBase, ResidualType* resBase, util::ThreadLocalStorage& threadLocalMem)
{
	...
	for (unsigned int col = 0; col < _disc.nCol; ++col, y += idxr.strideColCell(), res += idxr.strideColCell())
	{
		...
		for (auto i = 0; i < _reaction.getDynReactionVector("bulk").size(); i++)
		{
			...
			if (wantRes)
				_reaction.getDynReactionVector("bulk")[i]->residualFluxAdd(t, secIdx, colPos, _disc.nComp, y, res, -1.0, tlmAlloc);

			if (wantJac)
			{
				// static_cast should be sufficient here, but this statement is also analyzed when wantJac = false
				_reaction.getDynReactionVector("bulk")[i]->analyticJacobianAdd(t, secIdx, colPos, _disc.nComp, reinterpret_cast<double const*>(y), -1.0, _convDispOp.jacobian().row(col * idxr.strideColCell()), tlmAlloc);
			}
		}
	}
	...
	if (numReacCrossPhase > 0 || numReacParticle > 0 || numReacSolid > 0)
	{
		...
		for (unsigned int reac = 0; reac < numReacSolid; ++reac)
		{
			...
			if (wantRes)
			{
				...
				std::fill_n(static_cast<ResidualType*>(fluxSolid), nTotalBound, 0.0);
				_reaction.getDynReactionVector("solid")[offSetCrossPhase + reac]->residualFluxAdd(t, secIdx, colPos, _disc.nBoundBeforeType[parType], y + _disc.nComp, static_cast<ResidualType*>(fluxSolid), -1.0, buffer);
			...
			if (wantJac)
			{
				...
				// static_cast should be sufficient here, but this statement is also analyzed when wantJac = false
				_reaction.getDynReactionVector("solid")[offSetSolid + reac]->analyticJacobianAdd(t, secIdx, colPos, _disc.nBoundBeforeType[parType], reinterpret_cast<double const*>(y + _disc.nComp), -1.0, dmv.row(0, _disc.nComp), buffer);
