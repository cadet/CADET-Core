...
bool GeneralRateModel<ConvDispOperator>::configure(IParameterProvider& paramProvider)
{
	...
	for (int parType = 0; parType < _disc.nParType; parType++)
	{
		...
		bool parDiffParTypeDep = paramProvider.exists("PAR_DIFFUSION_PARTYPE_DEPENDENT") ? paramProvider.getInt("PAR_DIFFUSION_PARTYPE_DEPENDENT") : true;
		if (parType > 0)
			tmpMode = _parDiffusionMode;
		_parDiffusionMode = readAndRegisterMultiplexCompSecParam(paramProvider, _parameters, _parDiffusion, "PAR_DIFFUSION", _disc.nComp, parType, parDiffParTypeDep, _unitOpIdx, _parDiffusion.size());
		...
		if (paramProvider.exists("PAR_SURFDIFFUSION"))
		{
			bool surfDiffParTypeDep = paramProvider.exists("PAR_SURFDIFFUSION_PARTYPE_DEPENDENT") ? paramProvider.getInt("PAR_SURFDIFFUSION_PARTYPE_DEPENDENT") : true;
			if (parType > 0)
				tmpMode = _parSurfDiffusionMode;
			_parSurfDiffusionMode = readAndRegisterMultiplexBndCompSecParam(paramProvider, _parameters, _parSurfDiffusion, "PAR_SURFDIFFUSION", _disc.nComp, _disc.strideBound[parType], _disc.nBound + parType * _disc.nComp, parType, surfDiffParTypeDep, _unitOpIdx, _parSurfDiffusion.size());
	...
	double const* yDotBase, ResidualType* resBase, util::ThreadLocalStorage& threadLocalMem)
{
	...
	active const* const parSurfDiff = getSectionDependentSlice(_parSurfDiffusion, _disc.nBoundBeforeType[parType] * nSec, 1) + (nSec > 1 ? secIdx * _disc.strideBound[parType] : 0);
	...
	if (pId.unitOperation == _unitOpIdx)
	{
		for (int parType = 0; parType < _disc.nParType; parType++)
		{
			...
			if (singleTypeMultiplexCompTypeSecParameterValue(pId, hashString("FILM_DIFFUSION"), _filmDiffusionMode, _filmDiffusion, _disc.nComp, parType, value, nullptr, (_filmDiffusion.size() / _disc.nParType) * parType))
				return true;
			if (singleTypeMultiplexCompTypeSecParameterValue(pId, hashString("PAR_DIFFUSION"), _parDiffusionMode, _parDiffusion, _disc.nComp, parType, value, nullptr, (_parDiffusion.size() / _disc.nParType) * parType))
				return true;
			if (singleTypeMultiplexBndCompTypeSecParameterValue(pId, hashString("PAR_SURFDIFFUSION"), _parSurfDiffusionMode, _parSurfDiffusion, _disc.nComp, _disc.strideBound[parType], _disc.boundOffset + parType * _disc.nComp, parType, value, nullptr, (_parSurfDiffusion.size() / _disc.nParType) * parType))
				return true;
	...
	if (pId.unitOperation == _unitOpIdx)
	{
		for (int parType = 0; parType < _disc.nParType; parType++)
		{
			...
			if (singleTypeMultiplexCompTypeSecParameterValue(pId, hashString("FILM_DIFFUSION"), _filmDiffusionMode, _filmDiffusion, _disc.nComp, parType, value, &_sensParams, (_filmDiffusion.size() / _disc.nParType) * parType))
				return;
			if (singleTypeMultiplexCompTypeSecParameterValue(pId, hashString("PAR_DIFFUSION"), _parDiffusionMode, _parDiffusion, _disc.nComp, parType, value, &_sensParams, (_parDiffusion.size() / _disc.nParType) * parType))
				return;
			if (singleTypeMultiplexBndCompTypeSecParameterValue(pId, hashString("PAR_SURFDIFFUSION"), _parSurfDiffusionMode, _parSurfDiffusion, _disc.nComp, _disc.strideBound[parType], _disc.boundOffset + parType * _disc.nComp, parType, value, &_sensParams, (_parSurfDiffusion.size() / _disc.nParType) * parType))
				return;
...
bool GeneralRateModel<ConvDispOperator>::setSensitiveParameter(const ParameterId& pId, unsigned int adDirection, double adValue)
{
	if (pId.unitOperation == _unitOpIdx)
	{
		for (int parType = 0; parType < _disc.nParType; parType++)
		{
			...
			if (singleTypeMultiplexCompTypeSecParameterAD(pId, hashString("PAR_DIFFUSION"), _parDiffusionMode, _parDiffusion, _disc.nComp, parType, adDirection, adValue, _sensParams, (_parDiffusion.size() / _disc.nParType) * parType))
			{
				LOG(Debug) << "Found parameter " << pId << ": Dir " << adDirection << " is set to " << adValue;
				return true;
			}

			if (singleTypeMultiplexBndCompTypeSecParameterAD(pId, hashString("PAR_SURFDIFFUSION"), _parSurfDiffusionMode, _parSurfDiffusion, _disc.nComp, _disc.strideBound[parType], _disc.boundOffset + parType * _disc.nComp, parType, adDirection, adValue, _sensParams, (_parSurfDiffusion.size() / _disc.nParType) * parType))
			{
				LOG(Debug) << "Found parameter " << pId << ": Dir " << adDirection << " is set to " << adValue;
				return true;
			}
