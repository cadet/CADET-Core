...
template <typename T> struct RadialFlowParameters
{
	T u;
	active const* d_rad;
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac,
		  bool wantRes = true>
int residualForwardsRadialFlow(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res,
							   RowIteratorType jacBegin, const RadialFlowParameters<ParamType>& p)
{
	...
	//		StencilType stencil(std::max(p.weno->stencilSize(), 3u), *p.stencilMemory, std::max(p.weno->order() - 1,
	// 1));
	...
	for (unsigned int comp = 0; comp < p.nComp; ++comp)
	{
		...
		for (unsigned int col = 0; col < p.nCol; ++col)
		{
			const ParamType denom =
				static_cast<ParamType>(p.cellCenters[col]) * static_cast<ParamType>(p.cellSizes[col]);
			...
			{
				const double relCoord =
					(static_cast<double>(p.cellBounds[col + 1]) - static_cast<double>(p.cellBounds[0])) /
					(static_cast<double>(p.cellBounds[p.nCol - 1]) - static_cast<double>(p.cellBounds[0]));
				const ParamType d_rad_right =
					d_rad *
					p.parDep->getValue(p.model, ColumnPosition{relCoord, 0.0, 0.0}, comp, ParTypeIndep, BoundStateIndep,
									   static_cast<ParamType>(p.u) / static_cast<ParamType>(p.cellBounds[col + 1]));
				if (wantRes)
					resBulkComp[col * p.strideCell] -=
						d_rad_right * static_cast<ParamType>(p.cellBounds[col + 1]) / denom *
						(stencil[1] - stencil[0]) /
						(static_cast<ParamType>(p.cellCenters[col + 1]) - static_cast<ParamType>(p.cellCenters[col]));
				// Jacobian entries
				if (wantJac)
				{
					const double val =
						static_cast<double>(d_rad_right) * static_cast<double>(p.cellBounds[col + 1]) /
						static_cast<double>(denom) /
						(static_cast<double>(p.cellCenters[col + 1]) - static_cast<double>(p.cellCenters[col]));
					jac[0] += val;
			...
			{
				const double relCoord =
					(static_cast<double>(p.cellBounds[col]) - static_cast<double>(p.cellBounds[0])) /
					(static_cast<double>(p.cellBounds[p.nCol - 1]) - static_cast<double>(p.cellBounds[0]));
				const ParamType d_rad_left =
					d_rad * p.parDep->getValue(p.model, ColumnPosition{relCoord, 0.0, 0.0}, comp, ParTypeIndep,
											   BoundStateIndep,
											   static_cast<ParamType>(p.u) / static_cast<ParamType>(p.cellBounds[col]));
				if (wantRes)
					resBulkComp[col * p.strideCell] -=
						d_rad_left * static_cast<ParamType>(p.cellBounds[col]) / denom * (stencil[-1] - stencil[0]) /
						(static_cast<ParamType>(p.cellCenters[col]) - static_cast<ParamType>(p.cellCenters[col - 1]));
				// Jacobian entries
				if (wantJac)
				{
					const double val =
						static_cast<double>(d_rad_left) * static_cast<double>(p.cellBounds[col]) /
						static_cast<double>(denom) /
						(static_cast<double>(p.cellCenters[col]) - static_cast<double>(p.cellCenters[col - 1]));
					jac[0] += val;
					jac[-p.strideCell] -= val;
				}
			}
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac,
		  bool wantRes = true>
int residualBackwardsRadialFlow(const SimulationTime& simTime, StateType const* y, double const* yDot,
								ResidualType* res, RowIteratorType jacBegin, const RadialFlowParameters<ParamType>& p)
{
	// The stencil caches parts of the state vector for better spatial coherence
	typedef CachingStencil<StateType, ArrayPool> StencilType;
	//		StencilType stencil(std::max(p.weno->stencilSize(), 3u), *p.stencilMemory, std::max(p.weno->order() - 1,
	...
	for (unsigned int comp = 0; comp < p.nComp; ++comp)
	{
		...
		for (unsigned int col = p.nCol - 1; col < p.nCol; --col)
		{
			...
			if (wantJac)
			{
				wenoOrder = 1;
				vm = stencil[0];
				//					wenoOrder = p.weno->template reconstruct<StateType, StencilType>(p.wenoEpsilon, col,
				// p.nCol, stencil, vm, p.wenoDerivatives);
			}
			else
			{
				wenoOrder = 1;
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac,
		  bool wantRes = true>
int residualKernelRadial(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res,
						 RowIteratorType jacBegin, const RadialFlowParameters<ParamType>& p)
{
	if (p.u >= 0.0)
		return impl::residualForwardsRadialFlow<StateType, ResidualType, ParamType, RowIteratorType, wantJac, wantRes>(
			simTime, y, yDot, res, jacBegin, p);
	else
		return impl::residualBackwardsRadialFlow<StateType, ResidualType, ParamType, RowIteratorType, wantJac, wantRes>(
			simTime, y, yDot, res, jacBegin, p);
}
