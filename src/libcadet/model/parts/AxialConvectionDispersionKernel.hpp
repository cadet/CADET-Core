...
// the impl is very different between eq and non eq grids. this struct is used for non-equidistant grids, where h is not constant
template <typename T, typename Reconstruction_t>
struct AxialFlowParametersNonEq
{
	T u;
	active const* d_ax;
	double* reconstructionDerivatives; //!< Holds derivatives of the reconstruction scheme
	...
	IParameterParameterDependence* parDep;
	const IModel& model;
	std::vector<active> cellFaces; //!< Instead of having nCol and h, we use positions of the cell faces (non-equidistant grid), defined correctly?
};
...
namespace impl
{
	...
	// for non-equidistant grids, we need to compute h for each cell separately inside the loop, contents to be updated. 
	template <typename StateType, typename ResidualType, typename ParamType, typename ReconstrType, typename RowIteratorType, bool wantJac, bool wantRes = true>
	int residualForwardsAxialFlowNonEq(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res, RowIteratorType jacBegin, const AxialFlowParameters<ParamType, ReconstrType>& p)
	{
		// h is the size of the cell
		const ParamType h2 = p.h * p.h;
...
template <typename StateType, typename ResidualType, typename ParamType, typename ReconstrType, typename RowIteratorType, bool wantJac, bool wantRes = true>
int residualKernelAxialNonEq(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res, RowIteratorType jacBegin, const AxialFlowParameters<ParamType, ReconstrType>& p)
{
	if (p.u >= 0.0)
		return impl::residualForwardsAxialFlowNonEq<StateType, ResidualType, ParamType, ReconstrType, RowIteratorType, wantJac, wantRes>(simTime, y, yDot, res, jacBegin, p);
	else
		// To update
		return impl::residualBackwardsAxialFlow<StateType, ResidualType, ParamType, ReconstrType, RowIteratorType, wantJac, wantRes>(simTime, y, yDot, res, jacBegin, p);
}
