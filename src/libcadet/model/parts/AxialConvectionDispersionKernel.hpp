...
template <typename T> struct AxialFlowParameters
{
	T u;
	active const* d_ax;
	T h;
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac,
		  bool wantRes = true>
int residualForwardsAxialFlow(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res,
							  RowIteratorType jacBegin, const AxialFlowParameters<ParamType>& p)
{
	...
	for (unsigned int comp = 0; comp < p.nComp; ++comp)
	{
		...
		for (unsigned int col = 0; col < p.nCol; ++col)
		{
			...
			if (cadet_likely(col < p.nCol - 1))
			{
				...
				const ParamType d_ax_right =
					d_ax * p.parDep->getValue(p.model, ColumnPosition{relCoord, 0.0, 0.0}, comp, ParTypeIndep,
											  BoundStateIndep, static_cast<ParamType>(p.u));
				if (wantRes)
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac,
		  bool wantRes = true>
int residualBackwardsAxialFlow(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res,
							   RowIteratorType jacBegin, const AxialFlowParameters<ParamType>& p)
{
	const ParamType h2 = p.h * p.h;
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac,
		  bool wantRes = true>
int residualKernelAxial(const SimulationTime& simTime, StateType const* y, double const* yDot, ResidualType* res,
						RowIteratorType jacBegin, const AxialFlowParameters<ParamType>& p)
{
	if (p.u >= 0.0)
		return impl::residualForwardsAxialFlow<StateType, ResidualType, ParamType, RowIteratorType, wantJac, wantRes>(
			simTime, y, yDot, res, jacBegin, p);
	else
		return impl::residualBackwardsAxialFlow<StateType, ResidualType, ParamType, RowIteratorType, wantJac, wantRes>(
			simTime, y, yDot, res, jacBegin, p);
}

void sparsityPatternAxial(linalg::SparsityPatternRowIterator itBegin, unsigned int nComp, unsigned int nCol,
