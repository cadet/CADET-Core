...
\end{align} @f]
 * Methods are described in @cite todo, and @cite Puttmann2013, @cite Puttmann2016 (forward sensitivities, AD, band
compression)
 *
...
public:
	...
	bool configureModelDiscretization(IParameterProvider& paramProvider, const IConfigHelper& helper,
									  unsigned int nComp, int polynomial_integration_mode, unsigned int nCells,
									  unsigned int polyDeg, unsigned int strideNode);
	...
	int residual(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, double* res,
				 Eigen::SparseMatrix<double, Eigen::RowMajor>& jac);
	int residual(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, active* res,
				 Eigen::SparseMatrix<double, Eigen::RowMajor>& jac);
	int residual(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, double* res,
				 WithoutParamSensitivity);
	int residual(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, active* res,
				 WithParamSensitivity);
	int residual(const IModel& model, double t, unsigned int secIdx, active const* y, double const* yDot, active* res,
				 WithParamSensitivity);
	int residual(const IModel& model, double t, unsigned int secIdx, active const* y, double const* yDot, active* res,
				 WithoutParamSensitivity);

	int calcStaticAnaJacobian(Eigen::SparseMatrix<double, Eigen::RowMajor>& jacobian, Eigen::MatrixXd& jacInlet,
							  const int bulkOffset = 0);
	...
	inline const active& currentVelocity(double pos) const CADET_NOEXCEPT
	{
		return _curVelocity;
	}
	inline bool forwardFlow() const CADET_NOEXCEPT
	{
		return _curVelocity >= 0.0;
	}

	inline double cellLeftBound(unsigned int idx) const CADET_NOEXCEPT
	{
	...
	double relativeCoordinate(unsigned int idx) const CADET_NOEXCEPT
	{
		...
		return (floor(idx / _nNodes) * static_cast<double>(_deltaZ) +
				0.5 * static_cast<double>(_deltaZ) * (1.0 + _nodes[idx % _nNodes])) /
			   static_cast<double>(_colLength);
	}

	inline const double* LGLnodes() const CADET_NOEXCEPT
	{
		return &_nodes[0];
	}
	...
	inline const active* currentDispersion(const int secIdx) const CADET_NOEXCEPT
	{
		return getSectionDependentSlice(_colDispersion, _nComp, secIdx);
	}
	inline const bool dispersionCompIndep() const CADET_NOEXCEPT
	{
		return _dispersionCompIndep;
	}
	...
	inline unsigned int nCells() const CADET_NOEXCEPT
	{
		return _nCells;
	}
	inline unsigned int nNodes() const CADET_NOEXCEPT
	{
		return _nNodes;
	}
	inline unsigned int nPoints() const CADET_NOEXCEPT
	{
		return _nPoints;
	}
	inline bool exactInt() const CADET_NOEXCEPT
	{
		return _exactInt;
	}
	...
	{
		return static_cast<int>(_strideNode);
	}
	...
	// @todo use more efficient seed vectors. currently, we treat the jacobian as banded, but the pattern is actually
	// more sparse when multiple components are considered (note that active type directions are limited) We have
	// different jacobian structure for exact integration and collocation DG scheme, i.e. we need different seed vectors
	// collocation DG: 2 * N_n * (strideNode) + 1 = total bandwidth (main diagonal entries maximally depend on the next
	// and last N_n liquid phase entries of same component)
	//    ex. int. DG: 4 * N_n * (strideNode) + 1 = total bandwidth (main diagonal entries maximally depend on the next
	//    and last 2*N_n liquid phase entries of same component)
	int requiredADdirs() const CADET_NOEXCEPT
	{
		return (_exactInt) ? 4 * _nNodes * strideColNode() + 1 : 2 * _nNodes * strideColNode() + 1;
	}
...
protected:
	...
	int residualImpl(const IModel& model, double t, unsigned int secIdx, StateType const* y, double const* yDot,
					 ResidualType* res, RowIteratorType jacBegin);
	...
	Eigen::MatrixXd DGjacobianConvBlock()
	{
		...
		// Convection block [ d RHS_conv / d c ], additionally depends on upwind flux part from corresponding neighbour
		// cell
		Eigen::MatrixXd convBlock = Eigen::MatrixXd::Zero(_nNodes, _nNodes + 1);

		if (_curVelocity >= 0.0)
		{ // forward flow -> Convection block additionally depends on last entry of previous cell
		...
		{ // backward flow -> Convection block additionally depends on first entry of subsequent cell
	...
	Eigen::MatrixXd getGBlock(unsigned int cellIdx)
	{
		...
		else
		{
			...
			if (cellIdx == 1)
			{
				...
				if (cellIdx == _nCells)
				{ // adjust for special case one cell
					gBlock(0, 0) += 0.5 * _invWeights[0] * 2 / static_cast<double>(_deltaZ);
					gBlock(_nNodes - 1, _nNodes + 1) -=
						0.5 * _invWeights[_nNodes - 1] * 2 / static_cast<double>(_deltaZ);
	 ...
	 */
	Eigen::MatrixXd auxBlockGstar(unsigned int cellIdx, Eigen::MatrixXd leftG, Eigen::MatrixXd middleG,
								  Eigen::MatrixXd rightG)
	{
		...
		// NOTE: N = polyDeg
		// indices  gStarDC    :     0   ,   1   , ..., _nNodes; _nNodes+1, ..., 2 * _nNodes;	2*_nNodes+1, ..., 3 *
		// _nNodes; 3*_nNodes+1 derivative index j  : -(N+1)-1, -(N+1),... ,  -1   ;   0     , ...,		N	 ;	  N + 1
		// , ..., 2N + 2    ; 2(N+1) +1 auxiliary block [d g^* / d c]
		if (cellIdx != 1)
		{
	...
	Eigen::MatrixXd DGjacobianDispBlock(unsigned int cellIdx)
	{
		...
		if (_exactInt)
		{
			...
			Eigen::MatrixXd gStarDC =
				auxBlockGstar(cellIdx, getGBlock(cellIdx - 1), gBlock, getGBlock(cellIdx + 1)); // Numerical flux block

			//  indices  dispBlock :   0	 ,   1   , ..., _nNodes;	_nNodes+1, ..., 2 * _nNodes;	2*_nNodes+1,
			//  ..., 3 * _nNodes; 3*_nNodes+1
			//	derivative index j  : -(N+1)-1, -(N+1),...,	 -1	  ;   0     , ...,		N	 ;	  N + 1	  , ..., 2N + 2
			//; 2(N+1) +1
		...
		{ // inexact integration collocation DGSEM
			...
			// NOTE: N = polyDeg
			// cell indices :  0  , ..., _nNodes - 1;	_nNodes, ..., 2 * _nNodes - 1;	2 * _nNodes, ..., 3 * _nNodes -
			// 1
			...
			if (cellIdx > 1)
			{
				dispBlock(0, _nNodes - 1) +=
					-_invWeights[0] *
					(-0.5 * GBlock(0, 0) + 0.5 * GBlockLeft(_nNodes - 1, _nNodes)); // G_N,N		i=0, j=-1
				dispBlock(0, _nNodes) +=
					-_invWeights[0] *
					(-0.5 * GBlock(0, 1) + 0.5 * GBlockLeft(_nNodes - 1, _nNodes + 1)); // G_N,N+1	i=0, j=0
				dispBlock.block(0, _nNodes + 1, 1, _nNodes) +=
					-_invWeights[0] * (-0.5 * GBlock.block(0, 2, 1, _nNodes)); // G_i,j		i=0, j=1,...,N+1
				dispBlock.block(0, 0, 1, _nNodes - 1) +=
					-_invWeights[0] *
					(0.5 * GBlockLeft.block(_nNodes - 1, 1, 1, _nNodes - 1)); // G_N,j+N+1		i=0, j=-N-1,...,-2
			}
			else if (cellIdx == 1)
			{ // left boundary cell
				dispBlock.block(0, _nNodes - 1, 1, _nNodes + 2) +=
					-_invWeights[0] * (-GBlock.block(0, 0, 1, _nNodes + 2)); // G_N,N		i=0, j=-1,...,N+1
			}
			if (cellIdx < _nCells)
			{
				dispBlock.block(_nNodes - 1, _nNodes - 1, 1, _nNodes) +=
					_invWeights[_nNodes - 1] *
					(-0.5 * GBlock.block(_nNodes - 1, 0, 1, _nNodes)); // G_i,j+N+1		i=N, j=-1,...,N-1
				dispBlock(_nNodes - 1, 2 * _nNodes - 1) +=
					_invWeights[_nNodes - 1] *
					(-0.5 * GBlock(_nNodes - 1, _nNodes) + 0.5 * GBlockRight(0, 0)); // G_i,j		i=N, j=N
				dispBlock(_nNodes - 1, 2 * _nNodes) +=
					_invWeights[_nNodes - 1] *
					(-0.5 * GBlock(_nNodes - 1, _nNodes + 1) + 0.5 * GBlockRight(0, 1)); // G_i,j		i=N, j=N+1
				dispBlock.block(_nNodes - 1, 2 * _nNodes + 1, 1, _nNodes - 1) +=
					_invWeights[_nNodes - 1] *
					(0.5 * GBlockRight.block(0, 2, 1, _nNodes - 1)); // G_0,j-N-1		i=N, j=N+2,...,2N+1
			}
			else if (cellIdx == _nCells)
			{ // right boundary cell
				dispBlock.block(_nNodes - 1, _nNodes - 1, 1, _nNodes + 2) +=
					_invWeights[_nNodes - 1] *
					(-GBlock.block(_nNodes - 1, 0, 1, _nNodes + 2)); // G_i,j+N+1		i=N, j=--1,...,N+1
			}
	 ...
	 * @detail performs matrix-vector multiplication optimized depending on state types and stores the result in
	 * stateDer.
	 */
	template <typename StateType, typename ResidualType>
	void volumeIntegral(Eigen::Map<const Vector<StateType, Dynamic>, 0, InnerStride<Dynamic>>& state,
						Eigen::Map<Vector<ResidualType, Dynamic>, 0, InnerStride<Dynamic>>& stateDer)
	{
		...
		for (unsigned int Cell = 0; Cell < _nCells; Cell++)
		{
			...
			if constexpr (std::is_same_v<StateType, double>)
			{
				...
				{
					stateDer.segment(Cell * _nNodes, _nNodes) -=
						(_polyDerM * state.segment(Cell * _nNodes, _nNodes)).template cast<ResidualType>();
				}
			}
			else
			{ // both active types
				...
				stateDer.segment(Cell * _nNodes, _nNodes) -=
					_polyDerM.template cast<active>() * state.segment(Cell * _nNodes, _nNodes);
			}
		}
	}
	template <typename StateType, typename ResidualType>
	void volumeIntegral(Eigen::Map<Vector<StateType, Dynamic>, 0, InnerStride<Dynamic>>& state,
						Eigen::Map<Vector<ResidualType, Dynamic>, 0, InnerStride<Dynamic>>& stateDer)
	{
		Eigen::Map<const Vector<StateType, Dynamic>, 0, InnerStride<Dynamic>> state_const(state.data(), state.size(),
																						  InnerStride<Dynamic>(1));
		volumeIntegral(state_const, stateDer);
	}
	/*
	 * @brief calculates the interface fluxes h* of Convection Dispersion equation
	 */
	template <typename StateType, typename ParamType> void InterfaceFlux(const StateType* C, ParamType _dispersion)
	{
		...
		{ // forward flow (upwind num. flux)
			...
			_surfaceFlux[0] = _curVelocity * _boundary[0];

			// outlet (right) boundary interface
			_surfaceFlux[_nCells] = _curVelocity * (C[_nCells * strideCell - strideNode]) -
									0.5 * (-2.0 / static_cast<ParamType>(_deltaZ)) * _dispersion *
										(g[_nCells * strideCell_g - strideNode_g] // last cell last node
										 + _boundary[3]);                         // right boundary value g
		}
		else
		{ // backward flow (upwind num. flux)
			// calculate inner interface fluxes
			for (unsigned int Cell = 1; Cell < _nCells; Cell++)
			{
				// h* = h*_conv + h*_disp
			...
			_surfaceFlux[_nCells] = _curVelocity * _boundary[0];

			// outlet boundary interface
			_surfaceFlux[0] = _curVelocity * (C[0]) - 0.5 * (-2.0 / static_cast<ParamType>(_deltaZ)) * _dispersion *
	 ...
	 */
	template <typename StateType, typename ResidualType>
	void surfaceIntegral(const StateType* state, ResidualType* stateDer, unsigned int strideNode_state,
						 unsigned int strideCell_state, unsigned int strideNode_stateDer,
						 unsigned int strideCell_stateDer)
	{

		if (_exactInt)
		{ // non-collocated integration -> dense mass matrix
			for (unsigned int Cell = 0; Cell < _nCells; Cell++)
			{
				// strong surface integral -> M^-1 B [state - state*]
				for (unsigned int Node = 0; Node < _nNodes; Node++)
				{
					stateDer[Cell * strideCell_stateDer + Node * strideNode_stateDer] -= static_cast<ResidualType>(
						_invMM(Node, 0) * (state[Cell * strideCell_state] - _surfaceFlux[Cell]) -
						_invMM(Node, _polyDeg) *
		...
		{ // collocated numerical integration -> diagonal mass matrix
			for (unsigned int Cell = 0; Cell < _nCells; Cell++)
			{
				...
				stateDer[Cell * strideCell_stateDer] // first cell, node
					-=
					static_cast<ResidualType>(_invWeights[0] * (state[Cell * strideCell_state] - _surfaceFlux(Cell)));

				stateDer[Cell * strideCell_stateDer + _polyDeg * strideNode_stateDer] // last cell, node
					+= static_cast<ResidualType>(
						_invWeights[_polyDeg] *
						(state[Cell * strideCell_state + _polyDeg * strideNode_state] - _surfaceFlux(Cell + 1)));
			}
		}
	}
	 ...
	 * @detail to implement Danckwert boundary conditions, we only need to set the solid wall BC values for auxiliary
	 * variable
	 */
	template <typename StateType> void calcBoundaryValues()
	{
		...
		_boundary[2] =
			-reinterpret_cast<StateType*>(_auxState)[0]; // g_l left boundary (inlet/outlet for forward/backward flow)
		_boundary[3] = -reinterpret_cast<StateType*>(
			_auxState)[_nPoints - 1]; // g_r right boundary (outlet/inlet for forward/backward flow)
	}
	...
	int ConvDispNodalPattern(std::vector<T>& tripletList, const int offC = 0)
	{
			...
			for (unsigned int cell = 1; cell < _nCells; cell++)
			{
				for (unsigned int comp = 0; comp < _nComp; comp++)
				{
					for (unsigned int i = 0; i < _nNodes; i++)
					{
						...
						{
							// row: jump over inlet DOFs and previous cells, add component offset and go node strides
							// from there for each convection block entry col: jump over inlet DOFs and previous cells,
							// go back one node, add component offset and go node strides from there for each convection
							// block entry
							tripletList.push_back(
								T(offC + cell * strideColCell() + comp * strideColComp() + i * strideColNode(),
								  offC + cell * strideColCell() - strideColNode() + comp * strideColComp() +
									  j * strideColNode(),
								  0.0));
						}
					}
		...
		{ // backward flow upwind entry -> first node of subsequent cell
			...
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int i = 0; i < _nNodes; i++)
				{
					...
					{
						tripletList.push_back(
							T(offC + (_nCells - 1) * strideColCell() + comp * strideColComp() + i * strideColNode(),
				...
				for (unsigned int comp = 0; comp < _nComp; comp++)
				{
					for (unsigned int i = 0; i < _nNodes; i++)
					{
						for (unsigned int j = 0; j < _nNodes + 1; j++)
						{
							// row: jump over inlet DOFs and previous cells, add component offset and go node strides
							// from there for each convection block entry col: jump over inlet DOFs and previous cells,
							// add component offset and go node strides from there for each convection block entry
							tripletList.push_back(
								T(offC + cell * strideColCell() + comp * strideColComp() + i * strideColNode(),
								  offC + cell * strideColCell() + comp * strideColComp() + j * strideColNode(), 0.0));
						}
					}
				}
			}
				...
				for (unsigned int comp = 0; comp < _nComp; comp++)
				{
					for (unsigned int i = 0; i < _nNodes; i++)
					{
						for (unsigned int j = 0; j < 3 * _nNodes; j++)
						{
							...
							if ((j >= _nNodes - 1 && j <= 2 * _nNodes) || (i == 0 && j <= 2 * _nNodes) ||
								(i == _nNodes - 1 && j >= _nNodes - 1))
								// row: jump over inlet DOFs and previous cells, add component offset and go node
								// strides from there for each entry col: jump over inlet DOFs and previous cells, go
								// back one cell, add component offset and go node strides from there for each entry
								tripletList.push_back(T(
									offC + cell * strideColCell() + comp * strideColComp() + i * strideColNode(),
		...
		if (_nCells != 1)
		{ // Note: special case _nCells = 1 already set by advection block
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int i = 0; i < _nNodes; i++)
				{
				...
				for (unsigned int i = 0; i < _nNodes; i++)
				{
					for (unsigned int j = 0; j < 2 * _nNodes; j++)
					{
						...
						if ((j >= _nNodes - 1 && j <= 2 * _nNodes) || (i == 0 && j <= 2 * _nNodes) ||
							...
							tripletList.push_back(
								T(offC + (_nCells - 1) * strideColCell() + comp * strideColComp() + i * strideColNode(),
								  offC + (_nCells - 1 - 1) * strideColCell() + comp * strideColComp() +
									  j * strideColNode(),
								  0.0));
					}
				}
	...
	/**
	 * @brief sets the sparsity pattern of the convection dispersion Jacobian for the exact integration (her: modal) DG
	 * scheme
	 */
	int ConvDispModalPattern(std::vector<T>& tripletList, const int offC = 0)
	{
		...
		{ // forward flow upwind entry -> last node of previous cell
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int i = 0; i < _nNodes; i++)
				{
					// tripletList.push_back(T(offC + comp * strideColComp() + i * strideColNode(), comp *
					// strideColComp(), 0.0)); // inlet DOFs not included in Jacobian
					for (unsigned int j = 1; j < _nNodes + 1; j++)
					{
						tripletList.push_back(T(offC + comp * strideColComp() + i * strideColNode(),
												offC + comp * strideColComp() + (j - 1) * strideColNode(), 0.0));
			...
			for (unsigned int cell = 2; cell < _nCells - 2; cell++)
			{
				for (unsigned int comp = 0; comp < _nComp; comp++)
				{
					for (unsigned int i = 0; i < _nNodes; i++)
					{
						...
						{
							// row: jump over inlet DOFs and previous cells, add component offset and go node strides
							// from there for each dispersion block entry col: jump over inlet DOFs and previous cells,
							// go back one cell and one node, add component offset and go node strides from there for
							// each dispersion block entry
							tripletList.push_back(
								T(offC + cell * strideColCell() + comp * strideColComp() + i * strideColNode(),
								  offC + cell * strideColCell() - (_nNodes + 1) * strideColNode() +
									  comp * strideColComp() + j * strideColNode(),
								  0.0));
						}
					}
				}
		...
		if (_nCells >= 4u)
		{
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int i = 0; i < _nNodes; i++)
				{
					...
					{
						// row: jump over inlet DOFs and previous cell, add component offset and go node strides from
						// there for each dispersion block entry col: jump over inlet DOFs, add component offset and go
						// node strides from there for each dispersion block entry. Also adjust for iterator j (-1)
						tripletList.push_back(
							T(offC + _nNodes * strideColNode() + comp * strideColComp() + i * strideColNode(),
							  offC + comp * strideColComp() + (j - 1) * strideColNode(), 0.0));
					}
				}
			}
		...
		{ // special case: only depends on the two neighbouring cells
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int i = 0; i < _nNodes; i++)
				{
		...
		if (_nCells >= 4u)
		{
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int i = 0; i < _nNodes; i++)
				{
					...
					{
						// row: jump over inlet DOFs and previous cells, add component offset and go node strides from
						// there for each dispersion block entry col: jump over inlet DOFs and previous cells, go back
						// one cell and one node, add component offset and go node strides from there for each
						// dispersion block entry.
						tripletList.push_back(
							T(offC + (_nCells - 2) * strideColCell() + comp * strideColComp() + i * strideColNode(),
							  offC + (_nCells - 2) * strideColCell() - (_nNodes + 1) * strideColNode() +
								  comp * strideColComp() + j * strideColNode(),
							  0.0));
					}
				}
		...
		for (unsigned int comp = 0; comp < _nComp; comp++)
		{
			for (unsigned int i = 0; i < _nNodes; i++)
			{
				...
				{
					// row: jump over inlet DOFs, add component offset and go node strides from there for each
					// dispersion block entry col: jump over inlet DOFs, add component offset, adjust for iterator j
					// (-Nnodes-1) and go node strides from there for each dispersion block entry.
					tripletList.push_back(T(offC + comp * strideColComp() + i * strideColNode(),
			...
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int i = 0; i < _nNodes; i++)
				{
					for (unsigned int j = 0; j < 2 * _nNodes + 1; j++)
					{
						...
						tripletList.push_back(
							T(offC + (_nCells - 1) * strideColCell() + comp * strideColComp() + i * strideColNode(),
							  offC + (_nCells - 1) * strideColCell() - (_nNodes + 1) * strideColNode() +
								  comp * strideColComp() + j * strideColNode(),
							  0.0));
					}
				}
		...
		{ // special case for _nCells == 2: depends only on left cell
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int i = 0; i < _nNodes; i++)
				{
					for (unsigned int j = 0; j < 2 * _nNodes; j++)
					{
						// row: jump over inlet DOFs and previous cells, add component offset and go node strides from
						// there for each dispersion block entry col: jump over inlet DOFs and previous cells, go back
						// one cell, add component offset and go node strides from there for each dispersion block
						// entry.
						tripletList.push_back(
							T(offC + (_nCells - 1) * strideColCell() + comp * strideColComp() + i * strideColNode(),
							  offC + (_nCells - 1) * strideColCell() - _nNodes * strideColNode() +
								  comp * strideColComp() + j * strideColNode(),
							  0.0));
					}
				}
	 ...
	 */
	int calcConvDispCollocationDGSEMJacobian(Eigen::SparseMatrix<double, Eigen::RowMajor>& jacobian,
											 Eigen::MatrixXd& jacInlet, const int offC = 0)
	{
		...
		if (_nCells >= 3u)
		{
			...
			linalg::BandedEigenSparseRowIterator jacIt(
				jacobian, offC + strideColCell()); // row iterator starting at second cell and component
					...
					for (unsigned int comp = 0; comp < _nComp; comp++, ++jacIt)
					{
						for (unsigned int j = 0; j < dispBlock.cols(); j++)
						{
							...
							if ((j >= _nNodes - 1 && j <= 2 * _nNodes) || (i == 0 && j <= 2 * _nNodes) ||
								...
								jacIt[-strideColCell() + (j - i) * strideColNode()] =
									dispBlock(i, j) * static_cast<double>(currentDispersion(_curSection)[comp]);
						}
					}
				}
			}
		...
		if (_nCells != 1u)
		{ // "standard" case
			linalg::BandedEigenSparseRowIterator jacIt(jacobian,
													   offC); // row iterator starting at first cell and component
				...
				for (unsigned int comp = 0; comp < _nComp; comp++, ++jacIt)
				{
					for (unsigned int j = _nNodes; j < dispBlock.cols(); j++)
					{
						...
						if ((j >= _nNodes - 1 && j <= 2 * _nNodes) || (i == 0 && j <= 2 * _nNodes) ||
							...
							// row: iterator is at current node i and current component comp
							// col: jump to node j
							jacIt[((j - _nNodes) - i) * strideColNode()] =
								dispBlock(i, j) * static_cast<double>(currentDispersion(_curSection)[comp]);
					}
				}
			}
		...
		{ // "standard" case
			...
			linalg::BandedEigenSparseRowIterator jacIt(
				jacobian, offC + (_nCells - 1) * strideColCell()); // row iterator starting at last cell
		...
		if (_curVelocity >= 0.0)
		{ // forward flow upwind convection
			// special inlet DOF treatment for first cell (inlet boundary cell)
			jacInlet(0, 0) =
				static_cast<double>(_curVelocity) * convBlock(0, 0); // only first node depends on inlet concentration
				...
				for (unsigned int i = 0; i < convBlock.rows(); i++, jacIt += strideColBound)
				{
					for (unsigned int comp = 0; comp < _nComp; comp++, ++jacIt)
					{
						for (unsigned int j = 0; j < convBlock.cols(); j++)
						{
							...
							jacIt[-strideColNode() + (j - i) * strideColNode()] +=
								static_cast<double>(_curVelocity) * convBlock(i, j);
						}
					}
				}
			}
		...
		{ // backward flow upwind convection
			// non-inlet cells
			for (unsigned int cell = 0; cell < _nCells - 1u; cell++)
			{
				for (unsigned int i = 0; i < convBlock.rows(); i++, jacIt += strideColBound)
			...
			jacInlet(0, 0) =
				static_cast<double>(_curVelocity) *
	 ...
	 */
	void insertCompDepLiquidJacBlock(Eigen::MatrixXd block, linalg::BandedEigenSparseRowIterator& jac, int offCol,
									 unsigned int nCells, const active* Compfactor)
	{
	 ...
	 * @param [in] nCells determines how often the block is added (diagonally)
	 */
	void addLiquidJacBlock(Eigen::MatrixXd block, linalg::BandedEigenSparseRowIterator& jac, int offCol,
						   unsigned int nCells)
	{
	...
	/**
	 * @brief analytically calculates the convection dispersion jacobian for the exact integration (here: modal) DG
	 * scheme
	 */
								  ...
								  const int offC = 0)
	{
		...
		{
			linalg::BandedEigenSparseRowIterator jacIt(
				jacobian, offC + strideColCell() * 2); // row iterator starting at third cell, first component
			// insert all (nCol - 4) inner cell blocks
			insertCompDepLiquidJacBlock(_DGjacAxDispBlocks[2], jacIt, -(strideColCell() + strideColNode()),
										_nCells - 4u, currentDispersion(_curSection));
		}
		...
		{
			linalg::BandedEigenSparseRowIterator jacIt(
				jacobian, offC + strideColCell()); // row iterator starting at second cell, first component

			insertCompDepLiquidJacBlock(_DGjacAxDispBlocks[1].block(0, 1, _nNodes, 3 * _nNodes + 1), jacIt,
										-strideColCell(), 1u, currentDispersion(_curSection));

			jacIt += (_nCells - 4) * strideColCell(); // move iterator to preultimate cell (already at third cell)
			insertCompDepLiquidJacBlock(_DGjacAxDispBlocks[_nCells > 4 ? 3 : 2].block(0, 0, _nNodes, 3 * _nNodes + 1),
										jacIt, -(strideColCell() + strideColNode()), 1u,
										currentDispersion(_curSection));
		}
		...
		if (_nCells >= 3)
		{
			...
			linalg::BandedEigenSparseRowIterator jacIt(jacobian,
													   offC); // row iterator starting at first cell, first component

			insertCompDepLiquidJacBlock(_DGjacAxDispBlocks[0].block(0, _nNodes + 1, _nNodes, 2 * _nNodes + 1), jacIt, 0,
										1u, currentDispersion(_curSection));

			jacIt += (_nCells - 2) * strideColCell(); // move iterator to last cell (already at second cell)
			insertCompDepLiquidJacBlock(
				_DGjacAxDispBlocks[std::min(_nCells, 5u) - 1u].block(0, 0, _nNodes, 2 * _nNodes + 1), jacIt,
				-(strideColCell() + strideColNode()), 1u, currentDispersion(_curSection));
		}
		...
		if (_nCells == 1)
		{
			...
			insertCompDepLiquidJacBlock(_DGjacAxDispBlocks[0].block(0, _nNodes + 1, _nNodes, _nNodes), jacIt, 0, 1u,
										currentDispersion(_curSection));
		}
		else if (_nCells == 2)
		{
			linalg::BandedEigenSparseRowIterator jacIt(jacobian,
													   offC); // row iterator starting at first cell, first component
			// left Bacobian block
			insertCompDepLiquidJacBlock(_DGjacAxDispBlocks[0].block(0, _nNodes + 1, _nNodes, 2 * _nNodes), jacIt, 0, 1u,
										currentDispersion(_curSection));
			// right Bacobian block, iterator is already moved to second cell
			insertCompDepLiquidJacBlock(_DGjacAxDispBlocks[1].block(0, 1, _nNodes, 2 * _nNodes), jacIt,
										-strideColCell(), 1u, currentDispersion(_curSection));
		}
		else if (_nCells == 3)
		{
			linalg::BandedEigenSparseRowIterator jacIt(
				jacobian, offC + strideColCell()); // row iterator starting at first cell, first component
			insertCompDepLiquidJacBlock(_DGjacAxDispBlocks[1].block(0, 1, _nNodes, 3 * _nNodes), jacIt,
										-strideColCell(), 1u, currentDispersion(_curSection));
		}
		...
		{ // Forward flow
			...
			jacInlet = static_cast<double>(_curVelocity) *
					   _DGjacAxConvBlock.col(0); // only first cell depends on inlet concentration
			addLiquidJacBlock(static_cast<double>(_curVelocity) * _DGjacAxConvBlock.block(0, 1, _nNodes, _nNodes), jac,
							  0, 1);
			if (_nCells > 1) // iterator already moved to second cell
				addLiquidJacBlock(static_cast<double>(_curVelocity) * _DGjacAxConvBlock, jac, -strideColNode(),
								  _nCells - 1);
		}
		else
		{ // Backward flow
			...
			jacInlet =
				static_cast<double>(_curVelocity) *
				_DGjacAxConvBlock.col(_DGjacAxConvBlock.cols() - 1); // only last cell depends on inlet concentration
			addLiquidJacBlock(static_cast<double>(_curVelocity) * _DGjacAxConvBlock.block(0, 0, _nNodes, _nNodes), jac,
							  0, 1);
		}
 ...
 */
template <typename BaseOperator> class ConvectionDispersionOperatorDG
{
public:
	...
	int residual(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, active* res,
				 Eigen::SparseMatrix<double, Eigen::RowMajor>& jac);
	int residual(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, double* res,
				 bool wantJac, WithoutParamSensitivity);
	int residual(const IModel& model, double t, unsigned int secIdx, active const* y, double const* yDot, active* res,
				 bool wantJac, WithParamSensitivity);
	int residual(const IModel& model, double t, unsigned int secIdx, active const* y, double const* yDot, active* res,
				 bool wantJac, WithoutParamSensitivity);
	int residual(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, active* res,
				 bool wantJac, WithParamSensitivity);
	...
	{
		return _baseOp.currentVelocity(pos);
	}
	inline bool forwardFlow() const CADET_NOEXCEPT
	{
		return _baseOp.forwardFlow();
	}

	inline double cellLeftBound(unsigned int idx) const CADET_NOEXCEPT
	{
		return _baseOp.cellLeftBound(idx);
	}
	inline double relativeCoordinate(unsigned int idx) const CADET_NOEXCEPT
	{
		return _baseOp.relativeCoordinate(idx);
	}

	inline Eigen::SparseMatrix<double, Eigen::RowMajor>& jacobian() CADET_NOEXCEPT
	{
		return _jacC;
	}
	inline const Eigen::SparseMatrix<double, Eigen::RowMajor>& jacobian() const CADET_NOEXCEPT
	{
		return _jacC;
	}

	inline Eigen::SparseMatrix<double, Eigen::RowMajor>& jacobianDisc() CADET_NOEXCEPT
	{
		return _jacCdisc;
	}
	inline const Eigen::SparseMatrix<double, Eigen::RowMajor>& jacobianDisc() const CADET_NOEXCEPT
	{
		return _jacCdisc;
	}
...
// extern template class ConvectionDispersionOperatorDG<RadialConvectionDispersionOperatorBaseDG>; // todo radial flow
// DG

typedef ConvectionDispersionOperatorDG<AxialConvectionDispersionOperatorBaseDG> AxialConvectionDispersionOperatorDG;
// typedef ConvectionDispersionOperatorDG<RadialConvectionDispersionOperatorBaseDG>
// RadialConvectionDispersionOperatorDG; // todo radial flow DG
