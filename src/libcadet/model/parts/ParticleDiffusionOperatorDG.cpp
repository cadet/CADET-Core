...
namespace parts
{
			...
			for (unsigned int comp = 0; comp < nComp; comp++)
			{
				...
				for (int bnd = 0; bnd < _nBound[parType * _nComp + comp]; bnd++)
				{
					if (parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)] != 0.0) // some bound states might still not be effected by surface diffusion
					{
						...
						sum_cp_cs += static_cast<ParamType>(_invBetaP[_nComp * parType + comp]) * static_cast<ParamType>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]) * c_s;
				...
				if (_hasSurfaceDiffusion[parType])
				{
					for (int bnd = 0; bnd < _nBound[parType * _nComp + comp]; bnd++)
					{
						if (parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)] != 0.0) // some bound states might still not be effected by surface diffusion
						{
							...
							g_pSum += g_p.template cast<ResidualType>() * static_cast<ParamType>(_invBetaP[_nComp * parType + comp]) * static_cast<ParamType>(parSurfDiff[getOffsetSurfDiff(parType, comp, bnd)]);
	...
	*/
	unsigned int ParticleDiffusionOperatorDG::getOffsetSurfDiff(unsigned int parType, unsigned int comp, unsigned int bnd)
	{
	 ...
	 */
	void ParticleDiffusionOperatorDG::orderSurfDiff()
	{
		for (unsigned int type = 0; type < _nParType; type++) {
			for (unsigned int comp = 0; comp < _nComp; comp++) {
		...
		for (unsigned int i = 0; i < block.rows(); i++, jac += strideParLiquid())
		{
			for (unsigned int comp = 0; comp < _nComp; comp++)
			{
				for (unsigned int bnd = 0; bnd < _nBound[type * _nComp + comp]; bnd++, ++jac)
				{
					...
					if (static_cast<double>(surfDiffPtr[_offsetSurfDiff[offsetBoundComp(ParticleTypeIndex{ type }, ComponentIndex{ comp }) + bnd]]) != 0.0
						&& !reqBinding[offsetBoundComp(ParticleTypeIndex{ type }, ComponentIndex{ comp }) + bnd]) {
						// row, col: at current node and bound state
						jac[0] += block(i, i)
							* static_cast<double>(surfDiffPtr[_offsetSurfDiff[offsetBoundComp(ParticleTypeIndex{ type }, ComponentIndex{ comp }) + bnd]]);
					}
				}
			}
		}
	...
	bool ParticleDiffusionOperatorDG::setParameter(const ParameterId& pId, double value)
	{
		if (multiplexCompTypeSecParameterValue(pId, hashString("PAR_DIFFUSION"), _parDiffusionMode, _parDiffusion, _nParType, _nComp, value, nullptr))
			return true;
		if (multiplexBndCompTypeSecParameterValue(pId, hashString("PAR_SURFDIFFUSION"), _parSurfDiffusionMode, _parSurfDiffusion, _nParType, _nComp, _strideBound, _nBound, _boundOffset, value, nullptr))
			return true;
	...
	bool ParticleDiffusionOperatorDG::setSensitiveParameterValue(const std::unordered_set<active*>& sensParams, const ParameterId& pId, double value)
	{
		if (multiplexCompTypeSecParameterValue(pId, hashString("PAR_DIFFUSION"), _parDiffusionMode, _parDiffusion, _nParType, _nComp, value, &sensParams))
			return true;
		if (multiplexBndCompTypeSecParameterValue(pId, hashString("PAR_SURFDIFFUSION"), _parSurfDiffusionMode, _parSurfDiffusion, _nParType, _nComp, _strideBound, _nBound, _boundOffset, value, &sensParams))
			return true;
	...
	bool ParticleDiffusionOperatorDG::setSensitiveParameter(std::unordered_set<active*>& sensParams, const ParameterId& pId, unsigned int adDirection, double adValue)
	{
		...
		if (multiplexBndCompTypeSecParameterAD(pId, hashString("PAR_SURFDIFFUSION"), _parSurfDiffusionMode, _parSurfDiffusion, _nParType, _nComp, _strideBound, _nBound, _boundOffset, adDirection, adValue, sensParams))
		{
			LOG(Debug) << "Found parameter " << pId << ": Dir " << adDirection << " is set to " << adValue;
			return true;
		}
