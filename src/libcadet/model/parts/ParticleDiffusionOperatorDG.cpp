...
namespace parts
{
	 ...
	 */
	ParticleDiffusionOperatorDG::ParticleDiffusionOperatorDG() : _reqBinding(nullptr), _localFlux(nullptr), _deltaR(nullptr), _Ir(nullptr), _DGjacParDispBlocks(nullptr), _minus_InvMM_ST(nullptr), _parInvMM(nullptr), _parDepSurfDiffusion(nullptr)
	{
	}
	...
	{
		const int nBoundBeforeType = ParticleDiffusionOperatorBase::configureModelDiscretization(paramProvider, helper, nComp, parTypeIdx, nParType, strideBulkComp);
		...
		if (paramProvider.exists("PAR_SURFDIFFUSION"))
		{
			...
			{
				double const* const lsd = surfDiff.data() + nBoundBeforeType;
