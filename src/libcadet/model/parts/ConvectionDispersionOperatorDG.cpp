...
// #include "model/parts/AxialConvectionDispersionKernelDG.hpp" // todo radial flow DG and outsource residual
// implementation to kernel #include "model/parts/RadialConvectionDispersionKernelDG.hpp"
 ...
 */
bool AxialConvectionDispersionOperatorBaseDG::configureModelDiscretization(
	IParameterProvider& paramProvider, const IConfigHelper& helper, unsigned int nComp, int polynomial_integration_mode,
	unsigned int nCells, unsigned int polyDeg, unsigned int strideNode)
{

	_nComp = nComp;
	_exactInt = static_cast<bool>(
 ...
 * @return @c true if configuration went fine, @c false otherwise
 */
bool AxialConvectionDispersionOperatorBaseDG::configure(UnitOpIdx unitOpIdx, IParameterProvider& paramProvider,
														std::unordered_map<ParameterId, active*>& parameters)
{
	...
	// we only need unique dispersion blocks, which are given by cells 1, 2, nCol for inexact integration DG and by
	// cells 1, 2, 3, nCol-1, nCol for eaxct integration DG
	...
	// note that convection jacobian block is computet in notifyDiscontinuousSectionTransition() since this block needs
	// to be recomputed when flow direction changes
 ...
 */
bool AxialConvectionDispersionOperatorBaseDG::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx,
																				   MatrixXd& jacInlet)
{
	...
	{ // forward flow upwind convection
		if (_exactInt)
			...
			jacInlet(0, 0) = static_cast<double>(_curVelocity) *
							 _DGjacAxConvBlock(0, 0); // only first node depends on inlet concentration
	}
	else
	{ // backward flow upwind convection
		if (_exactInt)
			...
			jacInlet(0, 0) =
				static_cast<double>(_curVelocity) *
				_DGjacAxConvBlock(_DGjacAxConvBlock.rows() - 1,
								  _DGjacAxConvBlock.cols() - 1); // only last node depends on inlet concentration
	}
 ...
 */
void AxialConvectionDispersionOperatorBaseDG::setFlowRates(const active& in, const active& out,
 ...
 */
int AxialConvectionDispersionOperatorBaseDG::residual(const IModel& model, double t, unsigned int secIdx,
													  double const* y, double const* yDot, double* res,
													  Eigen::SparseMatrix<double, Eigen::RowMajor>& jac)
{
	...
	return residualImpl<double, double, double, linalg::BandedEigenSparseRowIterator, true>(model, t, secIdx, y, yDot,
																							res, jacIt);
}
...
{
	return residualImpl<double, double, double, linalg::BandedEigenSparseRowIterator, false>(
		model, t, secIdx, y, yDot, res, linalg::BandedEigenSparseRowIterator());
}

int AxialConvectionDispersionOperatorBaseDG::residual(const IModel& model, double t, unsigned int secIdx,
													  active const* y, double const* yDot, active* res,
													  WithoutParamSensitivity)
{
	return residualImpl<active, active, double, linalg::BandedEigenSparseRowIterator, false>(
		model, t, secIdx, y, yDot, res, linalg::BandedEigenSparseRowIterator());
}

int AxialConvectionDispersionOperatorBaseDG::residual(const IModel& model, double t, unsigned int secIdx,
													  double const* y, double const* yDot, active* res,
													  Eigen::SparseMatrix<double, Eigen::RowMajor>& jac)
{
	...
	return residualImpl<double, active, active, linalg::BandedEigenSparseRowIterator, true>(model, t, secIdx, y, yDot,
																							res, jacIt);
}
...
{
	return residualImpl<double, active, active, linalg::BandedEigenSparseRowIterator, false>(
		model, t, secIdx, y, yDot, res, linalg::BandedEigenSparseRowIterator());
}
...
int AxialConvectionDispersionOperatorBaseDG::residual(const IModel& model, double t, unsigned int secIdx,
													  ...
													  WithParamSensitivity)
{
	return residualImpl<active, active, active, linalg::BandedEigenSparseRowIterator, false>(
		model, t, secIdx, y, yDot, res, linalg::BandedEigenSparseRowIterator());
}

template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac>
int AxialConvectionDispersionOperatorBaseDG::residualImpl(const IModel& model, double t, unsigned int secIdx,
														  ...
														  RowIteratorType jacBegin)
{
	for (unsigned int comp = 0; comp < _nComp; comp++)
	{
		...
		Eigen::Map<const Vector<StateType, Dynamic>, 0, InnerStride<Dynamic>> _C(y + offsetC() + comp, _nPoints,
																				 InnerStride<Dynamic>(_strideNode));
		Eigen::Map<Vector<ResidualType, Dynamic>, 0, InnerStride<Dynamic>> _resC(res + offsetC() + comp, _nPoints,
																				 InnerStride<Dynamic>(_strideNode));
		Eigen::Map<Vector<ResidualType, Dynamic>, 0, InnerStride<>> _h(reinterpret_cast<ResidualType*>(_subsState),
																	   _nPoints, InnerStride<>(1));
		Eigen::Map<Vector<StateType, Dynamic>, 0, InnerStride<>> _g(reinterpret_cast<StateType*>(_auxState), _nPoints,
																	InnerStride<>(1));

		// Add time derivative to bulk residual
		if (yDot)
		{
			Eigen::Map<const VectorXd, 0, InnerStride<Dynamic>> _Cdot(yDot + offsetC() + comp, _nPoints,
																	  InnerStride<Dynamic>(_strideNode));
		...
		_h = 2.0 / static_cast<ParamType>(_deltaZ) *
			 (-u * _C + d_ax * (-2.0 / static_cast<ParamType>(_deltaZ)) * _g).template cast<ResidualType>();
		...
		// DG surface integral in strong form
		surfaceIntegral<ResidualType, ResidualType>(&_h[0], res + offsetC() + comp, 1u, _nNodes, _strideNode,
													_strideCell);
	}
 ...
 */
int AxialConvectionDispersionOperatorBaseDG::calcStaticAnaJacobian(Eigen::SparseMatrix<double, RowMajor>& jacobian,
																   Eigen::MatrixXd& jacInlet, const int bulkOffset)
{
...
unsigned int AxialConvectionDispersionOperatorBaseDG::nConvDispEntries(bool pureNNZ)
{
	...
	if (_exactInt)
	{
		...
		{
			return _nComp *
				   ((3u * _nCells - 2u) * _nNodes * _nNodes + (2u * _nCells - 3u) * _nNodes); // dispersion entries
		}
 ...
 */
void AxialConvectionDispersionOperatorBaseDG::multiplyWithDerivativeJacobian(const SimulationTime& simTime,
 ...
 */
void AxialConvectionDispersionOperatorBaseDG::addTimeDerivativeToJacobian(
	double alpha, Eigen::SparseMatrix<double, Eigen::RowMajor>& jacDisc, unsigned int blockOffset)
{
	const int gapCell = strideColNode() - static_cast<int>(_nComp) * strideColComp();
...
unsigned int AxialConvectionDispersionOperatorBaseDG::jacobianLowerBandwidth() const CADET_NOEXCEPT
{
	...
	// collocation DG: 2 * N_n * (N_c + N_q) + 1 = total bandwidth (main diagonal entries maximally depend on the next
...
bool AxialConvectionDispersionOperatorBaseDG::setSensitiveParameterValue(const std::unordered_set<active*>& sensParams,
...
bool AxialConvectionDispersionOperatorBaseDG::setSensitiveParameter(std::unordered_set<active*>& sensParams,
 ...
 */
template <typename Operator> ConvectionDispersionOperatorDG<Operator>::ConvectionDispersionOperatorDG()
{
}

template <typename Operator> ConvectionDispersionOperatorDG<Operator>::~ConvectionDispersionOperatorDG() CADET_NOEXCEPT
{
}
 ...
 */
template <typename Operator> int ConvectionDispersionOperatorDG<Operator>::requiredADdirs() const CADET_NOEXCEPT
{
...
bool ConvectionDispersionOperatorDG<Operator>::configureModelDiscretization(
	IParameterProvider& paramProvider, const IConfigHelper& helper, unsigned int nComp, int polynomial_integration_mode,
	unsigned int nCells, unsigned int polyDeg, unsigned int strideNode)
{
	const bool retVal = _baseOp.configureModelDiscretization(paramProvider, helper, nComp, polynomial_integration_mode,
															 nCells, polyDeg, strideNode);
	...
	//_jacDisc.resize((_disc.nComp + _disc.strideBound) * _disc.nPoints, (_disc.nComp + _disc.strideBound) *
	//_disc.nPoints);
 ...
 */
template <typename Operator>
bool ConvectionDispersionOperatorDG<Operator>::configure(UnitOpIdx unitOpIdx, IParameterProvider& paramProvider,
														 std::unordered_map<ParameterId, active*>& parameters)
{
 ...
 */
template <typename Operator>
bool ConvectionDispersionOperatorDG<Operator>::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx,
...
void ConvectionDispersionOperatorDG<Operator>::prepareADvectors(const AdJacobianParams& adJac) const
{
	...
	const unsigned int lowerColBandwidth = _baseOp.exactInt() ? 2 * _baseOp.nNodes() * _baseOp.strideColNode()
															  : _baseOp.nNodes() * _baseOp.strideColNode();
	const unsigned int upperColBandwidth = lowerColBandwidth;

	// Column block
	ad::prepareAdVectorSeedsForBandMatrix(adJac.adY + _baseOp.nComp(), adJac.adDirOffset,
										  _baseOp.nComp() * _baseOp.nPoints(), lowerColBandwidth, upperColBandwidth,
										  lowerColBandwidth);
}
 ...
 */
template <typename Operator>
void ConvectionDispersionOperatorDG<Operator>::setFlowRates(const active& in, const active& out,
															const active& colPorosity) CADET_NOEXCEPT
{
 ...
 */
template <typename Operator>
int ConvectionDispersionOperatorDG<Operator>::residual(const IModel& model, double t, unsigned int secIdx,
 ...
 */
template <typename Operator>
void ConvectionDispersionOperatorDG<Operator>::multiplyWithDerivativeJacobian(const SimulationTime& simTime,
																			  double const* sDot, double* ret) const
{
 ...
 */
template <typename Operator>
void ConvectionDispersionOperatorDG<Operator>::extractJacobianFromAD(active const* const adRes,
 ...
 */
template <typename Operator>
double ConvectionDispersionOperatorDG<Operator>::checkAnalyticJacobianAgainstAd(active const* const adRes,
																				unsigned int adDirOffset) const
{
	// todo implement this function
	//// Column
	// const double maxDiffCol = ad::compareBandedJacobianWithAd(adRes + offsetC(), adDirOffset, _jacC.lowerBandwidth(),
	// _jacC); LOG(Debug) << "-> Col block diff: " << maxDiffCol;
 ...
 */
template <typename Operator> void ConvectionDispersionOperatorDG<Operator>::assembleDiscretizedJacobian(double alpha)
{
 ...
 * @param [in] alpha Factor in front of @f$ \frac{\partial F}{\partial \dot{y}} @f$
 */
template <typename Operator> void ConvectionDispersionOperatorDG<Operator>::addTimeDerivativeToJacobian(double alpha)
{
...
{
	// todo: this functionality is currently not needed for DG since we assemble and factorize a global jacobian, not
	// blocks (as in FV)
 ...
 */
template <typename Operator> bool ConvectionDispersionOperatorDG<Operator>::solveDiscretizedJacobian(double* rhs) const
{
	// todo: this functionality is currently not needed for DG since we assemble and factorize a global jacobian, not
	// blocks (as in FV)
 ...
 */
template <typename Operator>
bool ConvectionDispersionOperatorDG<Operator>::solveTimeDerivativeSystem(const SimulationTime& simTime,
																		 double* const rhs)
{
	// Assemble
