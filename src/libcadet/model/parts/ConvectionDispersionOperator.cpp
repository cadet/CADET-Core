 ...
 */
AxialConvectionDispersionOperatorBase::AxialConvectionDispersionOperatorBase()
	: _stencilMemory(sizeof(active) * Weno::maxStencilSize()), _wenoDerivatives(new double[Weno::maxStencilSize()]),
	  _weno(), _dispersionDep(nullptr)
{
}
 ...
 */
bool AxialConvectionDispersionOperatorBase::configureModelDiscretization(IParameterProvider& paramProvider,
 ...
 * @return @c true if configuration went fine, @c false otherwise
 */
bool AxialConvectionDispersionOperatorBase::configure(UnitOpIdx unitOpIdx, IParameterProvider& paramProvider,
 ...
 */
void AxialConvectionDispersionOperatorBase::setFlowRates(const active& in, const active& out,
 ...
 */
int AxialConvectionDispersionOperatorBase::residual(const IModel& model, double t, unsigned int secIdx, double const* y,
													double const* yDot, double* res, linalg::BandMatrix& jac)
{
	// Reset Jacobian
	jac.setAll(0.0);

	return residualImpl<double, double, double, linalg::BandMatrix::RowIterator, true>(model, t, secIdx, y, yDot, res,
																					   jac.row(0));
}

int AxialConvectionDispersionOperatorBase::residual(const IModel& model, double t, unsigned int secIdx, double const* y,
													double const* yDot, double* res, WithoutParamSensitivity)
{
	return residualImpl<double, double, double, linalg::BandMatrix::RowIterator, false>(
		model, t, secIdx, y, yDot, res, linalg::BandMatrix::RowIterator());
}

int AxialConvectionDispersionOperatorBase::residual(const IModel& model, double t, unsigned int secIdx, active const* y,
													double const* yDot, active* res, WithoutParamSensitivity)
{
...
int AxialConvectionDispersionOperatorBase::residual(const IModel& model, double t, unsigned int secIdx, double const* y,
													double const* yDot, active* res, linalg::BandMatrix& jac)
{
	// Reset Jacobian
	jac.setAll(0.0);

	return residualImpl<double, active, active, linalg::BandMatrix::RowIterator, true>(model, t, secIdx, y, yDot, res,
																					   jac.row(0));
}

int AxialConvectionDispersionOperatorBase::residual(const IModel& model, double t, unsigned int secIdx, double const* y,
													double const* yDot, active* res, WithParamSensitivity)
{
...
int AxialConvectionDispersionOperatorBase::jacobian(const IModel& model, double t, unsigned int secIdx, double const* y,
													double const* yDot, double* res, linalg::BandMatrix& jac)
{
	// Reset Jacobian
	jac.setAll(0.0);

	return residualImpl<double, double, double, linalg::BandMatrix::RowIterator, true, false>(
		model, t, secIdx, y, nullptr, nullptr, jac.row(0));
}
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac,
		  bool wantRes>
int AxialConvectionDispersionOperatorBase::residualImpl(const IModel& model, double t, unsigned int secIdx,
														...
														RowIteratorType jacBegin)
{
	...
	convdisp::AxialFlowParameters<ParamType> fp{
		u,     d_c, h,      _wenoDerivatives, &_weno, &_stencilMemory, _wenoEpsilon, strideColCell(), _nComp,
		_nCol, 0u,  _nComp, _dispersionDep,   model};

	return convdisp::residualKernelAxial<StateType, ResidualType, ParamType, RowIteratorType, wantJac, wantRes>(
 ...
 */
void AxialConvectionDispersionOperatorBase::multiplyWithDerivativeJacobian(const SimulationTime& simTime,
 ...
 */
void AxialConvectionDispersionOperatorBase::addTimeDerivativeToJacobian(double alpha,
...
bool AxialConvectionDispersionOperatorBase::setSensitiveParameterValue(const std::unordered_set<active*>& sensParams,
...
bool AxialConvectionDispersionOperatorBase::setSensitiveParameter(std::unordered_set<active*>& sensParams,
 ...
 */
RadialConvectionDispersionOperatorBase::RadialConvectionDispersionOperatorBase()
	: _stencilMemory(sizeof(active) * Weno::maxStencilSize()), _dispersionDep(nullptr)
{
}
 ...
 * @return @c true if configuration went fine, @c false otherwise
 */
bool RadialConvectionDispersionOperatorBase::configureModelDiscretization(IParameterProvider& paramProvider,
																		  const IConfigHelper& helper,
																		  unsigned int nComp, unsigned int nCol,
																		  unsigned int strideCell)
{
 ...
 * @return @c true if configuration went fine, @c false otherwise
 */
bool RadialConvectionDispersionOperatorBase::configure(UnitOpIdx unitOpIdx, IParameterProvider& paramProvider,
													   std::unordered_map<ParameterId, active*>& parameters)
{
	// Read geometry parameters
	...
	parameters[makeParamId(hashString("COL_LENGTH"), unitOpIdx, CompIndep, ParTypeIndep, BoundStateIndep, ReactionIndep,
						   SectionIndep)] = &_colLength;
	parameters[makeParamId(hashString("COL_RADIUS_INNER"), unitOpIdx, CompIndep, ParTypeIndep, BoundStateIndep,
						   ReactionIndep, SectionIndep)] = &_innerRadius;
	parameters[makeParamId(hashString("COL_RADIUS_OUTER"), unitOpIdx, CompIndep, ParTypeIndep, BoundStateIndep,
						   ReactionIndep, SectionIndep)] = &_outerRadius;
 ...
 * @param [in] colPorosity Porosity used for computing interstitial velocity from volumetric flow rate
 */
void RadialConvectionDispersionOperatorBase::setFlowRates(const active& in, const active& out,
														  const active& colPorosity) CADET_NOEXCEPT
{
 ...
 * @return @c 0 on success, @c -1 on non-recoverable error, and @c +1 on recoverable error
 */
int RadialConvectionDispersionOperatorBase::residual(const IModel& model, double t, unsigned int secIdx,
...
int RadialConvectionDispersionOperatorBase::jacobian(const IModel& model, double t, unsigned int secIdx,
													 double const* y, double const* yDot, double* res,
													 linalg::BandMatrix& jac)
{
	// Reset Jacobian
...
template <typename StateType, typename ResidualType, typename ParamType, typename RowIteratorType, bool wantJac,
		  bool wantRes>
int RadialConvectionDispersionOperatorBase::residualImpl(const IModel& model, double t, unsigned int secIdx,
														 StateType const* y, double const* yDot, ResidualType* res,
														 RowIteratorType jacBegin)
{
	const ParamType u = static_cast<ParamType>(_curVelocity);
	...
	convdisp::RadialFlowParameters<ParamType> fp{u,
												 d_rad,
												 _cellCenters.data(),
												 _cellSizes.data(),
												 _cellBounds.data(),
	...
	return convdisp::residualKernelRadial<StateType, ResidualType, ParamType, RowIteratorType, wantJac, wantRes>(
		SimulationTime{t, secIdx}, y, yDot, res, jacBegin, fp);
}
 ...
 * @param [out] ret Vector @f$ z @f$ which stores the result of the operation
 */
void RadialConvectionDispersionOperatorBase::multiplyWithDerivativeJacobian(const SimulationTime& simTime,
																			double const* sDot, double* ret) const
{
	double* localRet = ret + offsetC();
	double const* localSdot = sDot + offsetC();
 ...
 * @param [in] alpha Factor in front of @f$ \frac{\partial F}{\partial \dot{y}} @f$
 */
void RadialConvectionDispersionOperatorBase::addTimeDerivativeToJacobian(double alpha,
																		 linalg::FactorizableBandMatrix& jacDisc)
{
	const int gapCell = strideColCell() - static_cast<int>(_nComp) * strideColComp();
	linalg::FactorizableBandMatrix::RowIterator jac = jacDisc.row(0);
...
bool RadialConvectionDispersionOperatorBase::setSensitiveParameterValue(const std::unordered_set<active*>& sensParams,
																		const ParameterId& pId, double value)
{
	// Check if parameter is in parameter dependence of column dispersion coefficient
	if (_dispersionDep)
...
bool RadialConvectionDispersionOperatorBase::setSensitiveParameter(std::unordered_set<active*>& sensParams,
																   const ParameterId& pId, unsigned int adDirection,
																   double adValue)
{
	// Check if parameter is in parameter dependence of column dispersion coefficient
 ...
 */
template <typename Operator> ConvectionDispersionOperator<Operator>::ConvectionDispersionOperator()
{
}

template <typename Operator> ConvectionDispersionOperator<Operator>::~ConvectionDispersionOperator() CADET_NOEXCEPT
{
}
 ...
 */
template <typename Operator> int ConvectionDispersionOperator<Operator>::requiredADdirs() const CADET_NOEXCEPT
{
	return _jacC.stride();
}
...
bool ConvectionDispersionOperator<Operator>::configureModelDiscretization(IParameterProvider& paramProvider,
																		  const IConfigHelper& helper,
																		  unsigned int nComp, unsigned int nCol)
{
 ...
 */
template <typename Operator>
bool ConvectionDispersionOperator<Operator>::configure(UnitOpIdx unitOpIdx, IParameterProvider& paramProvider,
													   std::unordered_map<ParameterId, active*>& parameters)
{
 ...
 */
template <typename Operator>
bool ConvectionDispersionOperator<Operator>::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx,
 ...
 */
template <typename Operator>
void ConvectionDispersionOperator<Operator>::setFlowRates(const active& in, const active& out,
														  const active& colPorosity) CADET_NOEXCEPT
{
 ...
 */
template <typename Operator>
int ConvectionDispersionOperator<Operator>::residual(const IModel& model, double t, unsigned int secIdx,
 ...
 */
template <typename Operator>
int ConvectionDispersionOperator<Operator>::jacobian(const IModel& model, double t, unsigned int secIdx,
													 double const* y, double const* yDot, double* res)
{
 ...
 */
template <typename Operator>
void ConvectionDispersionOperator<Operator>::multiplyWithDerivativeJacobian(const SimulationTime& simTime,
																			double const* sDot, double* ret) const
{
 ...
 */
template <typename Operator>
double ConvectionDispersionOperator<Operator>::checkAnalyticJacobianAgainstAd(active const* const adRes,
 ...
 */
template <typename Operator> void ConvectionDispersionOperator<Operator>::assembleDiscretizedJacobian(double alpha)
{
 ...
 * @param [in] alpha Factor in front of @f$ \frac{\partial F}{\partial \dot{y}} @f$
 */
template <typename Operator> void ConvectionDispersionOperator<Operator>::addTimeDerivativeToJacobian(double alpha)
{
 ...
 */
template <typename Operator> bool ConvectionDispersionOperator<Operator>::solveDiscretizedJacobian(double* rhs) const
{
