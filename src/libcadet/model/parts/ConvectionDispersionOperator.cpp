...
/**
 * @brief Creates an AxialConvectionDispersionOperator
 */
AxialConvectionDispersionOperator::AxialConvectionDispersionOperator()
{
}

AxialConvectionDispersionOperator::~AxialConvectionDispersionOperator() CADET_NOEXCEPT
{
}
 ...
 */
int AxialConvectionDispersionOperator::requiredADdirs() const CADET_NOEXCEPT
{
	return _jacC.stride();
}
 ...
 * @return @c true if configuration went fine, @c false otherwise
 */
bool AxialConvectionDispersionOperator::configureModelDiscretization(IParameterProvider& paramProvider, unsigned int nComp, unsigned int nCol)
{
 ...
 * @return @c true if configuration went fine, @c false otherwise
 */
bool AxialConvectionDispersionOperator::configure(UnitOpIdx unitOpIdx, IParameterProvider& paramProvider, std::unordered_map<ParameterId, active*>& parameters)
{
 ...
 * @return @c true if flow direction has changed, otherwise @c false
 */
bool AxialConvectionDispersionOperator::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx, const AdJacobianParams& adJac)
{
 ...
 * @param [in,out] adJac Jacobian information for AD (AD vectors for residual and state, direction offset)
 */
void AxialConvectionDispersionOperator::prepareADvectors(const AdJacobianParams& adJac) const
{
 ...
 * @param [in] colPorosity Porosity used for computing interstitial velocity from volumetric flow rate
 */
void AxialConvectionDispersionOperator::setFlowRates(const active& in, const active& out, const active& colPorosity) CADET_NOEXCEPT
{
 ...
 * @return @c 0 on success, @c -1 on non-recoverable error, and @c +1 on recoverable error
 */
int AxialConvectionDispersionOperator::residual(double t, unsigned int secIdx, double const* y, double const* yDot, double* res, bool wantJac, WithoutParamSensitivity)
{
	if (wantJac)
...
int AxialConvectionDispersionOperator::residual(double t, unsigned int secIdx, active const* y, double const* yDot, active* res, bool wantJac, WithoutParamSensitivity)
{
	return _baseOp.residual(t, secIdx, y, yDot, res, WithoutParamSensitivity());
}

int AxialConvectionDispersionOperator::residual(double t, unsigned int secIdx, active const* y, double const* yDot, active* res, bool wantJac, WithParamSensitivity)
{
	return _baseOp.residual(t, secIdx, y, yDot, res, WithParamSensitivity());
}

int AxialConvectionDispersionOperator::residual(double t, unsigned int secIdx, double const* y, double const* yDot, active* res, bool wantJac, WithParamSensitivity)
{
	if (wantJac)
		return _baseOp.residual(t, secIdx, y, yDot, res, _jacC);
	else
 ...
 * @param [out] ret Vector @f$ z @f$ which stores the result of the operation
 */
void AxialConvectionDispersionOperator::multiplyWithDerivativeJacobian(const SimulationTime& simTime, double const* sDot, double* ret) const
{
 ...
 */
void AxialConvectionDispersionOperator::extractJacobianFromAD(active const* const adRes, unsigned int adDirOffset)
{
 ...
 */
double AxialConvectionDispersionOperator::checkAnalyticJacobianAgainstAd(active const* const adRes, unsigned int adDirOffset) const
{
	// Column
 ...
 */
void AxialConvectionDispersionOperator::assembleDiscretizedJacobian(double alpha)
{
 ...
 * @param [in] alpha Factor in front of @f$ \frac{\partial F}{\partial \dot{y}} @f$
 */
void AxialConvectionDispersionOperator::addTimeDerivativeToJacobian(double alpha)
{
 ...
 */
bool AxialConvectionDispersionOperator::assembleAndFactorizeDiscretizedJacobian(double alpha)
{
 ...
 */
bool AxialConvectionDispersionOperator::solveDiscretizedJacobian(double* rhs) const
{
 ...
 */
bool AxialConvectionDispersionOperator::solveTimeDerivativeSystem(const SimulationTime& simTime, double* const rhs)
{
	// Assemble
