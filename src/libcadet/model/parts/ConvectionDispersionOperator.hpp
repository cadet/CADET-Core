...
public:
	...
	bool configureModelDiscretization(IParameterProvider& paramProvider, const IConfigHelper& helper,
									  unsigned int nComp, unsigned int nCol, unsigned int strideCell);
	...
	int jacobian(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, double* res,
				 linalg::BandMatrix& jac);
	int jacobian(const IModel& model, double t, unsigned int secIdx, double const* y, double const* yDot, active* res,
				 linalg::BandMatrix& jac);
	...
	} // todo delete once DG has its own operator
...
public:
	...
	inline const active& innerRadius() const CADET_NOEXCEPT
	{
		return _innerRadius;
	}
	inline const active& outerRadius() const CADET_NOEXCEPT
	{
		return _outerRadius;
	}
	...
	inline double cellCenter(unsigned int idx) const CADET_NOEXCEPT
	{
		return static_cast<double>(_cellCenters[idx]);
	}
	inline double relativeCoordinate(unsigned int idx) const CADET_NOEXCEPT
	{
		return (0.5 + idx) / _nCol;
 ...
 */
template <typename BaseOperator> class ConvectionDispersionOperator
{
public:
	...
	inline double inletJacobianFactor() const CADET_NOEXCEPT
	{
		return _baseOp.inletJacobianFactor();
	}

	inline double cellCenter(unsigned int idx) const CADET_NOEXCEPT
	{
		return _baseOp.cellCenter(idx);
	}
	inline double relativeCoordinate(unsigned int idx) const CADET_NOEXCEPT
	{
