...
*
* @f[\begin{align}
		\frac{\partial c_i}{\partial t} + \frac{1 - \varepsilon_t}{\varepsilon_t} \frac{\partial q_{i}}{\partial t} &= -
u \frac{\partial c_i}{\partial z} + D_{\text{ax},i} \frac{\partial^2 c_i}{\partial z^2} \\ a \frac{\partial
q_i}{\partial t} &= f_{\text{iso}}(c, q) \end{align} @f]
...
protected:
	...
	void addTimeDerivativeToJacobianNode(linalg::BandedEigenSparseRowIterator& jac, const Indexer& idxr, double alpha,
										 double invBetaP) const;
	...
	public:
		...
		template <typename real_t> inline real_t& c(real_t* const data, unsigned int node, unsigned int comp) const
		{
			return data[offsetC() + comp * strideColComp() + node * strideColNode()];
		}
		template <typename real_t>
		inline const real_t& c(real_t const* const data, unsigned int node, unsigned int comp) const
		{
			return data[offsetC() + comp * strideColComp() + node * strideColNode()];
		}
	...
	{
	public:
		Exporter(const Discretization& disc, const LumpedRateModelWithoutPoresDG& model, double const* data)
	...
	void setPattern(Eigen::SparseMatrix<double, RowMajor>& mat, bool stateDer, bool has_reaction)
	{
		...
		unsigned int reaction_entries =
			has_reaction ? _disc.nPoints * _disc.nComp * (_disc.strideBound + _disc.nComp) : 0;
		...
		if (stateDer)
			stateDerPattern(
				tripletList); // only adds [ d convDisp / d q_t ] because main diagonal is already included !
	 ...
	 * @detail the main also diagonal belongs to the state derivative pattern but is not set here, so it should be set
	 * previously.
	 */
		...
		for (unsigned int point = 0; point < _disc.nPoints; point++)
		{
			for (unsigned int comp = 0; comp < _disc.nComp; comp++)
			{
				if (_disc.nBound[comp]) // either one or null
					...
					// column: jump over inlet and liquid states, add component offset and go node strides to find
					// corresponding solid states
					tripletList.push_back(
						T(offC + point * idxr.strideColNode() + comp * idxr.strideColComp(),
	 ...
	 * @detail Independent of the isotherm, all liquid and solid entries (so all entries, the isotherm could
	 * theoretically depend on) at a discrete point are set.
	 */
	void bindingAndReactionPattern(std::vector<T>& tripletList, bool has_reaction)
	{
		...
		// isotherm pattern: loop over all discrete points and solid states and add all liquid plus solid entries at
		// that solid state at that discrete point
		for (unsigned int point = 0; point < _disc.nPoints; point++)
		{
			for (unsigned int solid = 0; solid < _disc.strideBound; solid++)
			{
				...
				{
					// row:		  jump over inlet and previous discrete points, liquid concentration and add the offset
					// of the current bound state column:    jump over inlet and previous discrete points. add entries
					// for all liquid and bound concentrations (conc)
					tripletList.push_back(T(offC + idxr.strideColNode() * point + idxr.strideColLiquid() + solid,
											offC + idxr.strideColNode() * point + conc, 0.0));
				}
			}
		}

		// reaction pattern: loop over all discrete points and liquid states and add all liquid plus solid entries at
		// that liquid state at that discrete point
		if (has_reaction)
		{
			for (unsigned int point = 0; point < _disc.nPoints; point++)
			{
				for (unsigned int liquid = 0; liquid < _disc.nComp; liquid++)
				{
					for (unsigned int conc = 0; conc < _disc.nComp + _disc.strideBound; conc++)
					{
						// row:		  jump over inlet and previous discrete points, liquid concentration and add the
						// offset of the current bound state column:    jump over inlet and previous discrete points.
						// add entries for all liquid and bound concentrations (conc)
