...
#include "model/StirredTankModelConstVolume.hpp"
...
CSTRConstVolumeModel::CSTRConstVolumeModel(UnitOpIdx unitOpIdx) : UnitOperationBase(unitOpIdx), _nComp(0), _nParType(0), _nBound(nullptr), _boundOffset(nullptr), _strideBound(nullptr), _offsetParType(nullptr), 
...
CSTRConstVolumeModel::~CSTRConstVolumeModel() CADET_NOEXCEPT
{
	delete[] _boundOffset;
	delete[] _nBound;
	delete[] _strideBound;
...
bool CSTRConstVolumeModel::configure(IParameterProvider& paramProvider)
{
...
unsigned int CSTRConstVolumeModel::threadLocalMemorySize() const CADET_NOEXCEPT
{
...
void CSTRConstVolumeModel::setSectionTimes(double const* secTimes, bool const* secContinuity, unsigned int nSections)
{
}

void CSTRConstVolumeModel::useAnalyticJacobian(const bool analyticJac)
{
#ifndef CADET_CHECK_ANALYTIC_JACOBIAN
...
void CSTRConstVolumeModel::notifyDiscontinuousSectionTransition(double t, unsigned int secIdx, const ConstSimulationState& simState, const AdJacobianParams& adJac)
{
...
void CSTRConstVolumeModel::reportSolution(ISolutionRecorder& recorder, double const* const solution) const
{
...
void CSTRConstVolumeModel::reportSolutionStructure(ISolutionRecorder& recorder) const
{
	Exporter expr(_nComp, _nParType, _nBound, _strideBound, _boundOffset, _totalBound, nullptr);
	recorder.unitOperationStructure(_unitOpIdx, *this, expr);
}

unsigned int CSTRConstVolumeModel::requiredADdirs() const CADET_NOEXCEPT
{
#ifndef CADET_CHECK_ANALYTIC_JACOBIAN
...
void CSTRConstVolumeModel::prepareADvectors(const AdJacobianParams& adJac) const
{
...
void CSTRConstVolumeModel::applyInitialCondition(const SimulationState& simState) const
{
	// Inlet DOFs
...
void CSTRConstVolumeModel::readInitialCondition(IParameterProvider& paramProvider)
{
...
int CSTRConstVolumeModel::residualImpl(double t, unsigned int secIdx, StateType const* const y, double const* const yDot, ResidualType* const res, LinearBufferAllocator tlmAlloc)
{
...
int CSTRConstVolumeModel::residual(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res,
...
int CSTRConstVolumeModel::residualWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double* const res, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
	return residual(simTime, simState, res, adJac, threadLocalMem, true, false);
}

int CSTRConstVolumeModel::residualSensFwdAdOnly(const SimulationTime& simTime, const ConstSimulationState& simState, active* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
...
int CSTRConstVolumeModel::residualSensFwdWithJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, const AdJacobianParams& adJac, util::ThreadLocalStorage& threadLocalMem)
{
...
void CSTRConstVolumeModel::initializeSensitivityStates(const std::vector<double*>& vecSensY) const
{
	const unsigned int nDof = numPureDofs();
...
void CSTRConstVolumeModel::consistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
	std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	// TODO: Handle v = 0
...
void CSTRConstVolumeModel::multiplyWithDerivativeJacobian(const SimulationTime& simTime, const ConstSimulationState& simState, double const* sDot, double* ret)
{
...
void CSTRConstVolumeModel::addTimeDerivativeJacobian(double t, double alpha, const ConstSimulationState& simState, MatrixType& mat)
{
	double const* const c = simState.vecStateY + _nComp;
	double const* const q = simState.vecStateY + 2 * _nComp;
 ...
 */
void CSTRConstVolumeModel::extractJacobianFromAD(active const* const adRes, unsigned int adDirOffset)
{
 ...
 * @param [in] adDirOffset Number of AD directions used for non-Jacobian purposes (e.g., parameter sensitivities)
 */
void CSTRConstVolumeModel::checkAnalyticJacobianAgainstAd(active const* const adRes, unsigned int adDirOffset) const
{
...
unsigned int CSTRConstVolumeModel::Exporter::numSolidPhaseDofs() const CADET_NOEXCEPT
{
	return _totalBound;
}

int CSTRConstVolumeModel::Exporter::writeMobilePhase(double* buffer) const
{
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
}

int CSTRConstVolumeModel::Exporter::writeSolidPhase(double* buffer) const
{
	if (_totalBound == 0)
		return 0;
...
int CSTRConstVolumeModel::Exporter::writeSolidPhase(unsigned int parType, double* buffer) const
{
	if (_totalBound == 0)
		return 0;
...
int CSTRConstVolumeModel::Exporter::writeVolume(double* buffer) const
{
	*buffer = _data[2 * _nComp + _totalBound];
	return 1;
}

int CSTRConstVolumeModel::Exporter::writeInlet(unsigned int port, double* buffer) const
{
	cadet_assert(port == 0);
...
int CSTRConstVolumeModel::Exporter::writeInlet(double* buffer) const
{
	std::copy_n(_data, _nComp, buffer);
	return _nComp;
}

int CSTRConstVolumeModel::Exporter::writeOutlet(unsigned int port, double* buffer) const
{
	cadet_assert(port == 0);
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
...
int CSTRConstVolumeModel::Exporter::writeOutlet(double* buffer) const
{
	std::copy_n(_data + _nComp, _nComp, buffer);
	return _nComp;
}
