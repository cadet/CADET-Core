...
/**
* @file
* Defines the CSTR model with pH control as a unit operation.
*/

#ifndef LIBCADET_CSTRPH_HPP_
#define LIBCADET_CSTRPH_HPP_
...
/**
 * @brief Continuous stirred tank (reactor) model with pH control
 * @details This model describes a continuous stirred tank reactor (CSTR) with variable volume using the ``well mixed assumption'' and modeling pH
 * The model does not include particles.
 */
class CSTRpH : public UnitOperationBase
{
public:

	CSTRpH(UnitOpIdx unitOpIdx);
	virtual ~CSTRpH() CADET_NOEXCEPT;
	...
	static const char* identifier() { return "CSTR_PH"; }
