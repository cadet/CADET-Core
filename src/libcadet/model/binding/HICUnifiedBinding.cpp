...
/*<codegen>
{
	...
	"constantParameters":
		[
			{ "type": "ScalarParameter", "varName": "refC1", "default": 0.0, "objName": "referencePh", "confName": "HICUNI_REFC1"},
...
/* Parameter description
 ...
 rh0 = osmotic effect of salt concentration on water activity. Calculated as osmotic_coefficient * molar_weight_of_water * ion_number
 refC1 = reference modifier component (defaults to 0)
*/
	...
	namespace model
	{
		...
		protected:
				...
				CpStateType const* yCp, ResidualType* res, LinearBufferAllocator workSpace) const
			{
				...
				const ParamType refC1 = static_cast<ParamType>(_p->refC1);
				...
				for (int i = 2; i < _nComp; ++i)
				{
					...
					const CpStateParamType kApH = kA * exp(kALin * (pH - refC1));
					const CpStateParamType nupH = nu + nuLin * (pH - refC1);
			...
			void jacobianImpl(double t, unsigned int secIdx, const ColumnPosition& colPos, double const* y, double const* yCp, int offsetCp, RowIterator jac, LinearBufferAllocator workSpace) const
			{
				...
				const double refC1 = static_cast<double>(_p->refC1);
				...
				for (int i = 2; i < _nComp; ++i)
				{
					...
					const double kApH = kA * exp(kALin * (pH - refC1));
					const double nupH = nu + nuLin * (pH - refC1);
