...
namespace cadet
{
	...
	namespace model
	{
		...
		inline const char* ExtHICWHSParamHandler::identifier() CADET_NOEXCEPT { return "EXT_HICWHS"; }
		...
		protected:
				...
				CpStateType const* yCp, ResidualType* res, LinearBufferAllocator workSpace) const
			{
				...
				for (int i = 0; i < _nComp; ++i)
				{
					...
					qSum += y[bndIdx];
					qSumOverqMax += y[bndIdx] / qMax;
					freeBindingSites -= y[bndIdx] / qMax;
				...
				for (int i = 0; i < _nComp; ++i)
				{
					...
					// If qSum, i.e. the term raised to nu*beta is <= zero: use Taylor series approximation
					if (qSum <= 0)
					{
					...
					{
						res[bndIdx] = kD * y[bndIdx] * pow(qSum, nu * beta)
							- kA * pow(freeBindingSites, nu) * yCp[i];
					}
			...
			void jacobianImpl(double t, unsigned int secIdx, const ColumnPosition& colPos, double const* y, double const* yCp, int offsetCp, RowIterator jac, LinearBufferAllocator workSpace) const
			{
				...
				for (int i = 0; i < _nComp; ++i)
				{
					...
					qSum += q_i;
					qSumOverqMax += q_i / static_cast<double>(_p->qMax[i]);
					freeBindingSites -= q_i / static_cast<double>(_p->qMax[i]);
				...
				for (int i = 0; i < _nComp; ++i)
				{
					...
					if (qSum <= 0)
					{
						// if q below zero the jacobian is computed for the Taylor series defined above
