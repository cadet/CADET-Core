	...
	namespace model
	{
		...
		protected:
				...
				CpStateType const* yCp, ResidualType* res, LinearBufferAllocator workSpace) const
			{
				...
				const ParamType water_activity = static_cast<ParamType>(0.1);
				...
				for (int i = 0; i < _nComp; ++i)
				{
					...
					CpStateParamType bulk_water = pow(water_activity, nu * beta);
					StateParamType qSumPowNu = pow(qSum, nu);

					res[bndIdx] = kD * y[bndIdx] * bulk_water - kA * qSumPowNu * yCp[i];
			...
			void jacobianImpl(double t, unsigned int secIdx, const ColumnPosition& colPos, double const* y, double const* yCp, int offsetCp, RowIterator jac, LinearBufferAllocator workSpace) const
			{
				...
				for (int i = 0; i < _nComp; ++i)
				{
					...
					// dres_i / dc_{p,0}
					jac[-bndIdx - offsetCp] = static_cast<double>(std::log(water_activity)) * beta0 * beta1 * kD * y[bndIdx] * nu * exp(beta1 * yCp[0]) * bulk_water;
