...
"COL_PROTEIN_RADIUS"}, { "type": "ScalarComponentDependentParameter", "varName": "kKin", "confName": "COL_KKIN"}
		],
	"constantParameters":
		[
...
inline bool ColloidalParamHandler::validateConfig(unsigned int nComp, unsigned int const* nBoundStates)
{
	if ((_kEqPhExp.size() != _kEqSaltPowerExp.size()) || (_kEqPhExp.size() != _kEqSaltPowerFact.size()) ||
		...
		throw InvalidParameterException(
			...
			"COL_LOGKEQ_SALT_EXPMULT, COL_PROTEIN_RADIUS, and COL_KKIN have to have the same size");
...
inline bool ExtColloidalParamHandler::validateConfig(unsigned int nComp, unsigned int const* nBoundStates)
{
	if ((_kEqPhExp.size() != _kEqSaltPowerExp.size()) || (_kEqPhExp.size() != _kEqSaltPowerFact.size()) ||
		...
		throw InvalidParameterException(
			...
			"EXTCOL_LOGKEQ_SALT_EXPMULT, COL_PROTEIN_RADIUS, and EXTCOL_KKIN have to have the same size");
...
protected:
				 ...
				 CpStateType const* yCp, ResidualType* res, LinearBufferAllocator workSpace) const
	{
		...
		else
		{
			const CpStateParamType kappa =
				1 / (pow(cpSalt, static_cast<ParamType>(p->kappaExp)) * static_cast<ParamType>(p->kappaFact) +
					  ...
					  int offsetCp, RowIterator jac, LinearBufferAllocator workSpace) const
	{
		...
		else
		{
			...
			const double kappa_dSalt = -1 / sqr(kappa_denom) * pow(cpSalt, static_cast<double>(p->kappaExp) - 1.0) *
