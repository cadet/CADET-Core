...
public:
	...
	{
		return !implementsAnalyticJacobian();
	}
...
protected:
	 ...
	 * @details The Jacobian can be calculated using automatic differentiation (AD) if analyticJacobian() is not
	 * implemented.
...
public:
	...
	virtual bool requiresWorkspace() const CADET_NOEXCEPT
	{
		return handler_t::requiresWorkspace() || BindingModelBase::requiresWorkspace();
	}

	virtual unsigned int workspaceSize(unsigned int nComp, unsigned int totalNumBoundStates,
									   unsigned int const* nBoundStates) const CADET_NOEXCEPT
	{
		return BindingModelBase::workspaceSize(nComp, totalNumBoundStates, nBoundStates) +
			   _paramHandler.cacheSize(nComp, totalNumBoundStates, nBoundStates);
	}
