 ...
 * @param [in] dest Destination matrix in dense storage format
 */
inline void copyMatrixSubset(const Eigen::MatrixXd& src, const ConstMaskArray& rowMask, const ConstMaskArray& colMask,
							 int rowOffset, int colOffset, detail::DenseMatrixBase& dest)
{
	cadet_assert(dest.rows() >= numMaskActive(rowMask));
	cadet_assert(dest.columns() >= numMaskActive(colMask));
	cadet_assert(src.rows() >= rowMask.len);
...
inline void copyMatrixSubset(const Eigen::SparseMatrix<double, Eigen::RowMajor>& src, const ConstMaskArray& rowMask,
							 const ConstMaskArray& colMask, int rowOffset, int colOffset, detail::DenseMatrixBase& dest)
{
	cadet_assert(dest.rows() >= numMaskActive(rowMask));
	cadet_assert(dest.columns() >= numMaskActive(colMask));
	cadet_assert(src.rows() >= rowMask.len);
