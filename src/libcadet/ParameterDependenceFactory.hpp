...
protected:
	 ...
	 */
	template <class ParamDep_t> void registerStateDependence(const std::string& name);

	/**
	 * @brief Registers an IParameterStateDependence
	 * @details The name of the parameter dependence is inferred from the static function
	 * IParameterStateDependence::identifier().
	 * @tparam ParamDep_t Type of the parameter dependence
	 */
	template <class ParamDep_t> void registerStateDependence();
	 ...
	 * @tparam ParamDep_t Type of the parameter dependence
	 */
	template <class ParamDep_t> void registerParameterDependence(const std::string& name);

	/**
	 * @brief Registers an IParameterParameterDependence
	 * @details The name of the parameter dependence is inferred from the static function
	 * IParameterParameterDependence::identifier().
	 * @tparam ParamDep_t Type of the parameter dependence
	 */
	template <class ParamDep_t> void registerParameterDependence();

	std::unordered_map<std::string, std::function<model::IParameterStateDependence*()>>
		_paramStateDeps; //!< Map with factory functions
	std::unordered_map<std::string, std::function<model::IParameterParameterDependence*()>>
		_paramParamDeps; //!< Map with factory functions
};
