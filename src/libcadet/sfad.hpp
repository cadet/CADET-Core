...
namespace sfad
{
	...
	public:
		...
		Fwd() : storage_t<real_t>(), _val(0)
		{
			setADValue(real_t(0));
		}
		Fwd(const real_t val) : storage_t<real_t>(), _val(val)
		{
			setADValue(real_t(0));
		}
		Fwd(const real_t val, real_t const* const grad) : storage_t<real_t>(), _val(val)
		{
			storage_t<real_t>::copyGradient(grad);
		}
		Fwd(const Fwd<real_t, storage_t>& cpy) : storage_t<real_t>(cpy), _val(cpy._val) { }
		Fwd(Fwd<real_t, storage_t>&& other) : storage_t<real_t>(std::move(other)), _val(std::move(other._val)) { }

		~Fwd() { }

		Fwd<real_t, storage_t>& operator=(Fwd<real_t, storage_t>&& other)
		{
		...
		const idx_t gradientSize() const { return detail::globalGradSize; }

		template<typename T, template <class T2> class s_t> friend void swap (Fwd<T, s_t>& x, Fwd<T, s_t>& y);
...
//				_grad[i] = (_grad[i] - _val / a._val * a._grad[i]) / a._val;
		...
		// Comparisons
		inline bool operator!= (const Fwd<real_t, storage_t>& v) const { return v != _val; }
		inline bool operator!= (const real_t v) const  { return v != _val; }
		inline friend bool operator!= (const real_t v, const Fwd<real_t, storage_t>& a) { return v != a._val; }

		inline bool operator== (const Fwd<real_t, storage_t>& v) const  { return v == _val; }
		inline bool operator== (const real_t v) const  { return v == _val; }
		inline friend bool operator== (const real_t v, const Fwd<real_t, storage_t>& a) { return v == a._val; }

		inline bool operator<= (const Fwd<real_t, storage_t>& v) const  { return _val <= v._val; }
		inline bool operator<= (const real_t v) const  { return _val <= v; }
		inline friend bool operator<= (const real_t v, const Fwd<real_t, storage_t>& a) { return v <= a._val; }
...
*/
	...
	template <typename real_t, template <class T> class storage_t>
	void swap(Fwd<real_t, storage_t>& x, Fwd<real_t, storage_t>& y)
	{
		using std::swap;
		swap(x._val, y._val);
