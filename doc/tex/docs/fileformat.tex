...
\texttt{INIT\_C} & A vector with initial concentrations for each comp.\ in the bulk mobile phase & \si{\mol\per\cubic\metre\of{Int}} & double & $\geq 0.0$ & \texttt{NCOMP}\\
\texttt{INIT\_CP} & A vector with initial concentrations for each comp.\ in the bead liquid phase (optional, \texttt{INIT\_C} is used if left out) & \si{\mol\per\cubic\metre\of{MP}} & double & $\geq 0.0$ & \texttt{NCOMP}\\
...
\midrule
\multicolumn{6}{l}{If \texttt{ADSORPTION\_MODEL} = \texttt{EXTERNAL\_MOBILE\_PHASE\_MODULATORS}:} \\ % Top level heading
\midrule
\begin{tabular}{@{}l@{}}
  \texttt{EXTMPM\_KA} \\
  \texttt{EXTMPM\_KA\_T} \\
