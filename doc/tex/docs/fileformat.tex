...
\begin{tikzpicture}[%
  ...
  \node {\hyperref[sec:FFInput]{input}} [edge from parent fork down]
    child[sibling distance=40mm] { node {\hyperref[tab:FFModelSystem]{model} } [edge from parent fork down]
...
\texttt{CONNECTIONS}& Matrix with list of connections in row-major storage. Columns are \emph{UnitOpID from}, \emph{UnitOpID to}, \emph{Component from}, \emph{Component to}. If the last two entries are $-1$, all components are connected. & -- & int & $\geq -1$ & $\texttt{NCONNECTIONS} \times 4$ \everyrow{}\\
\bottomrule
\end{tabu}
...
\texttt{INLET\_TYPE} & Specifies the type of inlet profile & -- & string & \texttt{PIECEWISE\_CUBIC\_POLY} & 1 \everyrow{}\\
\bottomrule
\end{tabu}
...
\end{tabular} & 1 \\
\texttt{MAX\_KRYLOV} & Defines the size of the Krylov subspace in the iterative linear SPGMR solver (0: \texttt{MAX\_KRYLOV} = \texttt{NCOL}) & -- & int & $0-\texttt{NCOL}$ & 1\\
...
\rowfont[c]\normalfont Dataset & Description & Unit & Type & Range & Length \everyrow{\midrule}\\      
\texttt{NTHREADS} & Number of used OpenMP threads & -- & int & $\geq 1$ & 1\\
...
\texttt{MAX\_STEPS} & Maximum number of timesteps taken by IDAS (0: IDAS default = 500), see IDAS guide 4.5, 36 & int & $\geq 0$ & 1 \everyrow{}\\
\bottomrule
\end{tabu}
