...
\texttt{COL\_DISPERSION} & Axial dispersion coefficient & \si{\square\metre\of{Int}\per\second} & double & $\geq 0.0$ & 1\\
\texttt{COL\_LENGTH} & Column length & \si{\metre} & double & $\geq 0.0$ & 1\\
\texttt{COL\_POROSITY} & Column porosity & -- & double & $\geq 0.0$ & 1\\
\texttt{FILM\_DIFFUSION} & A vector with film diffusion coefficients & \si{\metre\per\second} & double & $\geq 0.0$ & \texttt{NCOMP}\\
\texttt{PAR\_DIFFUSION} & A vector with particle diffusion coefficients & \si{\square\metre\of{MP}\per\second} & double & $\geq 0.0$ & \texttt{NCOMP}\\
\texttt{PAR\_POROSITY} & Particle porosity & -- & double & $\geq 0.0$ & 1\\
\texttt{PAR\_RADIUS} & Particle radius & \si{\metre} & double & $\geq 0.0$ & 1\\
\texttt{PAR\_SURFDIFFUSION} & A vector with particle surface diffusion coefficients & \si{\square\metre\of{SP}\per\second} & double & $\geq 0.0$ & \texttt{NCOMP}\\
\texttt{VELOCITY} & Interstitial velocity of the mobile phase & \si{\metre\per\second} & double & $\geq 0.0$ & 1\everyrow{}\\
\bottomrule
\end{tabu}
