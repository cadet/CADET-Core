...
\begin{align*}
  \frac{\mathrm{d} q_i}{\mathrm{d} t} = k_{a,i} c_{p,i}\left( \frac{\bar{q}_0 }{q_{\text{ref}}} \right)^{\frac{\nu_i}{\nu_0}} - k_{d,i}\: q_i\: \left(\frac{c_{p,0}}{c_{\text{ref}}}\right)^{\frac{\nu_i}{\nu_0}} && i = 1, \dots, N_{\text{comp}} - 1,
\end{align*}
...
\begin{align*}
  \frac{\mathrm{d} q_i}{\mathrm{d} t} &= c_{p,i}\left( \frac{\bar{q}_0}{q_{\text{ref}}} \right)^{\frac{\nu_i}{\nu_0}} \left[ k_{a,i,1} + k_{a,i,2} c_{p,i} \right] - k_{d,i}\: q_i\: \left(\frac{c_{p,0}}{c_{\text{ref}}}\right)^{\frac{\nu_i}{\nu_0}} && i = 1, \dots, N_{\text{comp}} - 1, \\
  \nu_0 q_0 &= \Lambda - \sum_{j=1}^{N_{\text{comp}} - 1} \nu_j q_j,
\end{align*}
...
\subsection{Extended Mobile Phase Modulator Langmuir}

This model is an extension of the mobile phase modulator Langmuir model (see Section~\ref{sec:MBMobilePhaseModulatorLangmuir}), which allows linear binding of some selected components. \index{Binding!Extended mobile phase modulator}
...
\begin{align*}
  \frac{\mathrm{d} q_{i,j}}{\mathrm{d} t} =& \phantom{+} k_{a,i}^{(j)} c_{p,i} \left(\frac{\bar{q}_0}{q_{\text{ref}}}\right)^{\frac{\nu_{i,j}}{\nu_0}} - k_{d,i}^{(j)}\: q_{i,j}\: \left(\frac{c_{p,0}}{c_{\text{ref}}}\right)^{\frac{\nu_{i,j}}{\nu_0}} \\
  &- \sum_{\ell = 0}^{j-1} \underbrace{k^{(i)}_{j\ell}\: q_{i,j}\: \left(\frac{c_{p,0}}{c_{\text{ref}}}\right)^{\frac{\nu_{i,j} - \nu_{i,\ell}}{\nu_0}}}_{\text{to weak state}} - \sum_{\ell = j+1}^{M_i - 1} \underbrace{k^{(i)}_{j\ell}\: q_{i,j}\: \left(\frac{\bar{q}_0}{q_{\text{ref}}}\right)^{\frac{\nu_{i,\ell} - \nu_{i,j}}{\nu_0}}}_{\text{to strong state}} \\
  &+ \sum_{\ell = 0}^{j-1} \underbrace{k^{(i)}_{\ell j}\: q_{i,\ell}\: \left(\frac{\bar{q}_0}{q_{\text{ref}}}\right)^{\frac{\nu_{i,j} - \nu_{i,\ell}}{\nu_0}}}_{\text{from weak state}} + \sum_{\ell = j+1}^{M_i - 1} \underbrace{k^{(i)}_{\ell j}\: q_{i,\ell}\: \left(\frac{c_{p,0}}{c_{\text{ref}}}\right)^{\frac{\nu_{i,\ell} - \nu_{i,j}}{\nu_0}}}_{\text{from strong state}} & \begin{aligned}
...
The generalized ion exchange model is based on the steric mass action model \cite{Huuk2017,Mollerup2008}. \index{Binding!Generalized ion exchange}
