...
The particle porosity $\varepsilon_p$ on the mobile phase side of the transport equations is replaced by the accessible porosity
\begin{align}
	\varepsilon_{p,i} = F_{\text{acc},i} \varepsilon_p
\end{align}
where the pore accessibility factor $F_{\text{acc},i}$ ranges in $(0, 1]$.

Small molecules which can enter any pore have $F_{\text{acc},i} = 1$ whereas larger molecules that can enter some but not small pores have values $0 < F_{\text{acc},i} < 1$.
