...
\begin{table}[!ht]
\centering
\begin{tabu}to \linewidth[m]{X>{\centering\arraybackslash}p{2cm}>{\centering\arraybackslash}p{2cm}>{\centering\arraybackslash}p{2cm}>{\centering\arraybackslash}p{2cm}} \toprule
\rowfont[c]\normalfont Unit operation model & Radial diffusion & Particle diffusion & Film diffusion & Multiple-particle \\ \midrule
2D General rate model & \cmark & \cmark & \cmark & \cmark \\
General rate model & \xmark & \cmark & \cmark & \cmark \\
Lumped rate model with pores & \xmark & \xmark & \cmark & \cmark \\
Lumped rate model without pores & \xmark & \xmark & \xmark & \xmark \\
Continuous stirred tank & \xmark & \xmark & \xmark & \cmark \\
\bottomrule
\end{tabu}
...
A particle type \index{Model!Multiple particle types}\index{Model!Particle types} has its own set of mass transfer parameters $\varepsilon_{p,j}$, $D_{p,j}$, $D_{s,j}$, etc (see Eq.~\eqref{eq:ModelBead}) and its own binding model $f_{\mathrm{ads}}$ (including a possibly differing number of bound states).
