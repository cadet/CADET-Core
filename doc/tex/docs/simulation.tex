...
The BDF order and size of the time step are chosen such that the error bounds are satisfied with minimal work.
If a time step fails (i.e., error bounds are not met), IDAS tries again with a smaller step size multiple times.
However, when a certain minimal step size is reached and the nonlinear equations could not be solved with acceptable error, IDAS quits and the simulation fails with an error message.

Error bounds are specified by an absolute tolerance (\texttt{ABSTOL}) and a relative tolerance (\texttt{RELTOL}).
