...
To debug specific tests (with flag [testHere]) from the Visual Studio IDE, you can add the following configuration to the launch.vs.json file mentioned in the [](#debugging) section:
...
To this end, it might be necessary to increase the maximum number of AD directions for your test case, which can be done via the cmake argument `NUM_MAX_AD_DIRS`, as described in the [](#build-options).
...
We utilize [CADET-Verification](https://github.com/cadet/CADET-Verification) and [CADET-RDM](https://github.com/cadet/CADET-RDM) to ensure reproducibility of the tests and thus maintainability and stability of the added modules.
...
Add (potentially reduced/minimallized) test cases, e.g. case studies from a paper publication to [CADET-Verification](https://github.com/cadet/CADET-Verification).
