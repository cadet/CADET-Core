...
For this purpose, we use [CADET-Python](#cadet-python), a file based frontend for CADET-Core.
CADET-Core still must be downloaded (or built from source) as explained in the [installation guide](#installation-core).
The Python frontend almost exactly maps to the documented [CADET file format](#file-format) except that all dataset names are lowercase.
...
From [file format](#axial-flow-column-1d-config), the path for this is `/input/model/unit_001/COL_POROSITY`.
...
In the `root` attribute of this object, the parameter structure is defined as described in the [file format reference](#FFIntroduction).
...
The available models are listed in the [unit operation chapter](#unit-operation-models).
The units of the different parameters and quantities are given in the corresponding [file format](#unit-operation-config) of the respective unit operation.
...
For the [general rate model model](#general-rate-model-model), we set the `unit_type` parameter of the corresponding unit operation model to `GENERAL_RATE_MODEL`.
In this group, we set the parameters related to transport and column geometry.
For a more detailed description of the parameters and their units, see the [corresponding file format](#axial-flow-column-1d-config).
...
The available models are listed in the [binding model chapter](#binding-models).

For the [Langmuir model](#multi-component-langmuir-model), we set the `adsorption_model` parameter of the corresponding unit operation model to `MULTI_COMPONENT_LANGMUIR`.
Then, we decide if we want to use the [rapid-equilibrium assumption](#binding-models) in the binding model (`is_kinetic = False`), which is not the case here (dynamic binding).
Finally, the parameters of the binding model have to be set for each component (they are vectors of length `n_components`).
They are described in the [corresponding file format specification](#multi-component-langmuir-config).
...
[Time sections](#SimTimeSectionsTransitions) are used to specify changes of parameter values during the simulation.
A section typically corresponds to an operating step (load, wash, elute etc.), but can also be used to indicate changes in connectivity, or even discontinuities of [model parameters](#section-dependent-parameters).
...
In order to specify the [connectivity of the network](#networks), we have to provide a list of connections.
...
For more information on the parameters, see the [file format specification](#FFModelSystemConnections).
:::
...
This restriction does not apply to a [CSTR model](#cstr-model), because it has a variable volume.
:::
...
For more information, see the [file format specification](#return).
...
The structure and format of the data is described in the [file format specification](#FFOutput).
...
[addict package]: https://github.com/mewwts/addict
