...
- ``reaction_phase_ZZZ`` refers to one of the phase-specific raction groups listed in :ref:`FFReaction`, e.g., ``reaction_liquid_ZZZ``, ``reaction_solid_ZZZ``, or ``reaction_particle_ZZZ`` (for particle type ``YYY``).
