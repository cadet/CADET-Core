...
For the [Langmuir model](#multi-component-langmuir-model), we use the `Langmuir` class.
Then, we decide if we want to use the [rapid-equilibrium assumption](#binding-models) in the binding model (`binding_model.is_kinetic = False`), which is not the case here (dynamic binding).
...
For the [general rate model model](#general-rate-model-model), we use the `GeneralRateModel` class.
...
For further details on the front-end and more examples please refer to the [CADET-Process](https://cadet-process.readthedocs.io/) documentation.
