...
This binding model is similar to the steric mass action model (see Section [](#steric-mass-action-model)) but is also capable of describing dimerization {cite}`Mollerup2008,Westerberg2012`.
...
For more information on model parameters required to define in CADET file format, see [](#self-association-config).
