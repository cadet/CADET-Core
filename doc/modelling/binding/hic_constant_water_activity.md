(hic-constant-water-activity-model)=
...
$$
\begin{align}     \beta &= \beta_0 e^{c_{p,0}\beta_1} \\     \frac{\mathrm{d}q_i}{\mathrm{d}t} &= k_{a,i} c_{p,i} \left( 1 - \sum_j \frac{q_j}{q_{max,j}} \right)^{\nu_i} - k_{d,i} q_i 0.1^{\nu_i \beta} \end{align}
$$
...
For more information on model parameters required to define in CADET file format, see [](#hic-constant-water-activity-config).
