.. 2d_lumped_rate_model_with_pores_model:
...
Using the inlet boundary condition Eq. :eq:`BCInlet2D`, each radial zone (``RAD_NELEM``in the ``discretization`` group) is equipped with its own inlet and outlet port.
