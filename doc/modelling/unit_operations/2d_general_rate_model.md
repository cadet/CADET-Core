(d-general-rate-model-model)=

# Two Dimensional General rate model (GRM2D)

The general rate model as introduced in Section [](#general-rate-model-model) assumes homogeneity in the cross sections of the column.
...
This model can be improved by introducing a radial coordinate $\rho \in [0, R]$, where $R$ is the column radius, in the interstitial volume Eq. {eq}`ModelColumn`:
...
> - $c^\ell_i\colon \left[0, T_{\text{end}}\right] \times [0, L] \times [0, R] \rightarrow \mathbb{R}^{\geq 0}$,
...
Note that, contrary to the standard general rate model as presented in Section [](#general-rate-model-model), the interstitial flow rate is always given by the volumetric flow rate.
The velocity parameter only determines the flow direction.

For information on model parameters see [](#axial-flow-column-2d-config) and [](#particle-model-config).
