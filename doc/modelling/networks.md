...
- The long format includes seven columns.
  ...
  Finally, the seventh column specifies the volumetric flow rate of this connection (see [](#FFModelConnectionSwitch)).
...
Each valve configuration consists of a network connectivity table as described in Section [](#MUOPNetworkConfig) and a section index.
...
See Tables [](#FFModelSystemConnections) and [](#FFModelConnectionSwitch).

(muopnetworkdynamicflowrates)=
...
(muopnetworklinearsolver)=

## Solution of the linear systems

Each time step in the simulation requires the solution of a nonlinear system Eq. {eq}`BDFNonlinSystem` (see Sec. [](#SimTimeIntegration)).
...
This default can be overridden by a flag (see Table [](#FFModelSolver)).
