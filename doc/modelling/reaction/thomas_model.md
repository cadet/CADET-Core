...
where $\beta = V_{\text{L}} / V_{\text{S}}$ is the liquid-solid phase ratio, $f_{\text{ads}}$ can be any single-component isotherm model, see [](#binding-models).
...
The above equations can be solved in CADET using [](#cstr-model) and configuring [](#multi-component-langmuir-model).
...
Since all mass transfer resistances within the beads are ignored in the Thomas model, it is reasonable to choose [](#lumped-rate-model-without-pores-model) and assign an isotherm model.

We note that the Thomas model was successfully applied in simple systems. For more complex systems, more advanced [](#reaction-models) should be considered.
