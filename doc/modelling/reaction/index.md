...
Reaction models describe the (net) fluxes $f_{\mathrm{react}}(c)$ of a
reaction mechanism.
...
For more information on the interface specification, see [](#FFReaction).
...
Further details on the interface specification are provided in [the corresponding section](#FFReaction).
...
(dependence-on-external-function-react)=
...
For the configuration of external function dependence and more information on model parameters required to define in CADET file format, see section [](#FFReaction).
