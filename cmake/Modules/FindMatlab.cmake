...
* ``TBB_LIBRARY`` the TBB library shipped with Matlab.
* ``UMFPACK_LIBRARY`` the UMFPACK library shipped with Matlab.
...
``Matlab_LAPACK_LIBRARY``
  ...
  is requested.
``Matlab_TBB_LIBRARY``
  Matlab's TBB library. Available only if the component ``TBB_LIBRARY``
  is requested.
``Matlab_UMFPACK_LIBRARY``
  Matlab's UMFPACK library. Available only if the component ``UMFPACK_LIBRARY``
  is requested.
...
# File modified by Samuel Leweke (Forschungszentrum Juelich GmbH) in 2020
# Added support for MKL, TBB, and UMFPACK bundled with Matlab
...
if("LAPACK_LIBRARY" IN_LIST Matlab_FIND_COMPONENTS)
  _Matlab_find_library(
    ${_matlab_lib_prefix_for_search}
    Matlab_BLAS_LIBRARY
    mwblas
...
if("TBB_LIBRARY" IN_LIST Matlab_FIND_COMPONENTS)
  _Matlab_find_library(
    ${_matlab_lib_prefix_for_search}
    Matlab_TBB_LIBRARY
    tbb libtbb.so.2
...
if("UMFPACK_LIBRARY" IN_LIST Matlab_FIND_COMPONENTS)
  _Matlab_find_library(
    ${_matlab_lib_prefix_for_search}
    Matlab_UMFPACK_LIBRARY
    mwumfpack
...
set(Matlab_LIBRARIES
  ...
  ${Matlab_LAPACK_LIBRARY} ${Matlab_BLAS_LIBRARY}
  ${Matlab_TBB_LIBRARY} ${Matlab_UMFPACK_LIBRARY})
